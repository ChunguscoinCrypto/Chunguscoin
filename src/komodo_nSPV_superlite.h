...
#ifndef CHUNGUSCOIN_NSPVSUPERLITE_H
#define CHUNGUSCOIN_NSPVSUPERLITE_H
...
// chunguscoin_nSPVresp is called from async message processing

void chunguscoin_nSPVresp(CNode *pfrom,std::vector<uint8_t> response) // received a response
{
...
// chunguscoin_nSPV from main polling loop (really this belongs in its own file, but it is so small, it ended up here)

void chunguscoin_nSPV(CNode *pto) // polling loop from SendMessages
{
...
UniValue NSPV_getinfo_json(struct NSPV_inforesp *ptr)
{
    ...
    result.push_back(Pair("result","success"));
    result.push_back(Pair("nSPV",CHUNGUSCOIN_NSPV==-1?"disabled":(CHUNGUSCOIN_NSPV_SUPERLITE?"superlite":"fullnode")));
...
#endif // CHUNGUSCOIN_NSPVSUPERLITE_H
