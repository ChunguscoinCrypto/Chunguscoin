...
// for l in `g 'parse notarisation' ~/.chunguscoin/debug.log | pyline 'l.split()[8]'`; do hoek decodeTx '{"hex":"'`src/chunguscoin-cli getrawtransaction "$l"`'"}' | jq '.outputs[1].script.op_return' | pyline 'import base64; print base64.b64decode(l).encode("hex")'; done
