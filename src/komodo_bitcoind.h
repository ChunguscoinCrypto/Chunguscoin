...
// chunguscoin functions that interact with bitcoind C++
...
int32_t chunguscoin_notaries(uint8_t pubkeys[64][33],int32_t height,uint32_t timestamp);
int32_t chunguscoin_electednotary(int32_t *numnotariesp,uint8_t *pubkey33,int32_t height,uint32_t timestamp);
int32_t chunguscoin_voutupdate(bool fJustCheck,int32_t *isratificationp,int32_t notaryid,uint8_t *scriptbuf,int32_t scriptlen,int32_t height,uint256 txhash,int32_t i,int32_t j,uint64_t *voutmaskp,int32_t *specialtxp,int32_t *notarizedheightp,uint64_t value,int32_t notarized,uint64_t signedmask,uint32_t timestamp);
unsigned int lwmaGetNextPOSRequired(const CBlockIndex* pindexLast, const Consensus::Params& params);
bool EnsureWalletIsAvailable(bool avoidException);
extern bool fRequestShutdown;
extern CScript CHUNGUSCOIN_EARLYTXID_SCRIPTPUB;
...
uint32_t chunguscoin_heightstamp(int32_t height);
...
char *chunguscoin_issuemethod(char *userpass,char *method,char *params,uint16_t port)
{
    //static void *cHandle;
...
int32_t notarizedtxid_height(char *dest,char *txidstr,int32_t *kmdnotarized_heightp)
{
    ...
    *kmdnotarized_heightp = 0;
    ...
    if ( userpass[0] != 0 )
    {
        ...
        {
            if ( (jsonstr= chunguscoin_issuemethod(userpass,(char *)"getinfo",params,port)) != 0 )
            {
                //LogPrintf("(%s)\n",jsonstr);
            ...
            if ( (jsonstr= chunguscoin_issuemethod(userpass,(char *)"getblockchaininfo",params,port)) != 0 )
            {
                //LogPrintf("(%s)\n",jsonstr);
                if ( (json= cJSON_Parse(jsonstr)) != 0 )
                {
        ...
        if ( (jsonstr= chunguscoin_issuemethod(userpass,(char *)"getrawtransaction",params,port)) != 0 )
        {
            //LogPrintf("(%s)\n",jsonstr);
            if ( (json= cJSON_Parse(jsonstr)) != 0 )
            {
...
int32_t chunguscoin_verifynotarizedscript(int32_t height,uint8_t *script,int32_t len,uint256 NOTARIZED_HASH)
{
...
void chunguscoin_reconsiderblock(uint256 blockhash)
{
    char params[256],*jsonstr,*hexstr;
    sprintf(params,"[\"%s\"]",blockhash.ToString().c_str());
    if ( (jsonstr= chunguscoin_issuemethod(ASSETCHAINS_USERPASS,(char *)"reconsiderblock",params,ASSETCHAINS_RPCPORT)) != 0 )
    {
        //LogPrintf("chunguscoin_reconsiderblock.(%s) (%s %u) -> (%s)\n",params,ASSETCHAINS_USERPASS,ASSETCHAINS_RPCPORT,jsonstr);
        free(jsonstr);
    }
    //LogPrintf("chunguscoin_reconsiderblock.(%s) (%s %u) -> NULL\n",params,ASSETCHAINS_USERPASS,ASSETCHAINS_RPCPORT);
}

int32_t chunguscoin_verifynotarization(char *symbol,char *dest,int32_t height,int32_t NOTARIZED_HEIGHT,uint256 NOTARIZED_HASH,uint256 NOTARIZED_DESTTXID)
{
    ...
    if ( strcmp(dest,"KMD") == 0 )
    {
        if ( KMDUSERPASS[0] != 0 )
        {
            ...
            {
                jsonstr = chunguscoin_issuemethod(KMDUSERPASS,(char *)"getrawtransaction",params,KMD_PORT);
    ...
    else if ( strcmp(dest,"BTC") == 0 )
    {
        if ( BTCUSERPASS[0] != 0 )
        {
            jsonstr = chunguscoin_issuemethod(BTCUSERPASS,(char *)"getrawtransaction",params,DEST_PORT);
        }
        //else jsonstr = _dex_getrawtransaction();
        else return(0);
    }
            ...
            if ( (txjson= jobj(json,(char *)"result")) != 0 && (vouts= jarray(&n,txjson,(char *)"vout")) != 0 )
            {
                ...
                if ( (skey= jobj(vout,(char *)"scriptPubKey")) != 0 )
                {
                    if ( (hexstr= jstr(skey,(char *)"hex")) != 0 )
                    {
                        ...
                        retval = chunguscoin_verifynotarizedscript(height,script,len,NOTARIZED_HASH);
                    }
                }
            }
            free_json(txjson);
...
CScript chunguscoin_makeopret(CBlock *pblock, bool fNew)
{
...
/*uint256 chunguscoin_getblockhash(int32_t height)
 {
 uint256 hash; char params[128],*hexstr,*jsonstr; cJSON *result; int32_t i; uint8_t revbuf[32];
 memset(&hash,0,sizeof(hash));
 sprintf(params,"[%d]",height);
 if ( (jsonstr= chunguscoin_issuemethod(KMDUSERPASS,(char *)"getblockhash",params,BITCOIND_RPCPORT)) != 0 )
 {
 ...
 uint256 _chunguscoin_getblockhash(int32_t height);*/

uint64_t chunguscoin_seed(int32_t height)
{
    uint64_t seed = 0;
    /*if ( 0 ) // problem during init time, seeds are needed for loading blockindex, so null seeds...
     ...
     height -= 10;
     if ( ASSETCHAINS_SYMBOL[0] == 0 )
     hash = _chunguscoin_getblockhash(height);
     if ( memcmp(&hash,&zero,sizeof(hash)) == 0 )
     hash = chunguscoin_getblockhash(height);
     int32_t i;
     for (i=0; i<32; i++)
     LogPrintf("%02x",((uint8_t *)&hash)[i]);
     LogPrintf(" seed.%d\n",height);
...
uint32_t chunguscoin_txtime(CScript &opret,uint64_t *valuep,uint256 hash, int32_t n, char *destaddr)
{
    CTxDestination address; CTransaction tx; uint256 hashBlock; int32_t numvouts;
    *valuep = 0;
    if (!GetTransaction(hash, tx,
#ifndef CHUNGUSCOIN_ZCASH
                        Params().GetConsensus(),
#endif
                        hashBlock, true))
    {
...
CBlockIndex *chunguscoin_getblockindex(uint256 hash)
{
    BlockMap::const_iterator it = mapBlockIndex.find(hash);
    return((it != mapBlockIndex.end()) ? it->second : NULL);
}

uint32_t chunguscoin_txtime2(uint64_t *valuep,uint256 hash,int32_t n,char *destaddr)
{
    ...
    *valuep = 0;
    ...
    if ( (pindex= chunguscoin_getblockindex(hashBlock)) != 0 )
        txtime = pindex->nTime;
...
int32_t chunguscoin_newStakerActive(int32_t height, uint32_t timestamp)
{
    if ( timestamp > nStakedDecemberHardforkTimestamp || chunguscoin_heightstamp(height) > nStakedDecemberHardforkTimestamp ) //December 2019 hardfork
        return(1);
    else return(0);
}

int32_t chunguscoin_hasOpRet(int32_t height, uint32_t timestamp)
{    
    return((ASSETCHAINS_MARMARA!=0 || chunguscoin_newStakerActive(height, timestamp) == 1));
}

bool chunguscoin_checkopret(CBlock *pblock, CScript &merkleroot)
{
    merkleroot = pblock->vtx.back().vout.back().scriptPubKey;
    return(merkleroot.IsOpReturn() && merkleroot == chunguscoin_makeopret(pblock, false));
}

bool chunguscoin_hardfork_active(uint32_t time)
{
...
uint256 chunguscoin_calcmerkleroot(CBlock *pblock, uint256 prevBlockHash, int32_t nHeight, bool fNew, CScript scriptPubKey)
{
    std::vector<uint256> vLeaves;
    // rereate coinbase tx
...
int32_t chunguscoin_isPoS(CBlock *pblock, int32_t height,CTxDestination *addressout)
{
    ...
    if ( ASSETCHAINS_STAKED != 0 )
    {
        ...
        if ( n > 1 && pblock->vtx[n-1].vin.size() == 1 && pblock->vtx[n-1].vout.size() == 1+chunguscoin_hasOpRet(height,pblock->nTime) )
        {
            txid = pblock->vtx[n-1].vin[0].prevout.hash;
            vout = pblock->vtx[n-1].vin[0].prevout.n;
            txtime = chunguscoin_txtime(opret,&value,txid,vout,destaddr);
            if ( ExtractDestination(pblock->vtx[n-1].vout[0].scriptPubKey,voutaddress) )
            {
                ...
                if ( chunguscoin_newStakerActive(height, pblock->nTime) != 0 ) 
                {
                    if ( DecodeStakingOpRet(pblock->vtx[n-1].vout[1].scriptPubKey, merkleroot) != 0 && chunguscoin_calcmerkleroot(pblock, pblock->hashPrevBlock, height, false, pblock->vtx[0].vout[0].scriptPubKey) == merkleroot )
                    {
                        return(1);
                    }
                }
...
void chunguscoin_disconnect(CBlockIndex *pindex,CBlock& block)
{
    char symbol[CHUNGUSCOIN_ASSETCHAIN_MAXLEN],dest[CHUNGUSCOIN_ASSETCHAIN_MAXLEN]; struct chunguscoin_state *sp;
    //LogPrintf("disconnect ht.%d\n",pindex->GetHeight());
    chunguscoin_init(pindex->GetHeight());
    if ( (sp= chunguscoin_stateptr(symbol,dest)) != 0 )
    {
        //sp->rewinding = pindex->GetHeight();
        //LogPrintf("-%d ",pindex->GetHeight());
    } else LogPrintf("chunguscoin_disconnect: ht.%d cant get chunguscoin_state.(%s)\n",pindex->GetHeight(),ASSETCHAINS_SYMBOL);
}

int32_t chunguscoin_is_notarytx(const CTransaction& tx)
{
...
int32_t chunguscoin_block2height(CBlock *block)
{
...
int32_t chunguscoin_block2pubkey33(uint8_t *pubkey33,CBlock *block)
{
    int32_t n;
    if ( CHUNGUSCOIN_LOADINGBLOCKS == 0 )
...
int32_t chunguscoin_blockload(CBlock& block,CBlockIndex *pindex)
{
    block.SetNull();
...
uint32_t chunguscoin_chainactive_timestamp()
{
...
CBlockIndex *chunguscoin_chainactive(int32_t height)
{
    if ( chainActive.LastTip() != 0 )
    {
        if ( height <= chainActive.LastTip()->GetHeight() )
            return(chainActive[height]);
        // else LogPrintf("chunguscoin_chainactive height %d > active.%d\n",height,chainActive.LastTip()->GetHeight());
    }
    //LogPrintf("chunguscoin_chainactive null chainActive.LastTip() height %d\n",height);
    return(0);
}
...
{
    CBlockIndex *ptr;
    if ( height > 0 && (ptr= chunguscoin_chainactive(height)) != 0 )
        return(ptr->nTime);
    //else LogPrintf("chunguscoin_heightstamp null ptr for block.%d\n",height);
    return(0);
}

/*void chunguscoin_pindex_init(CBlockIndex *pindex,int32_t height) gets data corrupted
{
    int32_t i,num; uint8_t pubkeys[64][33]; CBlock block;
    if ( pindex->didinit != 0 )
        return;
    //LogPrintf("pindex.%d chunguscoin_pindex_init notary.%d from height.%d\n",pindex->GetHeight(),pindex->notaryid,height);
    if ( pindex->didinit == 0 )
    {
        pindex->notaryid = -1;
        if ( CHUNGUSCOIN_LOADINGBLOCKS == 0 )
        ...
        {
            chunguscoin_block2pubkey33(pindex->pubkey33,&block);
            //for (i=0; i<33; i++)
            ...
            //    pindex->didinit = (CHUNGUSCOIN_LOADINGBLOCKS == 0);
        } // else LogPrintf("error loading block at %d/%d",pindex->GetHeight(),height);
    }
    if ( pindex->didinit != 0 && pindex->GetHeight() >= 0 && (num= chunguscoin_notaries(pubkeys,(int32_t)pindex->GetHeight(),(uint32_t)pindex->nTime)) > 0 )
    {
        for (i=0; i<num; i++)
        {
...
void chunguscoin_index2pubkey33(uint8_t *pubkey33,CBlockIndex *pindex,int32_t height)
{
    ...
    {
        if ( chunguscoin_blockload(block,pindex) == 0 )
            chunguscoin_block2pubkey33(pubkey33,&block);
    }
}

/*int8_t chunguscoin_minerid(int32_t height,uint8_t *destpubkey33)
{
    ...
    if ( (pindex= chainActive[height]) != 0 )
    {
        ...
        chunguscoin_index2pubkey33(pubkey33,pindex,height);
        if ( destpubkey33 != 0 )
            memcpy(destpubkey33,pindex->pubkey33,33);
        if ( pindex->didinit != 0 )
            return(pindex->notaryid);
        timestamp = pindex->GetBlockTime();
        if ( (num= chunguscoin_notaries(pubkeys,height,timestamp)) > 0 )
        {
            for (i=0; i<num; i++)
    ...
    LogPrintf("chunguscoin_minerid height.%d null pindex\n",height);
    return(chunguscoin_electednotary(&numnotaries,pubkey33,height,timestamp));
}*/

int32_t chunguscoin_eligiblenotary(uint8_t pubkeys[66][33],int32_t *mids,uint32_t blocktimes[66],int32_t *nonzpkeysp,int32_t height)
{
    ...
    n = chunguscoin_notaries(notarypubs33,height,0);
    for (i=duplicate=0; i<66; i++)
    {
        if ( (pindex= chunguscoin_chainactive(height-i)) != 0 )
        {
            blocktimes[i] = pindex->nTime;
            if ( chunguscoin_blockload(block,pindex) == 0 )
            {
                chunguscoin_block2pubkey33(pubkeys[i],&block);
                for (j=0; j<n; j++)
                {
...
int32_t chunguscoin_minerids(uint8_t *minerids,int32_t height,int32_t width)
{
    int32_t i,j,nonz,numnotaries; CBlock block; CBlockIndex *pindex; uint8_t notarypubs33[64][33],pubkey33[33];
    numnotaries = chunguscoin_notaries(notarypubs33,height,0);
    for (i=nonz=0; i<width; i++)
    {
        if ( height-i <= 0 )
            continue;
        if ( (pindex= chunguscoin_chainactive(height-width+i+1)) != 0 )
        {
            if ( chunguscoin_blockload(block,pindex) == 0 )
            {
                chunguscoin_block2pubkey33(pubkey33,&block);
...
int32_t chunguscoin_is_special(uint8_t pubkeys[66][33],int32_t mids[66],uint32_t blocktimes[66],int32_t height,uint8_t pubkey33[33],uint32_t blocktime)
{
    int32_t i,j,notaryid=0,minerid,limit,nid; uint8_t destpubkey33[33];
    chunguscoin_chosennotary(&notaryid,height,pubkey33,blocktimes[0]);
    if ( height >= 82000 )
    {
        if ( notaryid >= 0 )
        {
    ...
    else
    {
        if ( height >= 34000 && notaryid >= 0 )
        {
            ...
            for (i=1; i<limit; i++)
            {
                chunguscoin_chosennotary(&nid,height-i,pubkey33,blocktimes[i]);
                if ( nid == notaryid )
                {
                    //for (j=0; j<66; j++)
                    //    LogPrintf("%d ",mids[j]);
...
int32_t chunguscoin_MoM(int32_t *notarized_heightp,uint256 *MoMp,uint256 *kmdtxidp,int32_t nHeight,uint256 *MoMoMp,int32_t *MoMoMoffsetp,int32_t *MoMoMdepthp,int32_t *kmdstartip,int32_t *kmdendip)
{
    int32_t depth,notarized_ht; uint256 MoM,kmdtxid;
    depth = chunguscoin_MoMdata(&notarized_ht,&MoM,&kmdtxid,nHeight,MoMoMp,MoMoMoffsetp,MoMoMdepthp,kmdstartip,kmdendip);
...
CBlockIndex *chunguscoin_blockindex(uint256 hash)
{
...
int32_t chunguscoin_blockheight(uint256 hash)
{
    BlockMap::const_iterator it; CBlockIndex *pindex = 0;
    if ( (it = mapBlockIndex.find(hash)) != mapBlockIndex.end() )
    {
...
uint32_t chunguscoin_blocktime(uint256 hash)
{
    BlockMap::const_iterator it; CBlockIndex *pindex = 0;
    if ( (it = mapBlockIndex.find(hash)) != mapBlockIndex.end() )
    {
...
int32_t chunguscoin_checkpoint(int32_t *notarized_heightp,int32_t nHeight,uint256 hash)
{
    int32_t notarized_height,MoMdepth; uint256 MoM,notarized_hash,notarized_desttxid; CBlockIndex *notary,*pindex;
    if ( (pindex= chainActive.LastTip()) == 0 )
        return(-1);
    notarized_height = chunguscoin_notarizeddata(pindex->GetHeight(),&notarized_hash,&notarized_desttxid);
...
uint32_t chunguscoin_interest_args(uint32_t *txheighttimep,int32_t *txheightp,uint32_t *tiptimep,uint64_t *valuep,uint256 hash,int32_t n)
{
    LOCK(cs_main);
...
uint64_t chunguscoin_interest(int32_t txheight,uint64_t nValue,uint32_t nLockTime,uint32_t tiptime);

uint64_t chunguscoin_accrued_interest(int32_t *txheightp,uint32_t *locktimep,uint256 hash,int32_t n,int32_t checkheight,uint64_t checkvalue,int32_t tipheight)
{
    ...
    if ( (*locktimep= chunguscoin_interest_args(&txheighttimep,txheightp,&tiptime,&value,hash,n)) != 0 )
    {
        if ( (checkvalue == 0 || value == checkvalue) && (checkheight == 0 || *txheightp == checkheight) )
            return(chunguscoin_interest(*txheightp,value,*locktimep,tiptime));
        //LogPrintf("nValue %llu lock.%u:%u nTime.%u -> %llu\n",(long long)coins.vout[n].nValue,coins.nLockTime,timestamp,pindex->nTime,(long long)interest);
        else LogPrintf("chunguscoin_accrued_interest value mismatch %llu vs %llu or height mismatch %d vs %d\n",(long long)value,(long long)checkvalue,*txheightp,checkheight);
    }
    return(0);
}

int32_t chunguscoin_nextheight()
{
    CBlockIndex *pindex; int32_t ht;
    if ( (pindex= chainActive.LastTip()) != 0 && (ht= pindex->GetHeight()) > 0 )
        return(ht+1);
    else return(chunguscoin_longestchain() + 1);
}

int32_t chunguscoin_isrealtime(int32_t *kmdheightp)
{
    struct chunguscoin_state *sp; CBlockIndex *pindex;
    if ( (sp= chunguscoin_stateptrget((char *)"KMD")) != 0 )
        *kmdheightp = sp->CURRENT_HEIGHT;
    else *kmdheightp = 0;
    if ( (pindex= chainActive.LastTip()) != 0 && pindex->GetHeight() >= (int32_t)chunguscoin_longestchain() )
        return(1);
    else return(0);
}

int32_t chunguscoin_validate_interest(const CTransaction &tx,int32_t txheight,uint32_t cmptime,int32_t dispflag)
{
    dispflag = 1;
    if ( CHUNGUSCOIN_REWIND == 0 && ASSETCHAINS_SYMBOL[0] == 0 && (int64_t)tx.nLockTime >= LOCKTIME_THRESHOLD ) //1473793441 )
    {
        if ( txheight > 246748 )
        {
            if ( txheight < 247205 )
                cmptime -= 16000;
            if ( (int64_t)tx.nLockTime < cmptime-CHUNGUSCOIN_MAXMEMPOOLTIME )
            {
                if ( tx.nLockTime != 1477258935 && dispflag != 0 )
                {
                    LogPrintf("chunguscoin_validate_interest.%d reject.%d [%d] locktime %u cmp2.%u\n",dispflag,txheight,(int32_t)(tx.nLockTime - (cmptime-CHUNGUSCOIN_MAXMEMPOOLTIME)),(uint32_t)tx.nLockTime,cmptime);
                }
                return(-1);
            }
            if ( 0 && dispflag != 0 )
                LogPrintf("validateinterest.%d accept.%d [%d] locktime %u cmp2.%u\n",dispflag,(int32_t)txheight,(int32_t)(tx.nLockTime - (cmptime-CHUNGUSCOIN_MAXMEMPOOLTIME)),(int32_t)tx.nLockTime,cmptime);
        }
    }
    return(0);
}
...
/*
 chunguscoin_checkPOW (fast) is called early in the process and should only refer to data immediately available. it is a filter to prevent bad blocks from going into the local DB. The more blocks we can filter out at this stage, the less junk in the local DB that will just get purged later on.

 chunguscoin_checkPOW (slow) is called right before connecting blocks so all prior blocks can be assumed to be there and all checks must pass
...
uint64_t chunguscoin_commission(const CBlock *pblock,int32_t height)
{
...
uint32_t chunguscoin_segid32(char *coinaddr)
{
    bits256 addrhash;
...
int8_t chunguscoin_segid(int32_t nocache,int32_t height)
{
    ...
    if ( height > 0 && (pindex= chunguscoin_chainactive(height)) != 0 )
    {
        if ( nocache == 0 && pindex->segid >= -1 )
            return(pindex->segid);
        if ( chunguscoin_blockload(block,pindex) == 0 )
        {
            newStakerActive = chunguscoin_newStakerActive(height, block.nTime);
            txn_count = block.vtx.size();
            if ( txn_count > 1 && block.vtx[txn_count-1].vin.size() == 1 && block.vtx[txn_count-1].vout.size() == 1+chunguscoin_hasOpRet(height,pindex->nTime) )
            {
                ...
                if ( ExtractDestination(block.vtx[txn_count-1].vout[0].scriptPubKey,voutaddress) )
                {
                    ...
                    {
                        segid = chunguscoin_segid32(voutaddr) & 0x3f;
                        //LogPrintf( "chunguscoin_segid: ht.%i --> %i\n",height,pindex->segid);
                    }
                } //else LogPrintf("chunguscoin_segid ht.%d couldnt extract voutaddress\n",height);
            }
        }
        // The new staker sets segid in chunguscoin_checkPOW, this persists after restart by being saved in the blockindex for blocks past the HF timestamp, to keep backwards compatibility.
...
void chunguscoin_segids(uint8_t *hashbuf,int32_t height,int32_t n)
{
    ...
    else
    {
        ...
        for (i=0; i<n; i++)
        {
            hashbuf[i] = (uint8_t)chunguscoin_segid(0,height+i);
...
uint32_t chunguscoin_stakehash(uint256 *hashp,char *address,uint8_t *hashbuf,uint256 txid,int32_t vout)
{
    bits256 addrhash;
...
arith_uint256 chunguscoin_adaptivepow_target(int32_t height,arith_uint256 bnTarget,uint32_t nTime)
{
    arith_uint256 origtarget,easy; int32_t diff,tipdiff; int64_t mult; bool fNegative,fOverflow; CBlockIndex *tipindex;
    if ( height > 10 && (tipindex= chunguscoin_chainactive(height - 1)) != 0 ) // disable offchain diffchange
    {
        ...
        if ( diff >= 13 * ASSETCHAINS_BLOCKTIME && (height < 30 || tipdiff > 2*ASSETCHAINS_BLOCKTIME) )
        {
            ...
            easy.SetCompact(CHUNGUSCOIN_MINDIFF_NBITS,&fNegative,&fOverflow);
...
arith_uint256 chunguscoin_PoWtarget(int32_t *percPoSp,arith_uint256 target,int32_t height,int32_t goalperc,int32_t newStakerActive)
{
    ...
    *percPoSp = percPoS = 0;
    ...
    for (i=n=m=0; i<100; i++)
    {
        ...
        if ( ht <= 1 )
            continue;
        if ( (pindex= chunguscoin_chainactive(ht)) != 0 )
        {
            if ( chunguscoin_segid(0,ht) >= 0 )
            {
                n++;
                percPoS++;
...
uint32_t chunguscoin_stake(int32_t validateflag,arith_uint256 bnTarget,int32_t nHeight,uint256 txid,int32_t vout,uint32_t blocktime,uint32_t prevtime,char *destaddr,int32_t PoSperc)
{
    ...
    txtime = chunguscoin_txtime2(&value,txid,vout,address);
    if ( validateflag == 0 )
    {
    ...
    {
        //LogPrintf("chunguscoin_stake null %.8f %u %u %u\n",dstr(value),txtime,blocktime,prevtime);
        return(0);
    }
    if ( value < SATOSHIDEN )
        return(0);
    ...
    if ( (minage= nHeight*3) > 6000 ) // about 100 blocks
        minage = 6000;
    chunguscoin_segids(hashbuf,nHeight-101,100);
    segid32 = chunguscoin_stakehash(&hash,address,hashbuf,txid,vout);
...
int32_t chunguscoin_is_PoSblock(int32_t slowflag,int32_t height,CBlock *pblock,arith_uint256 bnTarget,arith_uint256 bhash)
{
    ...
    int32_t newStakerActive = chunguscoin_newStakerActive(height, pblock->nTime);
    // Get PoSperc and POW Target. slowflag only here, calling it when blocks out of order causes problems.
    if ( slowflag != 0 )
    {
        POWTarget = chunguscoin_PoWtarget(&PoSperc,bnTarget,height,ASSETCHAINS_STAKED,newStakerActive);
    }
    else 
    {
        // checks opret merkle root and existence of staking tx.
        return(chunguscoin_isPoS(pblock,height,0));
    }
    txn_count = pblock->vtx.size();
    ...
    if ( txn_count > 1 && pblock->vtx[txn_count-1].vin.size() == 1 && pblock->vtx[txn_count-1].vout.size() == 1+chunguscoin_hasOpRet(height,pblock->nTime) )
    {
        ...
        {
            if ( chunguscoin_isPoS(pblock,height,0) != 0 ) 
            {
                // checks utxo is eligible to stake this block
                eligible = chunguscoin_stake(1,bnTarget,height,txid,vout,pblock->nTime,prevtime+ASSETCHAINS_STAKED_BLOCK_FUTURE_HALF,(char *)"",PoSperc); 
            }
            if ( eligible == 0 || eligible > pblock->nTime )
            {
                if ( 0 && ASSETCHAINS_STAKED < 100 ) 
                    LogPrintf("chunguscoin_is_PoSblock PoS failure ht.%d eligible.%u vs blocktime.%u, lag.%d -> check to see if it is PoW block\n",height,eligible,(uint32_t)pblock->nTime,(int32_t)(eligible - pblock->nTime));
            }
            else 
            {
                isPoS = 1;
    ...
    if ( txn_count > 1 )
    {
...

        {
            ...
            if (validHash)
            {
                ...
                else if (!(pastBlockIndex = chunguscoin_chainactive(height - 100)))
                {
...
uint64_t chunguscoin_notarypayamount(int32_t nHeight, int64_t notarycount)
{
    ...
    if ( notarycount == 0 )
    {
        LogPrintf( "chunguscoin_notarypayamount failed num notaries is 0!\n");
        return(0);
    }
...
int32_t chunguscoin_getnotarizedheight(uint32_t timestamp,int32_t height, uint8_t *script, int32_t len)
{
    ...
    if ( len >= sizeof(uint32_t) && len <= sizeof(scriptbuf) )
    {
        ...
        if ( chunguscoin_voutupdate(true,&isratification,0,scriptbuf,len,height,uint256(),1,1,&voutmask,&specialtx,&notarizedheight,0,1,0,timestamp) != -2 )
        {
...
uint64_t chunguscoin_notarypay(CMutableTransaction &txNew, std::vector<int8_t> &NotarisationNotaries, uint32_t timestamp, int32_t height, uint8_t *script, int32_t len)
{
    // fetch notary pubkey array.
    uint64_t total = 0, AmountToPay = 0;
    int8_t numSN = 0; uint8_t notarypubkeys[64][33] = {0};
    numSN = chunguscoin_notaries(notarypubkeys, height, timestamp);
    ...
    int32_t notarizedheight = chunguscoin_getnotarizedheight(timestamp, height, script, len);
    ...
    AmountToPay = chunguscoin_notarypayamount(height,NotarisationNotaries.size());
    if ( AmountToPay == 0 )
        return(0);
...
uint64_t chunguscoin_checknotarypay(CBlock *pblock,int32_t height)
{
    ...
    if ( pblock->vtx[1].vout.size() == 2 && pblock->vtx[1].vout[1].nValue == 0 )
    {
        ...
        if ( script[0] == OP_RETURN )
        {
            ...
            totalsats = chunguscoin_notarypay(txNew, NotarisationNotaries, pblock->nTime, height, script, scriptlen);
        } 
        else 
        {
...
bool chunguscoin_appendACscriptpub()
{
    ...
    if ( ASSETCHAINS_SCRIPTPUB[ASSETCHAINS_SCRIPTPUB.back()] == 49 && ASSETCHAINS_SCRIPTPUB[ASSETCHAINS_SCRIPTPUB.back()-1] == 51 )
    {
        ...
        if ( myGetTransaction(CHUNGUSCOIN_EARLYTXID,tx,blockhash) && mapBlockIndex[blockhash]->GetHeight() < CHUNGUSCOIN_EARLYTXID_HEIGHT )
        {
        ...
        LogPrintf( "could not get CHUNGUSCOIN_EARLYTXID.%s OP_RETURN data. Restart with correct txid!\n", CHUNGUSCOIN_EARLYTXID.GetHex().c_str());
        StartShutdown();
    }
    return false;
}

void GetChunguscoinEarlytxidScriptPub()
{
    if ( CHUNGUSCOIN_EARLYTXID == zeroid )
    {
        LogPrintf( "Restart deamon with -earlytxid.\n");
        StartShutdown();
        return;
    }
    if ( ASSETCHAINS_EARLYTXIDCONTRACT == EVAL_PRICES && CHUNGUSCOIN_SNAPSHOT_INTERVAL == 0 )
    {
        LogPrintf( "Prices->paymentsCC contract must have -ac_snapshot enabled to pay out.\n");
        StartShutdown();
        return;
    }
    if ( chainActive.Height() < CHUNGUSCOIN_EARLYTXID_HEIGHT )
    {
        LogPrintf("Cannot fetch -earlytxid before block %d.\n",CHUNGUSCOIN_EARLYTXID_HEIGHT);
        StartShutdown();
        return;
    }
    ...
    if ( myGetTransaction(CHUNGUSCOIN_EARLYTXID,tx,blockhash) && mapBlockIndex[blockhash]->GetHeight() < CHUNGUSCOIN_EARLYTXID_HEIGHT )
    {
        ...
        if ( i < tx.vout.size() )
        {
            CHUNGUSCOIN_EARLYTXID_SCRIPTPUB = CScript(tx.vout[i].scriptPubKey.begin()+3, tx.vout[i].scriptPubKey.end());
            LogPrintf( "CHUNGUSCOIN_EARLYTXID_SCRIPTPUB.%s\n", HexStr(CHUNGUSCOIN_EARLYTXID_SCRIPTPUB.begin(),CHUNGUSCOIN_EARLYTXID_SCRIPTPUB.end()).c_str());
            return;
        }
    }
...
int64_t chunguscoin_checkcommission(CBlock *pblock,int32_t height)
{
    ...
    if ( ASSETCHAINS_COMMISSION != 0 || ASSETCHAINS_FOUNDERS_REWARD != 0 )
    {
        ...
        else if ( checktoshis != 0 )
        {
            ...
            {
                static bool didinit = false;
                if ( !didinit && height > CHUNGUSCOIN_EARLYTXID_HEIGHT && CHUNGUSCOIN_EARLYTXID != zeroid && chunguscoin_appendACscriptpub() )
                {
...
bool CHUNGUSCOIN_TEST_ASSETCHAIN_SKIP_POW = 0;

int32_t chunguscoin_checkPOW(int64_t stakeTxValue, int32_t slowflag,CBlock *pblock,int32_t height)
{
    ...
    if ( CHUNGUSCOIN_TEST_ASSETCHAIN_SKIP_POW == 0 && Params().NetworkIDString() == "regtest" )
        CHUNGUSCOIN_TEST_ASSETCHAIN_SKIP_POW = 1;
    if ( !CheckEquihashSolution(pblock, Params()) )
    {
        LogPrintf("chunguscoin_checkPOW slowflag.%d ht.%d CheckEquihashSolution failed\n",slowflag,height);
        return(-1);
    }
    hash = pblock->GetHash();
    bnTarget.SetCompact(pblock->nBits,&fNegative,&fOverflow);
    bhash = UintToArith256(hash);
    possible = chunguscoin_block2pubkey33(pubkey33,pblock);
    if ( height == 0 )
    {
        if ( slowflag != 0 )
        {
    ...
    //    bnTarget = chunguscoin_adaptivepow_target(height,bnTarget,pblock->nTime);
    ...
    if ( (ASSETCHAINS_SYMBOL[0] != 0 || height > 792000) && bhash > bnTarget )
    {
        ...
        {
            if ( (n= chunguscoin_notaries(pubkeys,height,pblock->nTime)) > 0 )
            {
                for (i=0; i<n; i++)
        ...
        {
            if ( CHUNGUSCOIN_TEST_ASSETCHAIN_SKIP_POW )
                return(0);
            if ( ASSETCHAINS_STAKED == 0 ) // chunguscoin_is_PoSblock will check bnTarget for staked chains
                return(-1);
        }
    }
    ...
    if ( ASSETCHAINS_STAKED != 0 && height >= 2 ) // must PoS or have at least 16x better PoW
    {
        ...
        pindex = it != mapBlockIndex.end() ? it->second : NULL;
        int32_t newStakerActive = chunguscoin_newStakerActive(height, pblock->nTime);
        if ( (is_PoSblock= chunguscoin_is_PoSblock(slowflag,height,pblock,bnTarget,bhash)) == 0 )
        {
            ...
            {
                bnTarget = chunguscoin_PoWtarget(&PoSperc,bnTarget,height,ASSETCHAINS_STAKED,newStakerActive);
                if ( bhash > bnTarget ) 
                {
                    for (i=31; i>=16; i--)
    ...
    if ( failed == 0 && ASSETCHAINS_COMMISSION != 0 ) 
    {
        ...
        {
            if ( chunguscoin_checkcommission(pblock,height) < 0 )
                return(-1);
        }
    }
    ...
    if ( failed == 0 && ASSETCHAINS_NOTARY_PAY[0] != 0 && pblock->vtx.size() > 1 )
    {
        ...
        if ( slowflag == 0 && pblock->vtx[0].vout.size() > 1 )
        {
            ...
            if ( !chunguscoin_is_notarytx(pblock->vtx[1]) == 1 )
            {
            ...
            // Check min sigs.
            int8_t numSN = 0; uint8_t notarypubkeys[64][33] = {0};
            numSN = chunguscoin_notaries(notarypubkeys, height, pblock->nTime);
...
//LogPrintf("chunguscoin_checkPOW possible.%d slowflag.%d ht.%d notaryid.%d failed.%d\n",possible,slowflag,height,notaryid,failed);
    if ( failed != 0 && possible == 0 && notaryid < 0 )
        return(-1);
    else return(0);
}

int32_t chunguscoin_acpublic(uint32_t tiptime)
{
    ...
    if ( acpublic == 0 )
    {
        ...
        if ( (ASSETCHAINS_SYMBOL[0] == 0 || strcmp(ASSETCHAINS_SYMBOL,"ZEX") == 0) && tiptime >= CHUNGUSCOIN_SAPLING_DEADLINE )
            acpublic = 1;
    }
    return(acpublic);
}

int64_t chunguscoin_newcoins(int64_t *zfundsp,int64_t *sproutfundsp,int32_t nHeight,CBlock *pblock)
{
...
int64_t chunguscoin_coinsupply(int64_t *zfundsp,int64_t *sproutfundsp,int32_t height)
{
    CBlockIndex *pindex; CBlock block; int64_t zfunds=0,sproutfunds=0,supply = 0;
    //LogPrintf("coinsupply %d\n",height);
    *zfundsp = *sproutfundsp = 0;
    if ( (pindex= chunguscoin_chainactive(height)) != 0 )
    {
        while ( pindex != 0 && pindex->GetHeight() > 0 )
        {
            ...
            {
                if ( chunguscoin_blockload(block,pindex) == 0 )
                    pindex->newcoins = chunguscoin_newcoins(&pindex->zfunds,&pindex->sproutfunds,pindex->GetHeight(),&block);
                else
                {
...
struct chunguscoin_staking *chunguscoin_addutxo(struct chunguscoin_staking *array,int32_t *numkp,int32_t *maxkp,uint32_t txtime,uint64_t nValue,uint256 txid,int32_t vout,char *address,uint8_t *hashbuf,CScript pk)
{
    uint256 hash; uint32_t segid32; struct chunguscoin_staking *kp;
    segid32 = chunguscoin_stakehash(&hash,address,hashbuf,txid,vout);
    if ( *numkp >= *maxkp )
    {
        *maxkp += 1000;
        array = (struct chunguscoin_staking *)realloc(array,sizeof(*array) * (*maxkp));
...
int32_t chunguscoin_staked(CMutableTransaction &txNew,uint32_t nBits,uint32_t *blocktimep,uint32_t *txtimep,uint256 *utxotxidp,int32_t *utxovoutp,uint64_t *utxovaluep,uint8_t *utxosig, uint256 merkleroot)
{
    static struct chunguscoin_staking *array; static int32_t numkp,maxkp; static uint32_t lasttime;
    int32_t PoSperc = 0, newStakerActive; 
    set<CBitcoinAddress> setAddress; struct chunguscoin_staking *kp; int32_t winners,segid,minage,nHeight,counter=0,i,m,siglen=0,nMinDepth = 1,nMaxDepth = 99999999; vector<COutput> vecOutputs; uint32_t block_from_future_rejecttime,besttime,eligible,earliest = 0; CScript best_scriptPubKey; arith_uint256 mindiff,ratio,bnTarget,tmpTarget; CBlockIndex *tipindex,*pindex; CTxDestination address; bool fNegative,fOverflow; uint8_t hashbuf[256]; CTransaction tx; uint256 hashBlock;
    ...
    *utxovaluep = 0;
    ...
    //tmpTarget = chunguscoin_PoWtarget(&PoSperc,bnTarget,nHeight,ASSETCHAINS_STAKED);
    ...
    if ( array != 0 )
    {
        ...
        if ( ASSETCHAINS_MARMARA != 0 )
            resetstaker = true;
         else if ( ReadBlockFromDisk(block, pblockindex, 1) && chunguscoin_isPoS(&block, nHeight, &addressout) != 0 && IsMine(*pwalletMain,addressout) != 0 )
        {
              resetstaker = true;
    ...
    if ( resetstaker || array == 0 || time(NULL) > lasttime+600 )
    {
        ...
        if ( ASSETCHAINS_MARMARA == 0 )
        {
            BOOST_FOREACH(const COutput& out, vecOutputs)
            {
                ...
                {
                    //LogPrintf("chunguscoin_staked invalid depth %d\n",(int32_t)out.nDepth);
                    continue;
                }
                ...
                if ( ExtractDestination(pk,address) != 0 )
                {
                    if ( IsMine(*pwalletMain,address) == 0 )
                        continue;
                    if ( myGetTransaction(out.tx->GetHash(),tx,hashBlock) != 0 && (pindex= chunguscoin_getblockindex(hashBlock)) != 0 )
                    {
                        array = chunguscoin_addutxo(array,&numkp,&maxkp,(uint32_t)pindex->nTime,(uint64_t)nValue,out.tx->GetHash(),out.i,(char *)CBitcoinAddress(address).ToString().c_str(),hashbuf,(CScript)pk);
        ...
        else
        {
            ...
            for (std::vector<std::pair<CAddressUnspentKey, CAddressUnspentValue> >::const_iterator it=unspentOutputs.begin(); it!=unspentOutputs.end(); it++)
            {
                ...
                if ( (nValue= it->second.satoshis) < COIN )
                    continue;
                if ( myGetTransaction(txid,tx,hashBlock) != 0 && (pindex= chunguscoin_getblockindex(hashBlock)) != 0 && myIsutxo_spentinmempool(ignoretxid,ignorevin,txid,vout) == 0 )
                {
                    ...
                    {
                        array = chunguscoin_addutxo(array,&numkp,&maxkp,(uint32_t)pindex->nTime,(uint64_t)nValue,txid,vout,coinaddr,hashbuf,(CScript)scriptPubKey);
                    }
    ...
    for (i=winners=0; i<numkp; i++)
    {
        ...
        kp = &array[i];
        eligible = chunguscoin_stake(0,bnTarget,nHeight,kp->txid,kp->vout,0,(uint32_t)tipindex->nTime+ASSETCHAINS_STAKED_BLOCK_FUTURE_HALF,kp->address,PoSperc);
        if ( eligible > 0 )
        {
            besttime = 0;
            if ( eligible == chunguscoin_stake(1,bnTarget,nHeight,kp->txid,kp->vout,eligible,(uint32_t)tipindex->nTime+ASSETCHAINS_STAKED_BLOCK_FUTURE_HALF,kp->address,PoSperc) )
            {
    ...
    if ( earliest != 0 )
    {
        ...
        if ( (newStakerActive= chunguscoin_newStakerActive(nHeight,earliest)) != 0 )
        {
