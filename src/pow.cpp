...
unsigned int GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock, const Consensus::Params& params)
{
    ...
    if ( ASSETCHAINS_ADAPTIVEPOW > 0 )
    {
        ...
        {
            easy.SetCompact(CHUNGUSCOIN_MINDIFF_NBITS & (~3),&fNegative,&fOverflow);
            if ( pblock != 0 )
            {
                origtarget = bnTarget;
            ...
            if ( mult > 1 ) // e^mult case, jl777:  test of mult > 1 failed when it was int64_t???
            {
                ...
                if ( bnTarget < origtarget || bnTarget > easy )
                {
                    ...
                    return(CHUNGUSCOIN_MINDIFF_NBITS & (~3));
                }
                {
                    int32_t z;
                    for (z=31; z>=0; z--)
...
extern int32_t CHUNGUSCOIN_CHOSEN_ONE;
...
bool CheckProofOfWork(const CBlockHeader &blkHeader, uint8_t *pubkey33, int32_t height, const Consensus::Params& params)
{
    ...
    tiptime = chunguscoin_chainactive_timestamp();
    bnTarget.SetCompact(blkHeader.nBits, &fNegative, &fOverflow);
    if ( height == 0 )
    {
        height = chunguscoin_currentheight() + 1;
        //LogPrintf("set height to %d\n",height);
    }
    if ( height > 34000 && ASSETCHAINS_SYMBOL[0] == 0 ) // 0 -> non-special notary
    {
        special = chunguscoin_chosennotary(&notaryid,height,pubkey33,tiptime);
        for (i=0; i<33; i++)
        {
            if ( pubkey33[i] != 0 )
                nonz++;
        ...
        flag = chunguscoin_eligiblenotary(pubkeys,mids,blocktimes,&nonzpkeys,height);
        special2 = chunguscoin_is_special(pubkeys,mids,blocktimes,height,pubkey33,blkHeader.nTime);
        if ( notaryid >= 0 )
        {
            if ( height > 10000 && height < 80000 && (special != 0 || special2 > 0) )
                ...
                flag = (height > 1000000 || (height % CHUNGUSCOIN_ELECTION_GAP) > 64 || (height % CHUNGUSCOIN_ELECTION_GAP) == 0);
            ...
            if ( (flag != 0 || special2 > 0) && special2 != -2 )
            {
                ...
                bnTarget.SetCompact(CHUNGUSCOIN_MINDIFF_NBITS,&fNegative,&fOverflow);
                /*
                const void* pblock = &blkHeader;
                CScript merkleroot = CScript();
                if ( height > nDecemberHardforkHeight && !chunguscoin_checkopret((CBlock*)pblock, merkleroot) ) // December 2019 hardfork
                {
                    LogPrintf("failed or missing merkleroot expected.%s != merkleroot.%s\n", chunguscoin_makeopret((CBlock*)pblock, false).ToString().c_str(), merkleroot.ToString().c_str());
                    return false;
                }
                */
            }
    ...
    //    bnTarget = chunguscoin_adaptivepow_target(height,bnTarget,blkHeader.nTime);
