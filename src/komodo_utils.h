...
int32_t chunguscoin_scriptitemlen(int32_t *opretlenp,uint8_t *script)
{
...
uint64_t chunguscoin_block_prg(uint32_t nHeight)
{
...
int64_t chunguscoin_block_unlocktime(uint32_t nHeight)
{
    ...
    else
    {
        if (strcmp(ASSETCHAINS_SYMBOL, "VRSC") != 0 || nHeight >= 12800)
        {
            unlocktime = chunguscoin_block_prg(nHeight) % (ASSETCHAINS_TIMEUNLOCKTO - ASSETCHAINS_TIMEUNLOCKFROM);
            unlocktime += ASSETCHAINS_TIMEUNLOCKFROM;
        }
        else
        {
            unlocktime = chunguscoin_block_prg(nHeight) / (0xffffffffffffffff / ((ASSETCHAINS_TIMEUNLOCKTO - ASSETCHAINS_TIMEUNLOCKFROM) + 1));
...
uint16_t _chunguscoin_userpass(char *username,char *password,FILE *fp)
{
...
void chunguscoin_statefname(char *fname,char *symbol,char *str)
{
    int32_t n,len;
    ...
    if ( (n= (int32_t)strlen(ASSETCHAINS_SYMBOL)) != 0 )
    {
        ...
        if ( !mapArgs.count("-datadir") && strcmp(ASSETCHAINS_SYMBOL,&fname[len - n]) == 0 )
            fname[len - n] = 0;
        else if(mapArgs.count("-datadir")) printf("DEBUG - chunguscoin_utils:1363: custom datadir\n");
        else
        {
...
void chunguscoin_configfile(char *symbol,uint16_t rpcport)
{
    ...
    if ( symbol != 0 && rpcport != 0 )
    {
        ...
        {
            _chunguscoin_userpass(myusername,mypassword,fp);
    ...
    if ( (fp= fopen(fname,"rb")) != 0 )
    {
        if ( (kmdport= _chunguscoin_userpass(username,password,fp)) != 0 )
            KMD_PORT = kmdport;
        sprintf(KMDUSERPASS,"%s:%s",username,password);
        fclose(fp);
//LogPrintf("CHUNGUSCOIN.(%s) -> userpass.(%s)\n",fname,KMDUSERPASS);
    } //else LogPrintf("couldnt open.(%s)\n",fname);
}

uint16_t chunguscoin_userpass(char *userpass,char *symbol)
{
    FILE *fp; uint16_t port = 0; char fname[512],username[512],password[512],confname[CHUNGUSCOIN_ASSETCHAIN_MAXLEN];
    userpass[0] = 0;
    if ( strcmp("KMD",symbol) == 0 )
    {
#ifdef __APPLE__
        sprintf(confname,"Chunguscoin.conf");
#else
        sprintf(confname,"chunguscoin.conf");
#endif
    }
        ...
        chunguscoin_statefname(fname,symbol,confname);
    } else sprintf(fname,"%s",GetDataDir().string().c_str());
    
    if ( (fp= fopen(fname,"rb")) != 0 )
    {
        port = _chunguscoin_userpass(username,password,fp);
...
uint32_t chunguscoin_assetmagic(char *symbol,uint64_t supply,uint8_t *extraptr,int32_t extralen)
{
...
uint16_t chunguscoin_assetport(uint32_t magic,int32_t extralen)
{
    if ( magic == 0x8de4eef9 )
        return(7770);
    else if ( extralen == 0 )
...
uint16_t chunguscoin_port(char *symbol,uint64_t supply,uint32_t *magicp,uint8_t *extraptr,int32_t extralen)
{
    if ( symbol == 0 || symbol[0] == 0 || strcmp("KMD",symbol) == 0 )
    {
        *magicp = 0x8de4eef9;
...
uint64_t chunguscoin_max_money()
{
    return chunguscoin_current_supply(10000000);
}

uint64_t chunguscoin_ac_block_subsidy(int nHeight)
{
...
void chunguscoin_args(char *argv0)
{
    ...
    IS_CHUNGUSCOIN_NOTARY = ntz_dest_path == "" ? 0 : 1;


    IS_STAKED_NOTARY = GetArg("-stakednotary", -1);
    CHUNGUSCOIN_NSPV = GetArg("-nSPV",0);
    memset(ccenables,0,sizeof(ccenables));
    memset(disablebits,0,sizeof(disablebits));
    memset(ccEnablesHeight,0,sizeof(ccEnablesHeight));
    if ( GetBoolArg("-gen", false) != 0 )
    {
        CHUNGUSCOIN_MININGTHREADS = GetArg("-genproclimit",-1);
    }
    if ( (CHUNGUSCOIN_EXCHANGEWALLET= GetBoolArg("-exchange", false)) != 0 )
        LogPrintf("CHUNGUSCOIN_EXCHANGEWALLET mode active\n");
    DONATION_PUBKEY = GetArg("-donation", "");
    NOTARY_PUBKEY = GetArg("-pubkey", "");
    CHUNGUSCOIN_DEALERNODE = GetArg("-dealer",0);
    CHUNGUSCOIN_TESTNODE = GetArg("-testnode",0);
    ...
    if ( strlen(NOTARY_PUBKEY.c_str()) == 66 )
    {
        ...
        if ( IS_CHUNGUSCOIN_NOTARY == 0 )
        {
            ...
            for (i=0; i<64; i++)
            {
                ...
                {
                    IS_CHUNGUSCOIN_NOTARY = 1;
                    CHUNGUSCOIN_MININGTHREADS = 1;
                    mapArgs ["-genproclimit"] = itostr(CHUNGUSCOIN_MININGTHREADS);
                    IS_STAKED_NOTARY = -1;
    ...
    if ( IS_STAKED_NOTARY != -1 && IS_CHUNGUSCOIN_NOTARY == true ) {
    ...
    if ( (CHUNGUSCOIN_REWIND= GetArg("-rewind",0)) != 0 )
    {
        LogPrintf("CHUNGUSCOIN_REWIND %d\n",CHUNGUSCOIN_REWIND);
    }
    CHUNGUSCOIN_EARLYTXID = Parseuint256(GetArg("-earlytxid","0").c_str());    
...
LogPrintf("extralen.%d before disable bits\n",extralen);
            ...
            if ( ASSETCHAINS_CBOPRET != 0 )
            {
                ...
                //chunguscoin_pricesinit();
                chunguscoin_cbopretupdate(1); // will set Mineropret
            ...
            if ( CHUNGUSCOIN_SNAPSHOT_INTERVAL != 0 )
            {
                extralen += iguana_rwnum(1,&extraptr[extralen],sizeof(CHUNGUSCOIN_SNAPSHOT_INTERVAL),(void *)&CHUNGUSCOIN_SNAPSHOT_INTERVAL);
            }
        ...
        MAX_MONEY = chunguscoin_max_money();

        if ( (baseid = chunguscoin_baseid(ASSETCHAINS_SYMBOL)) >= 0 && baseid < 32 )
        {
            //chunguscoin_maxallowed(baseid);
            LogPrintf("baseid.%d MAX_MONEY.%s %.8f\n",baseid,ASSETCHAINS_SYMBOL,(double)MAX_MONEY/SATOSHIDEN);
        }

        if ( ASSETCHAINS_CC >= CHUNGUSCOIN_FIRSTFUNGIBLEID && MAX_MONEY < 1000000LL*SATOSHIDEN )
            MAX_MONEY = 1000000LL*SATOSHIDEN;
        if ( CHUNGUSCOIN_BIT63SET(MAX_MONEY) != 0 )
            MAX_MONEY = CHUNGUSCOIN_MAXNVALUE;
        LogPrintf("MAX_MONEY %llu %.8f\n",(long long)MAX_MONEY,(double)MAX_MONEY/SATOSHIDEN);
        //LogPrintf("baseid.%d MAX_MONEY.%s %.8f\n",baseid,ASSETCHAINS_SYMBOL,(double)MAX_MONEY/SATOSHIDEN);
        uint16_t tmpport = chunguscoin_port(ASSETCHAINS_SYMBOL,ASSETCHAINS_SUPPLY,&ASSETCHAINS_MAGIC,extraptr,extralen);
        if ( GetArg("-port",0) != 0 )
        {
        ...
        if ( ASSETCHAINS_SYMBOL[0] != 0 )
        {
            ...
            if ( (port= chunguscoin_userpass(ASSETCHAINS_USERPASS,ASSETCHAINS_SYMBOL)) != 0 )
                ASSETCHAINS_RPCPORT = port;
            else chunguscoin_configfile(ASSETCHAINS_SYMBOL,ASSETCHAINS_P2PPORT + 1);
        ...
        //chunguscoin_assetchain_pubkeys((char *)ASSETCHAINS_NOTARIES.c_str());
    ...
    else
    {
        ...
        for (iter=0; iter<2; iter++)
        {
            ...
            while ( fname[strlen(fname)-1] != '\\' )
                fname[strlen(fname)-1] = 0;
            if ( iter == 0 )
                strcat(fname,"Chunguscoin\\chunguscoin.conf");
            ...
            if ( (fp= fopen(fname,"rb")) != 0 )
            {
                dest_rpc_port = _chunguscoin_userpass(username,password,fp);
    ...
    int32_t dpowconfs = CHUNGUSCOIN_DPOWCONFS;
    if ( ASSETCHAINS_SYMBOL[0] != 0 )
    {
    ...
    CHUNGUSCOIN_DPOWCONFS = GetArg("-dpowconfs",dpowconfs);
    if ( ASSETCHAINS_SYMBOL[0] == 0 || strcmp(ASSETCHAINS_SYMBOL,"SUPERNET") == 0 || strcmp(ASSETCHAINS_SYMBOL,"DEX") == 0 || strcmp(ASSETCHAINS_SYMBOL,"COQUI") == 0 || strcmp(ASSETCHAINS_SYMBOL,"PIRATE") == 0 || strcmp(ASSETCHAINS_SYMBOL,"KMDICE") == 0 )
        CHUNGUSCOIN_EXTRASATOSHI = 1;
}

void chunguscoin_nameset(char *symbol,char *dest,char *source)
{
    if ( source[0] == 0 )
    {
...
struct chunguscoin_state *chunguscoin_stateptrget(char *base)
{
    int32_t baseid;
    if ( base == 0 || base[0] == 0 || strcmp(base,(char *)"KMD") == 0 )
        return(&CHUNGUSCOIN_STATES[33]);
    else if ( (baseid= chunguscoin_baseid(base)) >= 0 )
        return(&CHUNGUSCOIN_STATES[baseid+1]);
    else return(&CHUNGUSCOIN_STATES[0]);
}

struct chunguscoin_state *chunguscoin_stateptr(char *symbol,char *dest)
{
    int32_t baseid;
    chunguscoin_nameset(symbol,dest,ASSETCHAINS_SYMBOL);
    return(chunguscoin_stateptrget(symbol));
}

/*
void chunguscoin_prefetch(FILE *fp)
{
...
bool chunguscoin_is_vSolutionsFixActive()
{
    return GetLatestTimestamp(chunguscoin_currentheight()) > nS5Timestamp;
}
