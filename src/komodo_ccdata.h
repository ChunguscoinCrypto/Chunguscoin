...
#ifndef H_CHUNGUSCOINCCDATA_H
#define H_CHUNGUSCOINCCDATA_H

struct chunguscoin_ccdata *CC_data;
...
uint256 chunguscoin_calcMoM(int32_t height,int32_t MoMdepth)
{
...
struct chunguscoin_ccdata_entry *chunguscoin_allMoMs(int32_t *nump,uint256 *MoMoMp,int32_t kmdstarti,int32_t kmdendi)
{
    struct chunguscoin_ccdata_entry *allMoMs=0; struct chunguscoin_ccdata *ccdata,*tmpptr; int32_t i,num,max;
    bool fMutated; std::vector<uint256> tree, leaves;
    num = max = 0;
    portable_mutex_lock(&CHUNGUSCOIN_CC_mutex);
    DL_FOREACH_SAFE(CC_data,ccdata,tmpptr)
    {
        if ( ccdata->MoMdata.height <= kmdendi && ccdata->MoMdata.height >= kmdstarti )
        {
            ...
            {
                max += 100;
                allMoMs = (struct chunguscoin_ccdata_entry *)realloc(allMoMs,max * sizeof(*allMoMs));
            }
    ...
    portable_mutex_unlock(&CHUNGUSCOIN_CC_mutex);
    if ( (*nump= num) > 0 )
    {
        for (i=0; i<num; i++)
...
int32_t chunguscoin_addpair(struct chunguscoin_ccdataMoMoM *mdata,int32_t notarized_height,int32_t offset,int32_t maxpairs)
{
    if ( maxpairs >= 0) {
        if ( mdata->numpairs >= maxpairs )
        {
            maxpairs += 100;
            mdata->pairs = (struct chunguscoin_ccdatapair *)realloc(mdata->pairs,sizeof(*mdata->pairs)*maxpairs);
            //LogPrintf("pairs reallocated to %p num.%d\n",mdata->pairs,mdata->numpairs);
        }
    } else {
        LogPrintf("chunguscoin_addpair.maxpairs %d must be >= 0\n",(int32_t)maxpairs);
        return(-1);
    }
...
int32_t chunguscoin_MoMoMdata(char *hexstr,int32_t hexsize,struct chunguscoin_ccdataMoMoM *mdata,char *symbol,int32_t kmdheight,int32_t notarized_height)
{
    uint8_t hexdata[8192]; struct chunguscoin_ccdata *ccdata,*tmpptr; int32_t len,maxpairs,i,retval=-1,depth,starti,endi,CCid=0; struct chunguscoin_ccdata_entry *allMoMs;
    ...
    {
        if ( (allMoMs= chunguscoin_allMoMs(&depth,&mdata->MoMoM,starti,endi)) != 0 )
        {
            mdata->MoMoMdepth = depth;
            for (i=0; i<depth; i++)
            {
                if ( strcmp(symbol,allMoMs[i].symbol) == 0 )
                    maxpairs = chunguscoin_addpair(mdata,allMoMs[i].notarized_height,i,maxpairs);
            }
            if ( mdata->numpairs > 0 )
            {
...
void chunguscoin_purge_ccdata(int32_t height)
{
    struct chunguscoin_ccdata *ccdata,*tmpptr;
...
int32_t chunguscoin_rwccdata(char *thischain,int32_t rwflag,struct chunguscoin_ccdata *ccdata,struct chunguscoin_ccdataMoMoM *MoMoMdata)
{
    uint256 hash,zero; bits256 tmp; int32_t i,nonz; struct chunguscoin_ccdata *ptr; struct notarized_checkpoint *np;
    ...
    if ( ASSETCHAINS_SYMBOL[0] == 0 )
    {
        ...
        {
            ptr = (struct chunguscoin_ccdata *)calloc(1,sizeof(*ptr));
            *ptr = *ccdata;
            portable_mutex_lock(&CHUNGUSCOIN_CC_mutex);
    ...
    else
    {
        if ( MoMoMdata != 0 && MoMoMdata->pairs != 0 )
        {
            ...
            {
                if ( (np= chunguscoin_npptr(MoMoMdata->pairs[i].notarized_height)) != 0 )
                {
