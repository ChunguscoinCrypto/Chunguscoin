...
// Copyright (c) 2015-2020 The Chunguscoin Platform developers
...
extern int32_t CHUNGUSCOIN_LOADINGBLOCKS,CHUNGUSCOIN_LONGESTCHAIN,CHUNGUSCOIN_INSYNC,CHUNGUSCOIN_CONNECTING,CHUNGUSCOIN_EXTRASATOSHI;
int32_t CHUNGUSCOIN_NEWBLOCKS;
int32_t chunguscoin_block2pubkey33(uint8_t *pubkey33,CBlock *block);
//void chunguscoin_broadcast(CBlock *pblock,int32_t limit);
bool Getscriptaddress(char *destaddr,const CScript &scriptPubKey);
void chunguscoin_setactivation(int32_t height);
...
const string strMessageMagic = "Chunguscoin Signed Message:\n";
...
#define CHUNGUSCOIN_ZCASH
#include "komodo.h"

UniValue chunguscoin_snapshot(int top)
{
    LOCK(cs_main);
    int64_t total = -1;
...
bool chunguscoin_snapshot2(std::map <std::string, CAmount> &addressAmounts)
{
...
bool chunguscoin_dailysnapshot(int32_t height)
{
    int reorglimit = 100; 
    ...
    // Under heavy reorg attack, its possible `chunguscoin_notarized_height` can return a height that can't be found on chain sync.
    ...
    if ( (extraoffset= height % CHUNGUSCOIN_SNAPSHOT_INTERVAL) != 0 )
    {
    ...
    else 
    {
        ...
        notarized_height = chunguscoin_notarized_height(&prevMoMheight,&notarized_hash,&notarized_desttxid);
    ...
    if ( !chunguscoin_snapshot2(addressAmounts) )
        return false;
    ...
    for (int32_t n = height; n > undo_height; n--) 
    {
        ...
        if ( (pindex= chunguscoin_chainactive(n)) == 0 || chunguscoin_blockload(block, pindex) != 0 ) 
            return false;
...
bool IsFinalTx(const CTransaction &tx, int nBlockHeight, int64_t nBlockTime)
{
    ...
    BOOST_FOREACH(const CTxIn& txin, tx.vin)
    {
        if ( !chunguscoin_hardfork_active(nBlockTime) && txin.nSequence == 0xfffffffe &&
        ...
        else if ( chunguscoin_hardfork_active(nBlockTime) && txin.nSequence == 0xfffffffe &&
            (
                ((int64_t)tx.nLockTime >= LOCKTIME_THRESHOLD && (int64_t)tx.nLockTime <= nBlockTime) ||
                ((int64_t)tx.nLockTime <  LOCKTIME_THRESHOLD && (int64_t)tx.nLockTime <= nBlockHeight))
            )
...
bool ContextualCheckCoinbaseTransaction(int32_t slowflag,const CBlock *block,CBlockIndex * const previndex,const CTransaction& tx, const int nHeight,int32_t validateprices)
{
    ...
    if (((uint64_t)(tx.GetValueOut()) >= ASSETCHAINS_TIMELOCKGTE) ||
        (((nHeight >= 31680) || strcmp(ASSETCHAINS_SYMBOL, "VRSC") != 0) && chunguscoin_ac_block_subsidy(nHeight) >= ASSETCHAINS_TIMELOCKGTE))
    {
            ...
            if (tx.vout[1].scriptPubKey.GetOp2(it, op, &opretData))
            {
                if (opretData.size() > 0 && opretData.data()[0] == OPRETTYPE_TIMELOCK)
                {
                    ...
                    if (CScriptID(opretScript) == scriptHash &&
                        ...
                        chunguscoin_block_unlocktime(nHeight) == unlocktime)
                    {
                        return(true);
                    }
                }
    ...
    {
        if ( chunguscoin_opretvalidate(block,previndex,nHeight,tx.vout[tx.vout.size()-1].scriptPubKey) < 0 )
            return(false);
    }
    return(true);
}
                      ...
                      libzcash::ProofVerifier& verifier,int32_t txIndex, int32_t numTxs)
{
    ...
    if ( ASSETCHAINS_SYMBOL[0] == 0 )
    {
        for (j=0; j<n; j++)
        {
            ...
            {
                if ( tx.vin[j].prevout.hash == array[k] && chunguscoin_checkvout(tx.vin[j].prevout.n,k,indallvouts) != 0 ) //(tx.vin[j].prevout.n == 1 || k >= indallvouts) )
                {
    ...
    if ( ASSETCHAINS_STAKED != 0 && chunguscoin_newStakerActive(0, tiptime) != 0 && tx.vout.size() == 2 && DecodeStakingOpRet(tx.vout[1].scriptPubKey, merkleroot) != 0 )
    {
...
int32_t chunguscoin_isnotaryvout(char *coinaddr,uint32_t tiptime) // from ac_private chains only
{
    ...
    if ( NOTARY_ADDRESSES[season-1][0][0] == 0 )
    {
        ...
        chunguscoin_notaries(pubkeys,0,tiptime);
    }
...
bool CheckTransactionWithoutProofVerification(uint32_t tiptime,const CTransaction& tx, CValidationState &state)
{
    ...
    int32_t invalid_private_taddr=0,z_z=0,z_t=0,t_z=0,acpublic = chunguscoin_acpublic(tiptime), current_season = getacseason(tiptime);
    /**
     * Previously:
    ...
    BOOST_FOREACH(const CTxOut& txout, tx.vout)
    {
        ...
        if ( ASSETCHAINS_PRIVATE != 0 )
        {
            ...
            if (iscoinbase == 0 && txout.nValue > 0)
            {
                ...
                if ( chunguscoin_isnotaryvout(destaddr,tiptime) == 0 )
                {
    ...
    BOOST_FOREACH(const JSDescription& joinsplit, tx.vjoinsplit)
    {
        ...
        if ( tiptime >= CHUNGUSCOIN_SAPLING_DEADLINE )
        {
...
//LogPrintf("addmempool 1\n");
    ...
    if ( ASSETCHAINS_SYMBOL[0] == 0 && chunguscoin_validate_interest(tx,chainActive.LastTip()->GetHeight()+1,chainActive.LastTip()->GetMedianTimePast() + 777,0) < 0 )
    {
        LogPrintf("AcceptToMemoryPool chunguscoin_validate_interest failure\n");
        return error("AcceptToMemoryPool: chunguscoin_validate_interest failed");
    }
...
//LogPrintf("addmempool 4\n");
    {
        ...
        if ( nValueOut > 777777*COIN && CHUNGUSCOIN_VALUETOOBIG(nValueOut - 777777*COIN) != 0 ) // some room for blockreward and txfees
...
//LogPrintf("addmempool 5\n");
        ...
        if ( CHUNGUSCOIN_CONNECTING <= 0 && chainActive.LastTip() != 0 )
        {
            flag = 1;
            CHUNGUSCOIN_CONNECTING = (1<<30) + (int32_t)chainActive.LastTip()->GetHeight() + 1;
        }
...
/*char *chunguscoin_getspendscript(uint256 hash,int32_t n)
 {
...
bool ReadBlockFromDisk(int32_t height,CBlock& block, const CDiskBlockPos& pos,bool checkPOW)
{
    ...
    if ( 0 && checkPOW != 0 )
    {
        chunguscoin_block2pubkey33(pubkey33,(CBlock *)&block);
...
//uint64_t chunguscoin_moneysupply(int32_t height);
...
CAmount GetBlockSubsidy(int nHeight, const Consensus::Params& consensusParams)
{
    ...
    if ( ASSETCHAINS_SYMBOL[0] == 0 )
    {
        ...
        else if ( nHeight < CHUNGUSCOIN_ENDOFERA )
            return(3 * COIN);
        else if ( nHeight < 2*CHUNGUSCOIN_ENDOFERA )
            return(2 * COIN);
        else return(COIN);
    }
    else
    {
        return(chunguscoin_ac_block_subsidy(nHeight));
    }
    /*
     // Mining slow start
    ...
    bool CheckTxInputs(const CTransaction& tx, CValidationState& state, const CCoinsViewCache& inputs, int nSpendHeight, const Consensus::Params& consensusParams)
    {
        ...
        for (unsigned int i = 0; i < tx.vin.size(); i++)
        {
            ...
            if (coins->IsCoinBase()) {
                ...
                {
                    uint64_t unlockTime = chunguscoin_block_unlocktime(coins->nHeight);
...
#ifdef CHUNGUSCOIN_ENABLE_INTEREST
            if ( ASSETCHAINS_SYMBOL[0] == 0 && nSpendHeight > 60000 )//chainActive.LastTip() != 0 && chainActive.LastTip()->GetHeight() >= 60000 )
            {
                if ( coins->vout[prevout.n].nValue >= 10*COIN )
                {
                    ...
                    if ( (interest= chunguscoin_accrued_interest(&txheight,&locktime,prevout.hash,prevout.n,0,coins->vout[prevout.n].nValue,(int32_t)nSpendHeight-1)) != 0 )
                    {
...
bool DisconnectBlock(CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool* pfClean)
{
    ...
    bool fClean = true;
    //chunguscoin_disconnect(pindex,block); does nothing?
...
bool ConnectBlock(const CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool fJustCheck,bool fCheckPOW)
{
    ...
    if ( CHUNGUSCOIN_NSPV_SUPERLITE )
        return(true);
    if ( CHUNGUSCOIN_STOPAT != 0 && pindex->GetHeight() > CHUNGUSCOIN_STOPAT )
        return(false);
    ...
    if ( ASSETCHAINS_NOTARY_PAY[0] != 0 && pindex->GetHeight() > 10 )
    {
        ...
        int32_t notarisationTx = chunguscoin_connectblock(true,pindex,*(CBlock *)&block);  
        ...
        if ( notarisationTx == 1 )
        {
            ...
            notarypaycheque = chunguscoin_checknotarypay((CBlock *)&block,(int32_t)pindex->GetHeight());
    ...
    //if ( CHUNGUSCOIN_TESTNET_EXPIRATION != 0 && pindex->GetHeight() > CHUNGUSCOIN_TESTNET_EXPIRATION ) // "testnet"
    //    return(false);
    ...
    for (unsigned int i = 0; i < block.vtx.size(); i++)
    {
        ...
        if ( CHUNGUSCOIN_VALUETOOBIG(valueout) != 0 )
        {
        ...
        //prevsum = voutsum;
        //voutsum += valueout;
        /*if ( CHUNGUSCOIN_VALUETOOBIG(voutsum) != 0 )
        {
        ...
        //if ( ASSETCHAINS_SYMBOL[0] == 0 )
        //    chunguscoin_earned_interest(pindex->GetHeight(),sum);
        CTxUndo undoDummy;
        if (i > 0) {
    ...
    if ( ASSETCHAINS_STAKED != 0 && fCheckPOW && chunguscoin_checkPOW(blockReward+stakeTxValue-notarypaycheque,1,(CBlock *)&block,pindex->GetHeight()) < 0 ) 
        return state.DoS(100, error("ConnectBlock: ac_staked chain failed slow chunguscoin_checkPOW"),REJECT_INVALID, "failed-slow_checkPOW");
    ...
    if ( ASSETCHAINS_COMMISSION != 0 || ASSETCHAINS_FOUNDERS_REWARD != 0 ) //ASSETCHAINS_OVERRIDE_PUBKEY33[0] != 0 &&
    {
        ...
        if ( (checktoshis= chunguscoin_commission((CBlock *)&block,(int32_t)pindex->GetHeight())) != 0 )
        {
    ...
    if ( block.vtx[0].GetValueOut() > blockReward+CHUNGUSCOIN_EXTRASATOSHI )
    {
        if ( ASSETCHAINS_SYMBOL[0] != 0 || pindex->GetHeight() >= CHUNGUSCOIN_NOTARIES_HEIGHT1 || block.vtx[0].vout[0].nValue > blockReward )
        {
            ...
            return state.DoS(100,
                             ...
                             REJECT_INVALID, "bad-cb-amount");
        } else if ( IS_CHUNGUSCOIN_NOTARY != 0 )
    ...
    //FlushStateToDisk();
    chunguscoin_connectblock(false,pindex,*(CBlock *)&block);  // dPoW state update.
...
bool static DisconnectTip(CValidationState &state, bool fBare = false) {
    ...
    //if ( ASSETCHAINS_SYMBOL[0] != 0 || pindexDelete->GetHeight() > 1400000 )
    {
        ...
        notarizedht = chunguscoin_notarized_height(&prevMoMheight,&notarizedhash,&txid);
    ...
    if (!fBare) {
        ...
        for (int i = 0; i < block.vtx.size(); i++)
        {
            ...
            if (tx.IsCoinBase() || (i == block.vtx.size()-1 && chunguscoin_newStakerActive(0, pindexDelete->nTime) == 0 && chunguscoin_isPoS((CBlock *)&block,pindexDelete->GetHeight(),0) != 0) || !AcceptToMemoryPool(mempool, stateDummy, tx, false, NULL))
            {
    ...
    {
        CTransaction &tx = block.vtx[i];
        //if ((i == (block.vtx.size() - 1)) && ((ASSETCHAINS_LWMAPOS && block.IsVerusPOSBlock()) || (ASSETCHAINS_STAKED != 0 && (chunguscoin_isPoS((CBlock *)&block) != 0))))
        if ( chunguscoin_newStakerActive(0, pindexDelete->nTime) == 0 && i == block.vtx.size()-1 && chunguscoin_isPoS((CBlock *)&block,pindexDelete->GetHeight(),0) != 0 )
        {
#ifdef ENABLE_WALLET
             // new staking tx cannot be accepted to mempool and expires in 1 block, so no need for this! :D
             if ( !GetBoolArg("-disablewallet", false) && CHUNGUSCOIN_NSPV_FULLNODE )
...
int32_t chunguscoin_activate_sapling(CBlockIndex *pindex)
{
    uint32_t blocktime,prevtime; CBlockIndex *prev; int32_t i,transition=0,height,prevht;
    int32_t activation = 0;
    if ( pindex == 0 )
    {
        LogPrintf("chunguscoin_activate_sapling null pindex\n");
        return(0);
    }
    height = pindex->GetHeight();
    blocktime = (uint32_t)pindex->nTime;
    //LogPrintf("chunguscoin_activate_sapling.%d starting blocktime %u cmp.%d\n",height,blocktime,blocktime > CHUNGUSCOIN_SAPLING_ACTIVATION);
    ...
    height = pindex->GetHeight();
    blocktime = (uint32_t)pindex->nTime;
    //LogPrintf("starting blocktime %u cmp.%d\n",blocktime,blocktime > CHUNGUSCOIN_SAPLING_ACTIVATION);
    if ( blocktime > CHUNGUSCOIN_SAPLING_ACTIVATION ) // find the earliest transition
    {
        while ( (prev= pindex->pprev) != 0 )
        {
            prevht = prev->GetHeight();
            prevtime = (uint32_t)prev->nTime;
            //LogPrintf("(%d, %u).%d -> (%d, %u).%d\n",prevht,prevtime,prevtime > CHUNGUSCOIN_SAPLING_ACTIVATION,height,blocktime,blocktime > CHUNGUSCOIN_SAPLING_ACTIVATION);
            if ( prevht+1 != height )
            {
                LogPrintf("chunguscoin_activate_sapling: unexpected non-contiguous ht %d vs %d\n",prevht,height);
                return(0);
            }
            if ( prevtime <= CHUNGUSCOIN_SAPLING_ACTIVATION && blocktime > CHUNGUSCOIN_SAPLING_ACTIVATION )
            {
                activation = height + 60;
                LogPrintf("%s transition at %d (%d, %u) -> (%d, %u)\n",ASSETCHAINS_SYMBOL,height,prevht,prevtime,height,blocktime);
            }
            if ( prevtime < CHUNGUSCOIN_SAPLING_ACTIVATION-3600*24 )
                break;
            pindex = prev;
            height = prevht;
            blocktime = prevtime;
    ...
    if ( activation != 0 )
    {
        chunguscoin_setactivation(activation);
...
bool static ConnectTip(CValidationState &state, CBlockIndex *pindexNew, CBlock *pblock) {
    ...
    CHUNGUSCOIN_CONNECTING = (int32_t)pindexNew->GetHeight();
    ...
    if ( CHUNGUSCOIN_LONGESTCHAIN != 0 && (pindexNew->GetHeight() == CHUNGUSCOIN_LONGESTCHAIN || pindexNew->GetHeight() == CHUNGUSCOIN_LONGESTCHAIN+1) )
        CHUNGUSCOIN_INSYNC = (int32_t)pindexNew->GetHeight();
    else CHUNGUSCOIN_INSYNC = 0;
    //LogPrintf("connect.%d insync.%d ASSETCHAINS_SAPLING.%d\n",(int32_t)pindexNew->GetHeight(),CHUNGUSCOIN_INSYNC,ASSETCHAINS_SAPLING);
    /*if ( CHUNGUSCOIN_INSYNC != 0 ) //ASSETCHAINS_SYMBOL[0] == 0 &&
        chunguscoin_broadcast(pblock,8);
    else if ( ASSETCHAINS_SYMBOL[0] != 0 )
        chunguscoin_broadcast(pblock,4);*/
    if ( CHUNGUSCOIN_NSPV_FULLNODE )
    {
        if ( ASSETCHAINS_CBOPRET != 0 )
            chunguscoin_pricesupdate(pindexNew->GetHeight(),pblock);
        if ( ASSETCHAINS_SAPLING <= 0 && pindexNew->nTime > CHUNGUSCOIN_SAPLING_ACTIVATION - 24*3600 )
            chunguscoin_activate_sapling(pindexNew);
        if ( ASSETCHAINS_CC != 0 && CHUNGUSCOIN_SNAPSHOT_INTERVAL != 0 && (pindexNew->GetHeight() % CHUNGUSCOIN_SNAPSHOT_INTERVAL) == 0 && pindexNew->GetHeight() >= CHUNGUSCOIN_SNAPSHOT_INTERVAL )
        {
            uint64_t start = time(NULL);
            if ( !chunguscoin_dailysnapshot(pindexNew->GetHeight()) )
            {
...
static bool ActivateBestChainStep(bool fSkipdpow, CValidationState &state, CBlockIndex *pindexMostWork, CBlock *pblock) {
    ...
    if ( CHUNGUSCOIN_REWIND != 0 )
    {
        CBlockIndex *tipindex;
        LogPrintf(">>>>>>>>>>> rewind start ht.%d -> CHUNGUSCOIN_REWIND.%d\n",chainActive.LastTip()->GetHeight(),CHUNGUSCOIN_REWIND);
        while ( CHUNGUSCOIN_REWIND > 0 && (tipindex= chainActive.LastTip()) != 0 && tipindex->GetHeight() > CHUNGUSCOIN_REWIND )
        {
            fBlocksDisconnected = true;
        ...
        LogPrintf("reached rewind.%d, best to do: ./chunguscoin-cli -ac_name=%s stop\n",CHUNGUSCOIN_REWIND,ASSETCHAINS_SYMBOL);
#ifdef WIN32
...
CBlockIndex* AddToBlockIndex(const CBlockHeader& block)
{
    ...
    */
    ...
    {
        if ( it->second != 0 ) // vNodes.size() >= CHUNGUSCOIN_LIMITED_NETWORKSIZE, change behavior to allow chunguscoin_ensure to work
        {
...
int32_t chunguscoin_check_deposit(int32_t height,const CBlock& block,uint32_t prevtime);
                ...
                bool fCheckPOW, bool fCheckMerkleRoot)
{
    ...
    if ( fCheckPOW )
    {
        ...
        if ( ASSETCHAINS_STAKED == 0 && chunguscoin_checkPOW(0,1,(CBlock *)&block,height) < 0 ) // checks Equihash
    ...
    {
        int32_t notaryid;
        int32_t special = chunguscoin_chosennotary(&notaryid,height,pubkey33,tiptime);
        if (notaryid > 0 || ( notaryid == 0 && height > nS5HardforkHeight ) ) {
            ...
            if (!chunguscoin_checkopret(pblockcopy, merkleroot)) {
                LogPrintf("failed or missing merkleroot expected.%s != merkleroot.%s\n", chunguscoin_makeopret(pblockcopy, false).ToString().c_str(), merkleroot.ToString().c_str());
    ...
    if ( ASSETCHAINS_CC != 0 ) // CC contracts might refer to transactions in the current block, from a CC spend within the same block and out of order
    {
        ...
        while ( 1 )
        {
            ...
            for (i=0; i<block.vtx.size(); i++)
            {
                ...
                if ( tx.IsCoinBase() || !tx.vjoinsplit.empty() || !tx.vShieldedSpend.empty() || (i == block.vtx.size()-1 && chunguscoin_isPoS((CBlock *)&block,height,0) != 0) )
                    continue;
                Tx = tx;
    ...
    {
        const CTransaction& tx = block.vtx[i];
        if ( chunguscoin_validate_interest(tx,height == 0 ? chunguscoin_block2height((CBlock *)&block) : height,block.nTime,0) < 0 )
        {
            LogPrintf("validate intrest failed for txnum.%i tx.%s\n", i, tx.ToString().c_str());
            return error("CheckBlock: chunguscoin_validate_interest failed");
        }
    ...
    if ( fCheckPOW && chunguscoin_check_deposit(height,block,(pindex==0||pindex->pprev==0)?0:pindex->pprev->nTime) < 0 )
    {
        //static uint32_t counter;
        //if ( counter++ < 100 && ASSETCHAINS_STAKED == 0 )
        //    LogPrintf("check deposit rejection\n");
        LogPrintf("CheckBlockHeader chunguscoin_check_deposit error");
        return(false);
    }
...
bool ContextualCheckBlockHeader(const CBlockHeader& block, CValidationState& state, CBlockIndex * const pindexPrev)
{
    ...
    if (fCheckpointsEnabled)
    {
        ...
        if ( nHeight != 0 )
        {
            ...
            if ( chunguscoin_checkpoint(&notarized_height,nHeight,hash) < 0 )
            {
                CBlockIndex *heightblock = chainActive[nHeight];
                if ( heightblock != 0 && heightblock->GetBlockHash() == hash )
                {
...
bool AcceptBlockHeader(int32_t *futureblockp,const CBlockHeader& block, CValidationState& state, CBlockIndex** ppindex)
{
    ...
    if (miSelf != mapBlockIndex.end())
    {
        ...
        {
            if ( ASSETCHAINS_CC == 0 )//&& (ASSETCHAINS_PRIVATE == 0 || CHUNGUSCOIN_INSYNC >= Params().GetConsensus().vUpgrades[Consensus::UPGRADE_SAPLING].nActivationHeight) )
        ...
        /*if ( pindex != 0 && hash == chunguscoin_requestedhash )
        {
            LogPrintf("AddToBlockIndex A chunguscoin_requestedhash %s\n",chunguscoin_requestedhash.ToString().c_str());
            memset(&chunguscoin_requestedhash,0,sizeof(chunguscoin_requestedhash));
            chunguscoin_requestedcount = 0;
        }*/
    ...
    /*if ( pindex != 0 && hash == chunguscoin_requestedhash )
    {
        LogPrintf("AddToBlockIndex chunguscoin_requestedhash %s\n",chunguscoin_requestedhash.ToString().c_str());
        memset(&chunguscoin_requestedhash,0,sizeof(chunguscoin_requestedhash));
        chunguscoin_requestedcount = 0;
    }*/
    return true;
    ...
    if ( (!CheckBlock(futureblockp,pindex->GetHeight(),pindex,block, state, verifier,0)) || !fContextualCheckBlock )
    {
        ...
        if ( saplinght > 0 && (tmpptr= chainActive.LastTip()) != 0 )
        {
            ...
            if ( ASSETCHAINS_CBOPRET != 0 )
            {
                ...
                longest = chunguscoin_longestchain();
                if ( (longest == 0 || ht < longest-6) && (tmpindex=chunguscoin_chainactive(ht)) != 0 )
                {
...
void chunguscoin_currentheight_set(int32_t height);

CBlockIndex *chunguscoin_ensure(CBlock *pblock, uint256 hash)
{
...
CBlockIndex *oldchunguscoin_ensure(CBlock *pblock, uint256 hash)
{
    CBlockIndex *pindex=0,*previndex=0;
    if ( (pindex = chunguscoin_getblockindex(hash)) == 0 )
    {
        pindex = new CBlockIndex();
        if (!pindex)
            throw runtime_error("chunguscoin_ensure: new CBlockIndex failed");
    ...
    if ( miSelf == mapBlockIndex.end() )
    {
        LogPrintf("chunguscoin_ensure unexpected missing hash %s\n",hash.ToString().c_str());
        return(0);
    }
    if ( miSelf->second == 0 ) // create pindex so first Accept block doesnt fail
    {
        ...
        if ( pindex != 0 )
        {
            miSelf->second = pindex;
            LogPrintf("Block header %s is already known, but without pindex -> ensured %p\n",hash.ToString().c_str(),miSelf->second);
        } else LogPrintf("chunguscoin_ensure unexpected null pindex\n");
    }
    /*if ( hash != Params().GetConsensus().hashGenesisBlock )
        {
            miSelf = mapBlockIndex.find(pblock->hashPrevBlock);
            ...
            if ( (miSelf= mapBlockIndex.find(pblock->hashPrevBlock)) != mapBlockIndex.end() )
            {
                if ( miSelf->second == 0 ) // create pindex so first Accept block doesnt fail
                {
                    ...
                    if ( previndex != 0 )
                    {
                        ...
                        LogPrintf("autocreate previndex %s\n",pblock->hashPrevBlock.ToString().c_str());
                    } else LogPrintf("chunguscoin_ensure unexpected null previndex\n");
                }
            } else LogPrintf("chunguscoin_ensure unexpected null miprev\n");
        }
     }*/
    return(pindex);
}
...
bool ProcessNewBlock(bool from_miner,int32_t height,CValidationState &state, CNode* pfrom, CBlock* pblock, bool fForceProcessing, CDiskBlockPos *dbp)
{
    ...
    //LogPrintf("ProcessBlock %d\n",(int32_t)chainActive.LastTip()->GetHeight());
    {
        LOCK(cs_main);
        if ( chainActive.LastTip() != 0 )
            chunguscoin_currentheight_set(chainActive.LastTip()->GetHeight());
        checked = CheckBlock(&futureblock,height!=0?height:chunguscoin_block2height(pblock),0,*pblock, state, verifier,0);
        bool fRequested = MarkBlockAsReceived(hash);
        fRequested |= fForceProcessing;
        if ( checked != 0 && chunguscoin_checkPOW(0,0,pblock,height) < 0 ) //from_miner && ASSETCHAINS_STAKED == 0
        {
            checked = 0;
...
//void chunguscoin_pindex_init(CBlockIndex *pindex,int32_t height);
...
bool static LoadBlockIndexDB()
{
    ...
    BOOST_FOREACH(const PAIRTYPE(uint256, CBlockIndex*)& item, mapBlockIndex)
    {
        ...
        //chunguscoin_pindex_init(pindex,(int32_t)pindex->GetHeight());
    }
    ...
    if ( ASSETCHAINS_SYMBOL[0] == 0 ) {
        ...
        int32_t longestchain = chunguscoin_longestchain();
        // TODO: chunguscoin_longestchain does not have the data it needs at the time LoadBlockIndexDB
    ...
    if ( (pindex= chainActive.LastTip()) != 0 )
    {
        if ( ASSETCHAINS_SAPLING <= 0 )
        {
            ...
            chunguscoin_activate_sapling(pindex);
        }
    }
    return true;
}
...
bool LoadBlockIndex()
{
    ...
    CHUNGUSCOIN_LOADINGBLOCKS = 1;
    if (!fReindex && !LoadBlockIndexDB())
    {
        CHUNGUSCOIN_LOADINGBLOCKS = 0;
        return false;
    }
        ...
        while (!blkdat.eof()) {
            ...
            try {
                ...
                if (mapBlockIndex.count(hash) == 0 || (mapBlockIndex[hash]->nStatus & BLOCK_HAVE_DATA) == 0) {
                    ...
                    if (state.IsError())
                        break;
                } else if (hash != chainparams.GetConsensus().hashGenesisBlock && chunguscoin_blockheight(hash) % 1000 == 0) {
                    LogPrintf("Block Import: already had block %s at height %d\n", hash.ToString(), chunguscoin_blockheight(hash));
                }
                ...
                if (send && (mi->second->nStatus & BLOCK_HAVE_DATA))
                {
                    ...
                    else
                    {
                        if (inv.type == MSG_BLOCK)
                        {
                            ...
                            //for (z=31; z>=0; z--)
                            //    LogPrintf("%02x",((uint8_t *)&hash)[z]);
                            //LogPrintf(" send block %d\n",chunguscoin_block2height(&block));
...
//LogPrintf("netmsg: %s\n", strCommand.c_str());
    ...
    {
        if ( CHUNGUSCOIN_NSPV == 0 )//&& CHUNGUSCOIN_INSYNC != 0 )
        {
            std::vector<uint8_t> payload;
            vRecv >> payload;
            chunguscoin_nSPVreq(pfrom,payload);
        }
        return(true);
    }
    else if (GetBoolArg("-nspv_msg", DEFAULT_NSPV_PROCESSING) && strCommand == "nSPV")
    {
        if ( CHUNGUSCOIN_NSPV_SUPERLITE )
        {
            std::vector<uint8_t> payload;
            vRecv >> payload;
            chunguscoin_nSPVresp(pfrom,payload);
        }
        return(true);
    }
    else if ( CHUNGUSCOIN_NSPV_SUPERLITE )
        return(true);
...
bool SendMessages(CNode* pto, bool fSendTrickle)
{
    const Consensus::Params& consensusParams = Params().GetConsensus();
    {
        ...
        /*CBlockIndex *pindex;
        if ( chunguscoin_requestedhash != zero && chunguscoin_requestedcount < 16 && (pindex= chunguscoin_getblockindex(chunguscoin_requestedhash)) != 0 )
        {
            LogPrint("net","chunguscoin_requestedhash.%d request %s to nodeid.%d\n",chunguscoin_requestedcount,chunguscoin_requestedhash.ToString().c_str(),pto->GetId());
            LogPrintf("chunguscoin_requestedhash.%d request %s to nodeid.%d\n",chunguscoin_requestedcount,chunguscoin_requestedhash.ToString().c_str(),pto->GetId());
            vGetData.push_back(CInv(MSG_BLOCK, chunguscoin_requestedhash));
            MarkBlockAsInFlight(pto->GetId(), chunguscoin_requestedhash, consensusParams, pindex);
            chunguscoin_requestedcount++;
            if ( chunguscoin_requestedcount > 16 )
            {
                memset(&chunguscoin_requestedhash,0,sizeof(chunguscoin_requestedhash));
                chunguscoin_requestedcount = 0;
            }
