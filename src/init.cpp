...
extern bool chunguscoin_dailysnapshot(int32_t height);
extern int32_t CHUNGUSCOIN_LOADINGBLOCKS;
extern bool VERUS_MINTBLOCKS;
extern char ASSETCHAINS_SYMBOL[];
extern int32_t CHUNGUSCOIN_SNAPSHOT_INTERVAL;
extern void chunguscoin_init(int32_t height);
...
std::string HelpMessage(HelpMessageMode mode)
{
    ...
    strUsage += HelpMessageOpt("-conf=<file>", strprintf(_("Specify configuration file (default: %s)"), "chunguscoin.conf"));
...
#ifndef _WIN32
    strUsage += HelpMessageOpt("-pid=<file>", strprintf(_("Specify pid file (default: %s)"), "chunguscoind.pid"));
#endif
    ...
    strUsage += HelpMessageGroup(_("Chunguscoin Asset Chain options:"));
...
void CleanupBlockRevFiles()
{
    ...
    path chunguscoinstate = GetDataDir() / "chunguscoinstate";
    remove(chunguscoinstate);
...
#define PROCESS_DEP_ENABLE 0x00000001
    ...
    // Sanity check
    if (!InitSanityCheck())
        return InitError(_("Initialization sanity check failed. Chunguscoin is shutting down."));
    ...
    try {
        ...
        if (!lock.try_lock())
            return InitError(strprintf(_("Cannot obtain a lock on data directory %s. Chunguscoin is probably already running."), strDataDir));
    } catch(const boost::interprocess::interprocess_exception& e) {
        return InitError(strprintf(_("Cannot obtain a lock on data directory %s. Chunguscoin is probably already running.") + " %s.", strDataDir, e.what()));
    }
    ...
    if (GetBoolArg("-shrinkdebugfile", !fDebug))
        ShrinkDebugFile();
    //LogPrintf("\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n");
    LogPrintf("Chunguscoin version %s (%s)\n", FormatFullVersion(), CLIENT_DATE);
    ...
    while (!fLoaded && !fRequestShutdown) {
        ...
        do {
            try {
                ...
                if (fReindex) {
                    boost::filesystem::remove(GetDataDir() / "chunguscoinstate");
                ...
                if ( ASSETCHAINS_CC != 0 && CHUNGUSCOIN_SNAPSHOT_INTERVAL != 0 && chainActive.Height() >= CHUNGUSCOIN_SNAPSHOT_INTERVAL )
                {
                    if ( !chunguscoin_dailysnapshot(chainActive.Height()) )
                    {
    ...
    if (fDisableWallet) {
        ...
        if (nLoadWalletRet != DB_LOAD_OK)
        {
            ...
            else if (nLoadWalletRet == DB_NONCRITICAL_ERROR)
            {
                ...
                strErrors << _("Error loading wallet.dat: Wallet requires newer version of Chunguscoin") << "\n";
