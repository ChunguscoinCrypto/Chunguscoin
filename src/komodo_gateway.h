...
{
    int32_t baseid; struct chunguscoin_state *sp; int64_t netliability,maxallowed,maxval;
    *available = *deposited = *issued = *withdrawn = *approved = *redeemed = 0;
    if ( (baseid= chunguscoin_baseid(base)) >= 0 )
    {
        if ( (sp= chunguscoin_stateptrget(base)) != 0 )
        {
            ...
            *redeemed = sp->redeemed;
            ...
            maxallowed = chunguscoin_maxallowed(baseid);
...
struct pax_transaction *chunguscoin_paxfind(uint256 txid,uint16_t vout,uint8_t type)
{
    struct pax_transaction *pax; uint8_t buf[35];
    pthread_mutex_lock(&chunguscoin_mutex);
    pax_keyset(buf,txid,vout,type);
    HASH_FIND(hh,PAX,buf,sizeof(buf),pax);
    pthread_mutex_unlock(&chunguscoin_mutex);
    return(pax);
}

struct pax_transaction *chunguscoin_paxfinds(uint256 txid,uint16_t vout)
{
    struct pax_transaction *pax; int32_t i; uint8_t types[] = { 'I', 'D', 'X', 'A', 'W' };
    for (i=0; i<sizeof(types)/sizeof(*types); i++)
        if ( (pax= chunguscoin_paxfind(txid,vout,types[i])) != 0 )
            return(pax);
    return(0);
}

struct pax_transaction *chunguscoin_paxmark(int32_t height,uint256 txid,uint16_t vout,uint8_t type,int32_t mark)
{
    struct pax_transaction *pax; uint8_t buf[35];
    pthread_mutex_lock(&chunguscoin_mutex);
    pax_keyset(buf,txid,vout,type);
    ...
    if ( pax != 0 )
    {
        ...
        //    LogPrintf("mark ht.%d %.8f %.8f\n",pax->height,dstr(pax->chunguscoinshis),dstr(pax->fiatoshis));
...
void chunguscoin_paxdelete(struct pax_transaction *pax)
{
...
void chunguscoin_gateway_deposit(char *coinaddr,uint64_t value,char *symbol,uint64_t fiatoshis,uint8_t *rmd160,uint256 txid,uint16_t vout,uint8_t type,int32_t height,int32_t otherheight,char *source,int32_t approved) // assetchain context
{
    struct pax_transaction *pax; uint8_t buf[35]; int32_t addflag = 0; struct chunguscoin_state *sp; char str[CHUNGUSCOIN_ASSETCHAIN_MAXLEN],dest[CHUNGUSCOIN_ASSETCHAIN_MAXLEN],*s;
    //if ( CHUNGUSCOIN_PAX == 0 )
    //    return;
    //if ( strcmp(symbol,ASSETCHAINS_SYMBOL) != 0 )
    //    return;
    sp = chunguscoin_stateptr(str,dest);
    pthread_mutex_lock(&chunguscoin_mutex);
    pax_keyset(buf,txid,vout,type);
    HASH_FIND(hh,PAX,buf,sizeof(buf),pax);
    if ( pax == 0 )
...
int32_t chunguscoin_rwapproval(int32_t rwflag,uint8_t *opretbuf,struct pax_transaction *pax)
{
    int32_t i,len = 0;
    if ( rwflag == 1 )
    {
    ...
    len += iguana_rwnum(rwflag,&opretbuf[len],sizeof(pax->chunguscoinshis),&pax->chunguscoinshis);
...
int32_t chunguscoin_issued_opreturn(char *base,uint256 *txids,uint16_t *vouts,int64_t *values,int64_t *srcvalues,int32_t *kmdheights,int32_t *otherheights,int8_t *baseids,uint8_t *rmd160s,uint8_t *opretbuf,int32_t opretlen,int32_t ischunguscoin)
{
    struct pax_transaction p,*pax; int32_t i,n=0,j,len=0,incr,height,otherheight; uint8_t type,rmd160[20]; uint64_t fiatoshis; char symbol[CHUNGUSCOIN_ASSETCHAIN_MAXLEN];
    //if ( CHUNGUSCOIN_PAX == 0 )
    //    return(0);
    incr = 34 + (ischunguscoin * (2*sizeof(fiatoshis) + 2*sizeof(height) + 20 + 4));
    ...
    //if ( ASSETCHAINS_SYMBOL[0] == 0 || strncmp(ASSETCHAINS_SYMBOL,base,strlen(base)) == 0 )
    {
        ...
        for (n=0; n<opretlen/incr; n++)
        {
            ...
            {
                memset(&p,0,sizeof(p));
                len += chunguscoin_rwapproval(0,&opretbuf[len],&p);
                if ( values != 0 && srcvalues != 0 && kmdheights != 0 && otherheights != 0 && baseids != 0 && rmd160s != 0 )
                {
                    txids[n] = p.txid;
                    vouts[n] = p.vout;
                    values[n] = (strcmp("KMD",base) == 0) ? p.chunguscoinshis : p.fiatoshis;
                    srcvalues[n] = (strcmp("KMD",base) == 0) ? p.fiatoshis : p.chunguscoinshis;
                    kmdheights[n] = p.height;
                    otherheights[n] = p.otherheight;
                    memcpy(&rmd160s[n * 20],p.rmd160,20);
                    baseids[n] = chunguscoin_baseid(p.source);
                    if ( 0 )
                    {
                        char coinaddr[64];
            ...
            else
            {
                ...
                baseids[n] = chunguscoin_baseid(base);
                if ( (pax= chunguscoin_paxfinds(txids[n],vouts[n])) != 0 )
                {
                    values[n] = (strcmp("KMD",base) == 0) ? pax->chunguscoinshis : pax->fiatoshis;
                    srcvalues[n] = (strcmp("KMD",base) == 0) ? pax->fiatoshis : pax->chunguscoinshis;
                    kmdheights[n] = pax->height;
                    otherheights[n] = pax->otherheight;
                    memcpy(&rmd160s[n * 20],pax->rmd160,20);
                }
            }
            //LogPrintf(" chunguscoin_issued_opreturn issuedtxid v%d i.%d opretlen.%d\n",vouts[n],n,opretlen);
        }
    }
    return(n);
}

int32_t chunguscoin_paxcmp(char *symbol,int32_t kmdheight,uint64_t value,uint64_t checkvalue,uint64_t seed)
{
    int32_t ratio;
...
uint64_t chunguscoin_paxtotal()
{
    struct pax_transaction *pax,*pax2,*tmp,*tmp2; char symbol[CHUNGUSCOIN_ASSETCHAIN_MAXLEN],dest[CHUNGUSCOIN_ASSETCHAIN_MAXLEN],*str; int32_t i,ht; int64_t checktoshis; uint64_t seed,total = 0; struct chunguscoin_state *basesp;
    if ( CHUNGUSCOIN_PASSPORT_INITDONE == 0 ) //CHUNGUSCOIN_PAX == 0 ||
        return(0);
    if ( chunguscoin_isrealtime(&ht) == 0 )
        return(0);
    else
    {
        HASH_ITER(hh,PAX,pax,tmp)
        {
            ...
            basesp = chunguscoin_stateptrget(str);
            if ( basesp != 0 && pax->didstats == 0 )
            {
                if ( pax->type == 'I' && (pax2= chunguscoin_paxfind(pax->txid,pax->vout,'D')) != 0 )
                {
                    if ( pax2->fiatoshis != 0 )
                    {
                        pax->chunguscoinshis = pax2->chunguscoinshis;
                        pax->fiatoshis = pax2->fiatoshis;
                        basesp->issued += pax->fiatoshis;
                        pax->didstats = 1;
                        if ( strcmp(str,ASSETCHAINS_SYMBOL) == 0 )
                            LogPrintf("########### %p issued %s += %.8f kmdheight.%d %.8f other.%d\n",basesp,str,dstr(pax->fiatoshis),pax->height,dstr(pax->chunguscoinshis),pax->otherheight);
                ...
                else if ( pax->type == 'W' )
                {
                    ...
                    if ( (checktoshis= chunguscoin_paxprice(&seed,pax->height,pax->source,(char *)"KMD",(uint64_t)pax->fiatoshis)) != 0 )
                    {
                        if ( chunguscoin_paxcmp(pax->source,pax->height,pax->chunguscoinshis,checktoshis,seed) != 0 )
                        {
                            pax->marked = pax->height;
                            //LogPrintf("WITHDRAW.%s mark <- %d %.8f != %.8f\n",pax->source,pax->height,dstr(checktoshis),dstr(pax->chunguscoinshis));
                        }
                        else if ( pax->validated == 0 )
                        {
                            pax->validated = pax->chunguscoinshis = checktoshis;
                            //int32_t j; for (j=0; j<32; j++)
                            //    LogPrintf("%02x",((uint8_t *)&pax->txid)[j]);
                            //if ( strcmp(str,ASSETCHAINS_SYMBOL) == 0 )
                            //    LogPrintf(" v%d %p got WITHDRAW.%s kmd.%d ht.%d %.8f -> %.8f/%.8f\n",pax->vout,pax,pax->source,pax->height,pax->otherheight,dstr(pax->fiatoshis),dstr(pax->chunguscoinshis),dstr(checktoshis));
                        }
                    }
                }
            }
    ...
    chunguscoin_stateptr(symbol,dest);
    HASH_ITER(hh,PAX,pax,tmp)
    {
        pax->ready = 0;
        if ( 0 && pax->type == 'A' )
            LogPrintf("%p pax.%s <- %s marked.%d %.8f -> %.8f validated.%d approved.%d\n",pax,pax->symbol,pax->source,pax->marked,dstr(pax->chunguscoinshis),dstr(pax->fiatoshis),pax->validated != 0,pax->approved != 0);
        if ( pax->marked != 0 )
            continue;
        if ( strcmp(symbol,pax->symbol) == 0 || pax->type == 'A' )
        {
            if ( pax->marked == 0 )
            {
                ...
                else if ( pax->approved != 0 && pax->type == 'A' )
                {
                    if ( pax->validated != 0 )
                    {
                        total += pax->chunguscoinshis;
                        pax->ready = 1;
                    }
                    else
                    {
                        seed = 0;
                        checktoshis = chunguscoin_paxprice(&seed,pax->height,pax->source,(char *)"KMD",(uint64_t)pax->fiatoshis);
                        //LogPrintf("paxtotal PAX_fiatdest ht.%d price %s %.8f -> KMD %.8f vs %.8f\n",pax->height,pax->symbol,(double)pax->fiatoshis/COIN,(double)pax->chunguscoinshis/COIN,(double)checktoshis/COIN);
                        //LogPrintf(" v%d %.8f k.%d ht.%d\n",pax->vout,dstr(pax->chunguscoinshis),pax->height,pax->otherheight);
                        if ( seed != 0 && checktoshis != 0 )
                        {
                            if ( checktoshis == pax->chunguscoinshis )
                            {
                                total += pax->chunguscoinshis;
                                pax->validated = pax->chunguscoinshis;
                                pax->ready = 1;
                ...
                if ( 0 && pax->ready != 0 )
                    LogPrintf("%p (%c) pax.%s marked.%d %.8f -> %.8f validated.%d approved.%d ready.%d ht.%d\n",pax,pax->type,pax->symbol,pax->marked,dstr(pax->chunguscoinshis),dstr(pax->fiatoshis),pax->validated != 0,pax->approved != 0,pax->ready,pax->height);
            }
        }
    }
...
#define pax_b (*(struct pax_transaction **)b)
    uint64_t aval,bval;
    aval = pax_a->fiatoshis + pax_a->chunguscoinshis + pax_a->height;
    bval = pax_b->fiatoshis + pax_b->chunguscoinshis + pax_b->height;
	if ( bval > aval )
		return(-1);
	else if ( bval < aval )
		return(1);
...
int32_t chunguscoin_pending_withdraws(char *opretstr) // todo: enforce deterministic order
{
    struct pax_transaction *pax,*pax2,*tmp,*paxes[64]; uint8_t opretbuf[16384*4]; int32_t i,n,ht,len=0; uint64_t total = 0;
    if ( CHUNGUSCOIN_PAX == 0 || CHUNGUSCOIN_PASSPORT_INITDONE == 0 )
        return(0);
    if ( chunguscoin_isrealtime(&ht) == 0 || ASSETCHAINS_SYMBOL[0] != 0 )
        return(0);
    n = 0;
    HASH_ITER(hh,PAX,pax,tmp)
    {
        if ( pax->type == 'W' )
        {
            if ( (pax2= chunguscoin_paxfind(pax->txid,pax->vout,'A')) != 0 )
            {
                if ( pax2->approved != 0 )
                    pax->approved = pax2->approved;
            }
            else if ( (pax2= chunguscoin_paxfind(pax->txid,pax->vout,'X')) != 0 )
            ...
            if ( pax->marked == 0 && pax->approved == 0 && pax->validated != 0 ) //strcmp((char *)"KMD",pax->symbol) == 0 &&
            {
                if ( n < sizeof(paxes)/sizeof(*paxes) )
                {
                    ...
                    //int32_t j; for (j=0; j<32; j++)
                    //    LogPrintf("%02x",((uint8_t *)&pax->txid)[j]);
                    //LogPrintf(" %s.(kmdht.%d ht.%d marked.%u approved.%d validated %.8f) %.8f\n",pax->source,pax->height,pax->otherheight,pax->marked,pax->approved,dstr(pax->validated),dstr(pax->chunguscoinshis));
                }
            }
        }
    }
    ...
    if ( n > 0 )
    {
        ...
        for (i=0; i<n; i++)
        {
            if ( len < (sizeof(opretbuf)>>3)*7 )
                len += chunguscoin_rwapproval(1,&opretbuf[len],paxes[i]);
        }
        if ( len > 0 )
            init_hexbytes_noT(opretstr,opretbuf,len);
    }
    //LogPrintf("chunguscoin_pending_withdraws len.%d PAXTOTAL %.8f\n",len,dstr(chunguscoin_paxtotal()));
    return(len);
}

int32_t chunguscoin_gateway_deposits(CMutableTransaction *txNew,char *base,int32_t tochunguscoin)
{
    struct pax_transaction *pax,*tmp; char symbol[CHUNGUSCOIN_ASSETCHAIN_MAXLEN],dest[CHUNGUSCOIN_ASSETCHAIN_MAXLEN]; uint8_t *script,opcode,opret[16384*4],data[16384*4]; int32_t i,baseid,ht,len=0,opretlen=0,numvouts=1; struct chunguscoin_state *sp; uint64_t available,deposited,issued,withdrawn,approved,redeemed,mask,sum = 0;
    if ( CHUNGUSCOIN_PASSPORT_INITDONE == 0 )//CHUNGUSCOIN_PAX == 0 ||
        return(0);
    struct chunguscoin_state *kmdsp = chunguscoin_stateptrget((char *)"KMD");
    sp = chunguscoin_stateptr(symbol,dest);
    strcpy(symbol,base);
    if ( ASSETCHAINS_SYMBOL[0] != 0 && chunguscoin_baseid(ASSETCHAINS_SYMBOL) < 0 )
        return(0);
    PENDING_CHUNGUSCOIN_TX = 0;
    for (i=0; i<3; i++)
    {
        if ( chunguscoin_isrealtime(&ht) != 0 )
            break;
    ...
    HASH_ITER(hh,PAX,pax,tmp)
    {
        if ( pax->type != 'D' && pax->type != 'A' )
            continue;
        {
#ifdef CHUNGUSCOIN_ASSETCHAINS_WAITNOTARIZE
        ...
        /*LogPrintf("pax.%s marked.%d %.8f -> %.8f ready.%d validated.%d\n",pax->symbol,pax->marked,dstr(pax->chunguscoinshis),dstr(pax->fiatoshis),pax->ready!=0,pax->validated!=0);
        ...
        if ( pax->type == 'A' && ASSETCHAINS_SYMBOL[0] == 0 )
        {
            if ( kmdsp != 0 )
            {
                if ( (baseid= chunguscoin_baseid(pax->symbol)) < 0 || ((1LL << baseid) & sp->RTmask) == 0 )
                {
        ...
        //LogPrintf("redeem.%d? (%c) %p pax.%s marked.%d %.8f -> %.8f ready.%d validated.%d approved.%d\n",tochunguscoin,pax->type,pax,pax->symbol,pax->marked,dstr(pax->chunguscoinshis),dstr(pax->fiatoshis),pax->ready!=0,pax->validated!=0,pax->approved!=0);
        if ( 0 && ASSETCHAINS_SYMBOL[0] != 0 )
            LogPrintf("pax.%s marked.%d %.8f -> %.8f\n",ASSETCHAINS_SYMBOL,pax->marked,dstr(pax->chunguscoinshis),dstr(pax->fiatoshis));
        if ( opcode == 'I' )
        {
            sum += pax->fiatoshis;
            if ( sum > available )
        ...
        txNew->vout[numvouts].nValue = (opcode == 'I') ? pax->fiatoshis : pax->chunguscoinshis;
        ...
        *script++ = 0xac;
        if ( tochunguscoin == 0 )
        {
            ...
            PENDING_CHUNGUSCOIN_TX += pax->fiatoshis;
        }
        else
        {
            len += chunguscoin_rwapproval(1,&data[len],pax);
            PENDING_CHUNGUSCOIN_TX += pax->chunguscoinshis;
            LogPrintf(" len.%d vout.%u DEPOSIT %.8f <- pax.%s pending ht %d %d %.8f | ",len,pax->vout,(double)txNew->vout[numvouts].nValue/COIN,symbol,pax->height,pax->otherheight,dstr(PENDING_CHUNGUSCOIN_TX));
        }
    ...
    if ( numvouts > 1 )
    {
        ...
        LogPrintf(" <- data[%d]\n",len);
        opretlen = chunguscoin_opreturnscript(opret,opcode,data,len);
        txNew->vout.resize(numvouts+1);
        ...
        LogPrintf(" <- opret, MINER deposits.%d (%s) vouts.%d %.8f opretlen.%d\n",tochunguscoin,ASSETCHAINS_SYMBOL,numvouts,dstr(PENDING_CHUNGUSCOIN_TX),opretlen);
        return(1);
    }
    return(0);
}
...
int32_t chunguscoin_checkvout(int32_t vout,int32_t k,int32_t indallvouts)
{
    if ( k < indallvouts )
        return(vout == 1);
...
int32_t chunguscoin_bannedset(int32_t *indallvoutsp,uint256 *array,int32_t max)
{
    int32_t i;
    if ( sizeof(banned_txids)/sizeof(*banned_txids) > max )
    {
        LogPrintf("chunguscoin_bannedset: buffer too small %ld vs %d\n",sizeof(banned_txids)/sizeof(*banned_txids),max);
        StartShutdown();
    }
...
void chunguscoin_passport_iteration();

int32_t chunguscoin_check_deposit(int32_t height,const CBlock& block,uint32_t prevtime) // verify above block is valid pax pricing
{
    static uint256 array[64]; static int32_t numbanned,indallvouts;
    int32_t i,j,k,n,ht,baseid,txn_count,activation,num,opretlen,offset=1,errs=0,notmatched=0,matched=0,kmdheights[256],otherheights[256]; uint256 hash,txids[256]; char symbol[CHUNGUSCOIN_ASSETCHAIN_MAXLEN],base[CHUNGUSCOIN_ASSETCHAIN_MAXLEN]; uint16_t vouts[256]; int8_t baseids[256]; uint8_t *script,opcode,rmd160s[256*20]; uint64_t total,subsidy,available,deposited,issued,withdrawn,approved,redeemed,seed; int64_t checktoshis,values[256],srcvalues[256]; struct pax_transaction *pax; struct chunguscoin_state *sp; CTransaction tx;
    activation = 235300;
    if ( *(int32_t *)&array[0] == 0 )
        numbanned = chunguscoin_bannedset(&indallvouts,array,(int32_t)(sizeof(array)/sizeof(*array)));
    ...
    if ( ASSETCHAINS_SYMBOL[0] == 0 )
    {
        for (i=0; i<txn_count; i++)
        {
            ...
            for (j=0; j<n; j++)
            {
                ...
                {
                    if ( block.vtx[i].vin[j].prevout.hash == array[k] && chunguscoin_checkvout(block.vtx[i].vin[j].prevout.n,k,indallvouts) != 0 ) //(block.vtx[i].vin[j].prevout.n == 1 || k >= indallvouts)  )
                    {
         ...
         NetworkUpgradeActive(height, Params().GetConsensus(), Consensus::UPGRADE_SAPLING) )
    {
        ...
        if ( ASSETCHAINS_SYMBOL[0] == 0 )
        {
            ...
            else if ( block.nBits == CHUNGUSCOIN_MINDIFF_NBITS && total > 0 ) // to deal with fee stealing
            {
                LogPrintf("notary mined ht.%d with extra %.8f\n",height,dstr(total));
                if ( height > CHUNGUSCOIN_NOTARIES_HEIGHT1 )
                    return(-1);
            }
            ...
            else if ( height > 814000 )
            {
                ...
                //int32_t notary = chunguscoin_electednotary(&num,script+1,height,0);
                //if ( (-1 * (chunguscoin_electednotary(&num,script+1,height,0) >= 0) * (height > 1000000)) < 0 )
                //    LogPrintf( ">>>>>>> FAILED BLOCK.%d notary.%d insync.%d\n",height,notary,CHUNGUSCOIN_INSYNC);
                //else
                //    LogPrintf( "<<<<<<< VALID BLOCK.%d notary.%d insync.%d\n",height,notary,CHUNGUSCOIN_INSYNC);
                return(-1 * (chunguscoin_electednotary(&num,script+1,height,0) >= 0) * (height > 1000000));
            }
        }
        else
        {
            ...
            {
                if ( (checktoshis= chunguscoin_checkcommission((CBlock *)&block,height)) < 0 )
                {
...
const char *chunguscoin_opreturn(int32_t height,uint64_t value,uint8_t *opretbuf,int32_t opretlen,uint256 txid,uint16_t vout,char *source)
{
    uint8_t rmd160[20],rmd160s[64*20],addrtype,shortflag,pubkey33[33]; int32_t didstats,i,j,n,kvheight,len,tochunguscoin,kmdheight,otherheights[64],kmdheights[64]; int8_t baseids[64]; char base[4],coinaddr[64],destaddr[64]; uint256 txids[64]; uint16_t vouts[64]; uint64_t convtoshis,seed; int64_t fee,fiatoshis,chunguscoinshis,checktoshis,values[64],srcvalues[64]; struct pax_transaction *pax,*pax2; struct chunguscoin_state *basesp; double diff;
    const char *typestr = "unknown";
    if ( ASSETCHAINS_SYMBOL[0] != 0 && chunguscoin_baseid(ASSETCHAINS_SYMBOL) < 0 && opretbuf[0] != 'K' )
    {
        //LogPrintf("chunguscoin_opreturn skip %s\n",ASSETCHAINS_SYMBOL);
        return("assetchain");
    }
    memset(baseids,0xff,sizeof(baseids));
    memset(values,0,sizeof(values));
    ...
    memset(kmdheights,0,sizeof(kmdheights));
    memset(otherheights,0,sizeof(otherheights));
    tochunguscoin = (chunguscoin_is_issuer() == 0);
    if ( opretbuf[0] == 'K' && opretlen != 40 )
    {
        chunguscoin_kvupdate(opretbuf,opretlen,value);
        return("kv");
    }
    else if ( ASSETCHAINS_SYMBOL[0] == 0 && CHUNGUSCOIN_PAX == 0 )
        return("nopax");
    if ( opretbuf[0] == 'D' )
    {
        tochunguscoin = 0;
        if ( opretlen == 38 ) // any KMD tx
        {
            ...
            bitcoin_address(coinaddr,addrtype,rmd160,20);
            checktoshis = PAX_fiatdest(&seed,tochunguscoin,destaddr,pubkey33,coinaddr,kmdheight,base,fiatoshis);
            if ( chunguscoin_paxcmp(base,kmdheight,value,checktoshis,kmdheight < 225000 ? seed : 0) != 0 )
                checktoshis = PAX_fiatdest(&seed,tochunguscoin,destaddr,pubkey33,coinaddr,height,base,fiatoshis);
            typestr = "deposit";
            if ( 0 && strcmp("NOK",base) == 0 )
            {
                LogPrintf("[%s] %s paxdeposit height.%d vs kmdheight.%d\n",ASSETCHAINS_SYMBOL,base,height,kmdheight);
                LogPrintf("(%s) (%s) kmdheight.%d vs height.%d check %.8f vs %.8f tochunguscoin.%d %d seed.%llx\n",ASSETCHAINS_SYMBOL,base,kmdheight,height,dstr(checktoshis),dstr(value),chunguscoin_is_issuer(),strncmp(ASSETCHAINS_SYMBOL,base,strlen(base)) == 0,(long long)seed);
                for (i=0; i<32; i++)
                    LogPrintf("%02x",((uint8_t *)&txid)[i]);
            ...
            if ( strcmp(base,ASSETCHAINS_SYMBOL) == 0 && (kmdheight > 195000 || kmdheight <= height) )
            {
                ...
                if ( chunguscoin_paxcmp(base,kmdheight,value,checktoshis,kmdheight < 225000 ? seed : 0) == 0 )
                {
                    if ( (pax= chunguscoin_paxfind(txid,vout,'D')) == 0 )
                    {
                        if ( (basesp= chunguscoin_stateptrget(base)) != 0 )
                        {
                        ...
                        chunguscoin_gateway_deposit(coinaddr,value,base,fiatoshis,rmd160,txid,vout,'D',kmdheight,height,(char *)"KMD",0);
                    }
                    if ( (pax= chunguscoin_paxfind(txid,vout,'D')) != 0 )
                    {
                        ...
                        if ( didstats == 0 && pax->didstats == 0 )
                        {
                            if ( (basesp= chunguscoin_stateptrget(base)) != 0 )
                            {
                                ...
                                didstats = 1;
                                if ( 0 && strcmp(base,ASSETCHAINS_SYMBOL) == 0 )
                                    LogPrintf("########### %p depositedB %s += %.8f/%.8f kmdheight.%d/%d %.8f/%.8f\n",basesp,base,dstr(fiatoshis),dstr(pax->fiatoshis),kmdheight,pax->height,dstr(value),dstr(pax->chunguscoinshis));
                            }
                        } //
                        if ( didstats != 0 )
                            pax->didstats = 1;
                        if ( (pax2= chunguscoin_paxfind(txid,vout,'I')) != 0 )
                        {
                            pax2->fiatoshis = pax->fiatoshis;
                            pax2->chunguscoinshis = pax->chunguscoinshis;
                            ...
                            if ( pax2->didstats == 0 )
                            {
                                if ( (basesp= chunguscoin_stateptrget(base)) != 0 )
                                {
                                    ...
                                    if ( 0 && strcmp(base,"USD") == 0 )
                                        LogPrintf("########### %p issueda %s += %.8f kmdheight.%d %.8f other.%d [%d]\n",basesp,base,dstr(pax2->fiatoshis),pax2->height,dstr(pax2->chunguscoinshis),pax2->otherheight,height);
                                }
                            }
                        }
                    }
    ...
    else if ( opretbuf[0] == 'I' )
    {
        ...
        {
            if ( (n= chunguscoin_issued_opreturn(base,txids,vouts,values,srcvalues,kmdheights,otherheights,baseids,rmd160s,opretbuf,opretlen,0)) > 0 )
            {
                for (i=0; i<n; i++)
                {
                    if ( baseids[i] < 0 )
                    ...
                    chunguscoin_gateway_deposit(coinaddr,0,ASSETCHAINS_SYMBOL,0,0,txids[i],vouts[i],'I',height,0,CURRENCIES[baseids[i]],0);
                    chunguscoin_paxmark(height,txids[i],vouts[i],'I',height);
                    if ( (pax= chunguscoin_paxfind(txids[i],vouts[i],'I')) != 0 )
                    {
                        pax->type = opretbuf[0];
                        strcpy(pax->source,(char *)&opretbuf[opretlen-4]);
                        if ( (pax2= chunguscoin_paxfind(txids[i],vouts[i],'D')) != 0 && pax2->fiatoshis != 0 && pax2->chunguscoinshis != 0 )
                        {
                            // realtime path?
                            pax->fiatoshis = pax2->fiatoshis;
                            pax->chunguscoinshis = pax2->chunguscoinshis;
                            ...
                            if ( pax->didstats == 0 )
                            {
                                if ( (basesp= chunguscoin_stateptrget(CURRENCIES[baseids[i]])) != 0 )
                                {
                                    basesp->issued += pax->fiatoshis;
                                    pax->didstats = 1;
                                    ...
                                    if ( 1 && strcmp(CURRENCIES[baseids[i]],"USD") == 0 )
                                        LogPrintf("########### %p issuedb %s += %.8f kmdheight.%d %.8f other.%d [%d]\n",basesp,CURRENCIES[baseids[i]],dstr(pax->fiatoshis),pax->height,dstr(pax->chunguscoinshis),pax->otherheight,height);
                                }
                            }
                        }
                    }
                    if ( (pax= chunguscoin_paxmark(height,txids[i],vouts[i],'I',height)) != 0 )
                        chunguscoin_paxdelete(pax);
                    if ( (pax= chunguscoin_paxmark(height,txids[i],vouts[i],'D',height)) != 0 )
                        chunguscoin_paxdelete(pax);
                }
    ...
    {
        if ( chunguscoin_baseid((char *)&opretbuf[opretlen-4]) >= 0 && strcmp(ASSETCHAINS_SYMBOL,(char *)&opretbuf[opretlen-4]) == 0 )
        {
            for (i=0; i<opretlen; i++)
                LogPrintf("%02x",opretbuf[i]);
        ...
        iguana_rwnum(0,&opretbuf[34],sizeof(kmdheight),&kmdheight);
        memset(base,0,sizeof(base));
        PAX_pubkey(0,&opretbuf[1],&addrtype,rmd160,base,&shortflag,&chunguscoinshis);
        bitcoin_address(coinaddr,addrtype,rmd160,20);
        checktoshis = PAX_fiatdest(&seed,tochunguscoin,destaddr,pubkey33,coinaddr,kmdheight,base,value);
        typestr = "withdraw";
        //LogPrintf(" [%s] WITHDRAW %s.height.%d vs height.%d check %.8f/%.8f vs %.8f tochunguscoin.%d %d seed.%llx -> (%s) len.%d\n",ASSETCHAINS_SYMBOL,base,kmdheight,height,dstr(checktoshis),dstr(chunguscoinshis),dstr(value),chunguscoin_is_issuer(),strncmp(ASSETCHAINS_SYMBOL,base,strlen(base)) == 0,(long long)seed,coinaddr,opretlen);
        didstats = 0;
        //if ( chunguscoin_paxcmp(base,kmdheight,chunguscoinshis,checktoshis,seed) == 0 )
        {
            if ( value != 0 && ((pax= chunguscoin_paxfind(txid,vout,'W')) == 0 || pax->didstats == 0) )
            {
                if ( (basesp= chunguscoin_stateptrget(base)) != 0 )
                {
                ...
                if ( 0 && strcmp(base,"RUB") == 0 && (pax == 0 || pax->approved == 0) )
                    LogPrintf("notarize %s %.8f -> %.8f kmd.%d other.%d\n",ASSETCHAINS_SYMBOL,dstr(value),dstr(chunguscoinshis),kmdheight,height);
            }
            chunguscoin_gateway_deposit(coinaddr,0,(char *)"KMD",value,rmd160,txid,vout,'W',kmdheight,height,source,0);
            if ( (pax= chunguscoin_paxfind(txid,vout,'W')) != 0 )
            {
                pax->type = opretbuf[0];
                ...
                pax->height = kmdheight;
                pax->otherheight = height;
                pax->chunguscoinshis = chunguscoinshis;
            }
        } // else LogPrintf("withdraw %s paxcmp ht.%d %d error value %.8f -> %.8f vs %.8f\n",base,kmdheight,height,dstr(value),dstr(chunguscoinshis),dstr(checktoshis));
        // need to allocate pax
    }
    else if ( height < 236000 && tochunguscoin != 0 && opretbuf[0] == 'A' && ASSETCHAINS_SYMBOL[0] == 0 )
    {
        tochunguscoin = 1;
        if ( 0 && ASSETCHAINS_SYMBOL[0] != 0 )
        {
            for (i=0; i<opretlen; i++)
                LogPrintf("%02x",opretbuf[i]);
            LogPrintf(" opret[%c] else path tochunguscoin.%d ht.%d before %.8f opretlen.%d\n",opretbuf[0],tochunguscoin,height,dstr(chunguscoin_paxtotal()),opretlen);
        }
        if ( (n= chunguscoin_issued_opreturn(base,txids,vouts,values,srcvalues,kmdheights,otherheights,baseids,rmd160s,opretbuf,opretlen,1)) > 0 )
        {
            for (i=0; i<n; i++)
            {
                //for (j=0; j<32; j++)
                ...
                if ( baseids[i] < 0 )
                {
                    ...
                    if ( (pax= chunguscoin_paxfind(txids[i],vouts[i],'W')) != 0 || (pax= chunguscoin_paxfind(txids[i],vouts[i],'X')) != 0 )
                    {
                        baseids[i] = chunguscoin_baseid(pax->symbol);
                ...
                didstats = 0;
                seed = 0;
                checktoshis = chunguscoin_paxprice(&seed,kmdheights[i],CURRENCIES[baseids[i]],(char *)"KMD",(uint64_t)values[i]);
                ...
                {
                    if ( (pax= chunguscoin_paxfind(txids[i],vouts[i],'A')) == 0 )
                    {
                        bitcoin_address(coinaddr,60,&rmd160s[i*20],20);
                        chunguscoin_gateway_deposit(coinaddr,srcvalues[i],CURRENCIES[baseids[i]],values[i],&rmd160s[i*20],txids[i],vouts[i],'A',kmdheights[i],otherheights[i],CURRENCIES[baseids[i]],kmdheights[i]);
                        if ( (pax= chunguscoin_paxfind(txids[i],vouts[i],'A')) == 0 )
                            LogPrintf("unexpected null pax for approve\n");
                        else pax->validated = checktoshis;
                        if ( (pax2= chunguscoin_paxfind(txids[i],vouts[i],'W')) != 0 )
                            pax2->approved = kmdheights[i];
                        chunguscoin_paxmark(height,txids[i],vouts[i],'W',height);
                        //chunguscoin_paxmark(height,txids[i],vouts[i],'A',height);
                        if ( values[i] != 0 && (basesp= chunguscoin_stateptrget(CURRENCIES[baseids[i]])) != 0 )
                        {
        ...
        if ( (n= chunguscoin_issued_opreturn(base,txids,vouts,values,srcvalues,kmdheights,otherheights,baseids,rmd160s,opretbuf,opretlen,1)) > 0 )
        {
            for (i=0; i<n; i++)
            {
                if ( baseids[i] < 0 )
                    continue;
                bitcoin_address(coinaddr,60,&rmd160s[i*20],20);
                chunguscoin_gateway_deposit(coinaddr,0,0,0,0,txids[i],vouts[i],'X',height,0,(char *)"KMD",0);
                chunguscoin_paxmark(height,txids[i],vouts[i],'W',height);
                chunguscoin_paxmark(height,txids[i],vouts[i],'A',height);
                chunguscoin_paxmark(height,txids[i],vouts[i],'X',height);
                if ( (pax= chunguscoin_paxfind(txids[i],vouts[i],'X')) != 0 )
                {
                    pax->type = opretbuf[0];
                    if ( height < 121842 ) // fields got switched around due to legacy issues and approves
                        value = srcvalues[i];
                    else value = values[i];
                    if ( baseids[i] >= 0 && value != 0 && (basesp= chunguscoin_stateptrget(CURRENCIES[baseids[i]])) != 0 )
                    {
                ...
                if ( (pax= chunguscoin_paxmark(height,txids[i],vouts[i],'W',height)) != 0 )
                    chunguscoin_paxdelete(pax);
                if ( (pax= chunguscoin_paxmark(height,txids[i],vouts[i],'A',height)) != 0 )
                    chunguscoin_paxdelete(pax);
                if ( (pax= chunguscoin_paxmark(height,txids[i],vouts[i],'X',height)) != 0 )
                    chunguscoin_paxdelete(pax);
            }
        } //else LogPrintf("chunguscoin_issued_opreturn returned %d\n",n);
    }
    return(typestr);
}

int32_t chunguscoin_parsestatefiledata(struct chunguscoin_state *sp,uint8_t *filedata,long *fposp,long datalen,char *symbol,char *dest);

void chunguscoin_stateind_set(struct chunguscoin_state *sp,uint32_t *inds,int32_t n,uint8_t *filedata,long datalen,char *symbol,char *dest)
{
        ...
        for (lastfpos=fpos=prevpos100=i=0; i<n; i++)
        {
            ...
            else
            {
                ...
                else
                {
                    ...
                    {
                        if ( CHUNGUSCOIN_PAX != 0 && numv > numV-1440 )
                            doissue = 1;
                        numv++;
                    }
                    else if ( func == 'R' )
                    ...
                    if ( doissue != 0 )
                    {
                        ...
                        chunguscoin_parsestatefiledata(sp,filedata,&lastfpos,datalen,symbol,dest);
                        count++;
                    }
                }
            }
...
long chunguscoin_stateind_validate(struct chunguscoin_state *sp,char *indfname,uint8_t *filedata,long datalen,uint32_t *prevpos100p,uint32_t *indcounterp,char *symbol,char *dest)
{
    ...
    if ( (inds= OS_fileptr(&fsize,indfname)) != 0 )
    {
        ...
        if ( (fsize % sizeof(uint32_t)) == 0 )
        {
            ...
            *prevpos100p = prevpos100;
            if ( sp != 0 )
                chunguscoin_stateind_set(sp,(uint32_t *)inds,n,filedata,fpos,symbol,dest);
...
long chunguscoin_indfile_update(FILE *indfp,uint32_t *prevpos100p,long lastfpos,long newfpos,uint8_t func,uint32_t *indcounterp)
{
    uint32_t tmp;
    if ( indfp != 0 )
    {
...
int32_t chunguscoin_faststateinit(struct chunguscoin_state *sp,char *fname,char *symbol,char *dest)
{
    ...
    {
        if ( 1 )//datalen >= (1LL << 32) || GetArg("-genind",0) != 0 || (validated= chunguscoin_stateind_validate(0,indfname,filedata,datalen,&prevpos100,&indcounter,symbol,dest)) < 0 )
        {
            lastfpos = fpos = 0;
            ...
            while ( (func= chunguscoin_parsestatefiledata(sp,filedata,&fpos,datalen,symbol,dest)) >= 0 )
            {
                lastfpos = chunguscoin_indfile_update(indfp,&prevpos100,lastfpos,fpos,func,&indcounter);
            }
            if ( indfp != 0 )
            {
                fclose(indfp);
                if ( (fpos= chunguscoin_stateind_validate(0,indfname,filedata,datalen,&prevpos100,&indcounter,symbol,dest)) < 0 )
                    LogPrintf("unexpected chunguscoinstate.ind validate failure %s datalen.%ld\n",indfname,datalen);
        ...
        else if ( validated > 0 )
        {
            if ( (indfp= fopen(indfname,"rb+")) != 0 )
            {
                ...
                if ( chunguscoin_stateind_validate(sp,indfname,filedata,datalen,&prevpos100,&indcounter,symbol,dest) < 0 )
                    LogPrintf("unexpected chunguscoinstate.ind validate failure %s datalen.%ld\n",indfname,datalen);
                else
                {
        ...
        } else LogPrintf("chunguscoin_faststateinit unexpected case\n");
        free(filedata);
        return(finished == 1);
    }
    return(-1);
...
uint64_t chunguscoin_interestsum();
...
void chunguscoin_passport_iteration()
{
    ...
    FILE *fp; uint8_t *filedata; long fpos,datalen,lastfpos; int32_t baseid,limit,n,ht,isrealtime,expired,refid,blocks,longest; struct chunguscoin_state *sp,*refsp; char *retstr,fname[512],*base,symbol[CHUNGUSCOIN_ASSETCHAIN_MAXLEN],dest[CHUNGUSCOIN_ASSETCHAIN_MAXLEN]; uint32_t buf[3],starttime; uint64_t RTmask = 0; //CBlockIndex *pindex;
    expired = 0;
    while ( 0 && CHUNGUSCOIN_INITDONE == 0 )
    {
        LogPrintf("[%s] PASSPORT iteration waiting for CHUNGUSCOIN_INITDONE\n",ASSETCHAINS_SYMBOL);
#ifdef WIN32
    ...
    if ( chunguscoin_chainactive_timestamp() > lastinterest )
    {
        if ( ASSETCHAINS_SYMBOL[0] == 0 )
            chunguscoin_interestsum();
        //chunguscoin_longestchain();
        lastinterest = chunguscoin_chainactive_timestamp();
    }
    refsp = chunguscoin_stateptr(symbol,dest);
    ...
    {
        limit = 10000000;
        refid = chunguscoin_baseid(ASSETCHAINS_SYMBOL)+1; // illegal base -> baseid.-1 -> 0
        if ( refid == 0 )
        {
            CHUNGUSCOIN_PASSPORT_INITDONE = 1;
            return;
        }
    }
    /*if ( CHUNGUSCOIN_PAX == 0 )
    ...
    for (baseid=32; baseid>=0; baseid--)
    {
        ...
        if ( baseid+1 != refid ) // only need to import state from a different coin
        {
            if ( baseid == 32 ) // only care about KMD's state
            {
                ...
                chunguscoin_statefname(fname,baseid<32?base:(char *)"",(char *)"chunguscoinstate");
                chunguscoin_nameset(symbol,dest,base);
                sp = chunguscoin_stateptrget(symbol);
                n = 0;
                if ( lastpos[baseid] == 0 && (filedata= OS_fileptr(&datalen,fname)) != 0 )
                {
                    ...
                    while ( chunguscoin_parsestatefiledata(sp,filedata,&fpos,datalen,symbol,dest) >= 0 )
                        lastfpos = fpos;
                ...
                else if ( (fp= fopen(fname,"rb")) != 0 && sp != 0 )
                {
                    ...
                    if ( ftell(fp) > lastpos[baseid] )
                    {
                        ...
                        while ( chunguscoin_parsestatefile(sp,fp,symbol,dest) >= 0 && n < limit )
                        {
                            if ( n == limit-1 )
                            {
                                if ( time(NULL) < starttime+maxseconds )
                        ...
                        if ( 0 && lastpos[baseid] == 0 && strcmp(symbol,"KMD") == 0 )
                            LogPrintf("from.(%s) lastpos[%s] %ld isrt.%d\n",ASSETCHAINS_SYMBOL,CURRENCIES[baseid],lastpos[baseid],chunguscoin_isrealtime(&ht));
                    } //else LogPrintf("%s.%ld ",CURRENCIES[baseid],ftell(fp));
                    fclose(fp);
                } else LogPrintf("load error.(%s) %p\n",fname,sp);
                chunguscoin_statefname(fname,baseid<32?base:(char *)"",(char *)"realtime");
                if ( (fp= fopen(fname,"rb")) != 0 )
                {
                    if ( fread(buf,1,sizeof(buf),fp) == sizeof(buf) )
                    {
                        ...
                        else if ( CHUNGUSCOIN_PAX != 0 && (time(NULL)-buf[2]) > 60 && ASSETCHAINS_SYMBOL[0] != 0 )
        ...
        else
        {
            ...
            if ( (fp= fopen(fname,"wb")) != 0 )
            {
                ...
                buf[1] = (uint32_t)chunguscoin_longestchain();
    ...
    //chunguscoin_paxtotal(); // calls chunguscoin_isrealtime(), which calls chunguscoin_longestchain()
    refsp->RTmask |= RTmask;
    if ( expired == 0 && CHUNGUSCOIN_PASSPORT_INITDONE == 0 )
    {
        CHUNGUSCOIN_PASSPORT_INITDONE = 1;
...
#define CHUNGUSCOIN_LOCALPRICE_CACHESIZE 13
#define CHUNGUSCOIN_MAXPRICES 2048
...
struct chunguscoin_extremeprice
{
    uint256 blockhash;
...
struct chunguscoin_priceinfo
{
    FILE *fp;
    char symbol[64];
} PRICES[CHUNGUSCOIN_MAXPRICES];

uint32_t PriceCache[CHUNGUSCOIN_LOCALPRICE_CACHESIZE][CHUNGUSCOIN_MAXPRICES];//4+sizeof(Cryptos)/sizeof(*Cryptos)+sizeof(Forex)/sizeof(*Forex)];
int64_t PriceMult[CHUNGUSCOIN_MAXPRICES];
int32_t chunguscoin_cbopretsize(uint64_t flags);

void chunguscoin_PriceCache_shift()
{
    int32_t i;
    for (i=CHUNGUSCOIN_LOCALPRICE_CACHESIZE-1; i>0; i--)
        memcpy(PriceCache[i],PriceCache[i-1],sizeof(PriceCache[i]));
    memcpy(PriceCache[0],Mineropret.data(),Mineropret.size());
}

int32_t _chunguscoin_heightpricebits(uint64_t *seedp,uint32_t *heightbits,CBlock *block)
{
...
// chunguscoin_heightpricebits() extracts the price data in the coinbase for nHeight
int32_t chunguscoin_heightpricebits(uint64_t *seedp,uint32_t *heightbits,int32_t nHeight)
{
    CBlockIndex *pindex; CBlock block;
    if ( seedp != 0 )
        *seedp = 0;
    if ( (pindex= chunguscoin_chainactive(nHeight)) != 0 )
    {
        if ( chunguscoin_blockload(block,pindex) == 0 )
        {
            return(_chunguscoin_heightpricebits(seedp,heightbits,&block));
        }
    }
...
/*
 chunguscoin_pricenew() is passed in a reference price, the change tolerance and the proposed price. it needs to return a clipped price if it is too big and also set a flag if it is at or above the limit
 */
uint32_t chunguscoin_pricenew(char *maxflagp,uint32_t price,uint32_t refprice,int64_t tolerance)
{
...
// chunguscoin_pricecmp() returns -1 if any of the prices are beyond the tolerance
int32_t chunguscoin_pricecmp(int32_t nHeight,int32_t n,char *maxflags,uint32_t *pricebitsA,uint32_t *pricebitsB,int64_t tolerance)
{
    int32_t i; uint32_t newprice;
    for (i=1; i<n; i++)
    {
        if ( (newprice= chunguscoin_pricenew(&maxflags[i],pricebitsA[i],pricebitsB[i],tolerance)) != 0 )
        {
...
// chunguscoin_priceclamp() clamps any price that is beyond tolerance
int32_t chunguscoin_priceclamp(int32_t n,uint32_t *pricebits,uint32_t *refprices,int64_t tolerance)
{
    int32_t i; uint32_t newprice; char maxflags[CHUNGUSCOIN_MAXPRICES];
    memset(maxflags,0,sizeof(maxflags));
    for (i=1; i<n; i++)
    {
        if ( (newprice= chunguscoin_pricenew(&maxflags[i],pricebits[i],refprices[i],tolerance)) != 0 )
        {
...
// chunguscoin_mineropret() returns a valid pricedata to add to the coinbase opreturn for nHeight
CScript chunguscoin_mineropret(int32_t nHeight)
{
    CScript opret; char maxflags[CHUNGUSCOIN_MAXPRICES]; uint32_t pricebits[CHUNGUSCOIN_MAXPRICES],prevbits[CHUNGUSCOIN_MAXPRICES]; int32_t maxflag,i,n,numzero=0;
    if ( Mineropret.size() >= PRICES_SIZEBIT0 )
    {
        ...
        while ( numzero > 0 )
        {
            ...
            if ( numzero != 0 )
            {
                LogPrintf(" chunguscoin_mineropret numzero.%d vs n.%d\n",numzero,n);
                chunguscoin_cbopretupdate(1);
                sleep(61);
            }
        }
        if ( chunguscoin_heightpricebits(0,prevbits,nHeight-1) > 0 )
        {
            memcpy(pricebits,Mineropret.data(),Mineropret.size());
            memset(maxflags,0,sizeof(maxflags));
            if ( chunguscoin_pricecmp(0,n,maxflags,pricebits,prevbits,PRICES_ERRORRATE) < 0 )
            {
                // if the new prices are outside tolerance, update Mineropret with clamped prices
                chunguscoin_priceclamp(n,pricebits,prevbits,PRICES_ERRORRATE);
...
/*
 chunguscoin_opretvalidate() is the entire price validation!
 ...
 The only way chunguscoin_opretvalidate() doesnt return an error is if maxflag is set or it is within tolerance of both the prior block and the local data. The local data validation only happens if it is a recent block and not a block from the past as the local node is only getting the current price data.
 
 */


void chunguscoin_queuelocalprice(int32_t dir,int32_t height,uint32_t timestamp,uint256 blockhash,int32_t ind,uint32_t pricebits)
{
...
int32_t chunguscoin_opretvalidate(const CBlock *block,CBlockIndex * const previndex,int32_t nHeight,CScript scriptPubKey)
{
    int32_t testchain_exemption = 0;
    std::vector<uint8_t> vopret; char maxflags[CHUNGUSCOIN_MAXPRICES]; uint256 bhash; double btcusd,btcgbp,btceur; uint32_t localbits[CHUNGUSCOIN_MAXPRICES],pricebits[CHUNGUSCOIN_MAXPRICES],prevbits[CHUNGUSCOIN_MAXPRICES],newprice; int32_t i,j,prevtime,maxflag,lag,lag2,lag3,n,errflag,iter; uint32_t now;
        ...
        if ( vopret.size() >= PRICES_SIZEBIT0 )
        {
            ...
            if ( nHeight > 2 )
            {
                ...
                if ( chunguscoin_heightpricebits(0,prevbits,nHeight-1) > 0 )
                {
                    ...
                    if ( chunguscoin_pricecmp(nHeight,n,maxflags,pricebits,prevbits,PRICES_ERRORRATE) < 0 )
                    {
                        for (i=1; i<n; i++)
                        ...
                        for (i=1; i<n; i++)
                        {
                            if ( (maxflag= maxflags[i]) != 0 && localbits[i] != 0 )
                            {
                                ...
                                if ( maxflag > 0 && localbits[i] < prevbits[i] )
                                {
                                    ...
                                    for (j=0; j<CHUNGUSCOIN_LOCALPRICE_CACHESIZE; j++)
                                        if ( PriceCache[j][i] >= prevbits[i] )
                                        {
                                            LogPrintf("i.%d within recent localprices[%d] %u >= %u\n",i,j,PriceCache[j][i],prevbits[i]);
                                            break;
                                        }
                                    if ( j == CHUNGUSCOIN_LOCALPRICE_CACHESIZE )
                                    {
                                        chunguscoin_queuelocalprice(1,nHeight,block->nTime,bhash,i,prevbits[i]);
                                        break;
                                    }
                                }
...
// chunguscoin_cbopretupdate() obtains the external price data and encodes it into Mineropret, which will then be used by the miner and validation
...
void chunguscoin_cbopretupdate(int32_t forceflag)
{
    static uint32_t lasttime,lastbtc,pending;
    static uint32_t pricebits[4],pricebuf[CHUNGUSCOIN_MAXPRICES],forexprices[sizeof(Forex)/sizeof(*Forex)];
    ...
    if ( Queued_reconsiderblock != zeroid )
    {
        ...
        chunguscoin_reconsiderblock(Queued_reconsiderblock);
    ...
    if ( (ASSETCHAINS_CBOPRET & 1) != 0 )
    {
//if ( chunguscoin_nextheight() > 333 ) // for debug only!
//    ASSETCHAINS_CBOPRET = 7;
        size = chunguscoin_cbopretsize(ASSETCHAINS_CBOPRET);
        ...
        if ( flags != 0 )
        {
            ...
            if ( ExtremePrice.dir != 0 && ExtremePrice.ind > 0 && ExtremePrice.ind < size/sizeof(uint32_t) && now < ExtremePrice.timestamp+3600 )
            {
                ...
                if ( (ExtremePrice.dir > 0 && PriceCache[0][ExtremePrice.ind] >= ExtremePrice.pricebits) || (ExtremePrice.dir < 0 && PriceCache[0][ExtremePrice.ind] <= ExtremePrice.pricebits) )
                {
                    ...
                    if ( (pindex= chunguscoin_blockindex(ExtremePrice.blockhash)) != 0 )
...
int64_t chunguscoin_pricemult(int32_t ind)
{
    int32_t i,j;
    if ( (ASSETCHAINS_CBOPRET & 1) != 0 && ind < CHUNGUSCOIN_MAXPRICES )
    {
        if ( PriceMult[0] == 0 )
        {
            for (i=0; i<4; i++)
...
char *chunguscoin_pricename(char *name,int32_t ind)
{
    strcpy(name,"error");
    if ( (ASSETCHAINS_CBOPRET & 1) != 0 && ind < CHUNGUSCOIN_MAXPRICES )
    {
...
int32_t chunguscoin_priceind(const char *symbol)
{
    char name[65]; int32_t i,n = (int32_t)(chunguscoin_cbopretsize(ASSETCHAINS_CBOPRET) / sizeof(uint32_t));
    for (i=1; i<n; i++)
    {
        chunguscoin_pricename(name,i);
...
int64_t chunguscoin_pricecorrelated(uint64_t seed,int32_t ind,uint32_t *rawprices,int32_t rawskip,uint32_t *nonzprices,int32_t smoothwidth)
{
    int32_t i,j,k,n,iter,correlation,maxcorrelation=0; int64_t firstprice,price,sum,den,mult,refprice,lowprice,highprice;
    if ( PRICES_DAYWINDOW < 2 || ind >= CHUNGUSCOIN_MAXPRICES )
        return(-1);
    mult = chunguscoin_pricemult(ind);
    if ( nonzprices != 0 )
...
int64_t chunguscoin_priceave(int64_t *buf,int64_t *correlated,int32_t cskip)
{
...
int32_t chunguscoin_pricesinit()
{
    ...
    for (i=0; i<CHUNGUSCOIN_MAXPRICES; i++)
    {
        if ( chunguscoin_pricename(PRICES[i].symbol,i) == 0 )
            break;
    ...
    LogPrintf("pricesinit done i.%d num.%d numprices.%d\n",i,num,(int32_t)(chunguscoin_cbopretsize(ASSETCHAINS_CBOPRET)/sizeof(uint32_t)));
    if ( i != num || i != chunguscoin_cbopretsize(ASSETCHAINS_CBOPRET)/sizeof(uint32_t) )
    {
...
void chunguscoin_pricesupdate(int32_t height,CBlock *pblock)
{
    static int numprices; static uint32_t *ptr32; static int64_t *ptr64,*tmpbuf;
    int32_t ind,offset,width; int64_t correlated,smoothed; uint64_t seed,rngval; uint32_t rawprices[CHUNGUSCOIN_MAXPRICES],buf[PRICES_MAXDATAPOINTS*2];
    width = PRICES_DAYWINDOW;//(2*PRICES_DAYWINDOW + PRICES_SMOOTHWIDTH);
    if ( numprices == 0 )
    {
        pthread_mutex_init(&pricemutex,0);
        numprices = (int32_t)(chunguscoin_cbopretsize(ASSETCHAINS_CBOPRET) / sizeof(uint32_t));
    ...
    if ( _chunguscoin_heightpricebits(&seed,rawprices,pblock) == numprices )
    {
            ...
            if ( height > PRICES_DAYWINDOW )
            {
                ...
                if ( fread(ptr32,sizeof(uint32_t),width*numprices,PRICES[0].fp) == width*numprices )
                {
                    ...
                    for (ind=1; ind<numprices; ind++)
                    {
                        ...
                        if ( (correlated= chunguscoin_pricecorrelated(rngval,ind,&ptr32[offset],-numprices,0,PRICES_SMOOTHWIDTH)) > 0 )
                        {
                            ...
                            else if ( height > PRICES_DAYWINDOW*2 )
                            {
                                ...
                                {
                                    if ( (smoothed= chunguscoin_priceave(tmpbuf,&ptr64[(PRICES_DAYWINDOW-1)*PRICES_MAXDATAPOINTS+1],-PRICES_MAXDATAPOINTS)) > 0 )
                                    {
                        ...
                        } else LogPrintf("error chunguscoin_pricecorrelated for ht.%d ind.%d\n",height,ind);
                    }
                    LogPrintf("height.%d\n",height);
...
int32_t chunguscoin_priceget(int64_t *buf64,int32_t ind,int32_t height,int32_t numblocks)
{
    FILE *fp; int32_t retval = PRICES_MAXDATAPOINTS;
    pthread_mutex_lock(&pricemutex);
    if ( ind < CHUNGUSCOIN_MAXPRICES && (fp= PRICES[ind].fp) != 0 )
    {
