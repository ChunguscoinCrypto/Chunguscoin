...
#define CHUNGUSCOIN_ENDOFERA 7777777
#define CHUNGUSCOIN_INTEREST ((uint64_t)5000000) //((uint64_t)(0.05 * COIN))   // 5%
extern int64_t MAX_MONEY;
extern uint8_t NOTARY_PUBKEY33[];

#ifdef notanymore
uint64_t chunguscoin_earned_interest(int32_t height,int64_t paidinterest)
{
...
uint64_t chunguscoin_moneysupply(int32_t height)
{
    if ( height <= 1 || ASSETCHAINS_SYMBOL[0] == 0 )
        return(0);
    else return(COIN * 100000000 + (height-1) * 3 + chunguscoin_earned_interest(height,-1));
}
#endif

uint64_t _chunguscoin_interestnew(int32_t txheight,uint64_t nValue,uint32_t nLockTime,uint32_t tiptime)
{
    int32_t minutes; uint64_t interest = 0;
    if ( nLockTime >= LOCKTIME_THRESHOLD && tiptime > nLockTime && (minutes= (tiptime - nLockTime) / 60) >= (CHUNGUSCOIN_MAXMEMPOOLTIME/60) )
    {
        if ( minutes > 365 * 24 * 60 )
            minutes = 365 * 24 * 60;
        if ( txheight >= 1000000 && minutes > 31 * 24 * 60 )
            minutes = 31 * 24 * 60;
        minutes -= ((CHUNGUSCOIN_MAXMEMPOOLTIME/60) - 1);
        interest = ((nValue / 10512000) * minutes);
    }
    return(interest);
}

uint64_t chunguscoin_interestnew(int32_t txheight,uint64_t nValue,uint32_t nLockTime,uint32_t tiptime)
{
    uint64_t interest = 0;
    if ( txheight < CHUNGUSCOIN_ENDOFERA && nLockTime >= LOCKTIME_THRESHOLD && tiptime != 0 && nLockTime < tiptime && nValue >= 10*COIN ) //chunguscoin_moneysupply(txheight) < MAX_MONEY &&
        interest = _chunguscoin_interestnew(txheight,nValue,nLockTime,tiptime);
    return(interest);
}
...
uint64_t chunguscoin_interest(int32_t txheight,uint64_t nValue,uint32_t nLockTime,uint32_t tiptime)
{
    ...
    if ( ASSETCHAINS_SYMBOL[0] != 0 )
        return(0);
    if ( txheight >= CHUNGUSCOIN_ENDOFERA )
        return(0);
    if ( nLockTime >= LOCKTIME_THRESHOLD && tiptime != 0 && nLockTime < tiptime && nValue >= 10*COIN ) //chunguscoin_moneysupply(txheight) < MAX_MONEY && 
    {
        if ( (minutes= (tiptime - nLockTime) / 60) >= 60 )
        {
            ...
            if ( denominator == 0 )
                denominator = 1; // max CHUNGUSCOIN_INTEREST per transfer, do it at least annually!
            if ( nValue > 25000LL*COIN )
            {
                ...
                if ( exception == 0 )
                {
                    ...
                    else if ( txheight < 1000000 )
                    {
                        ...
                        interestnew = _chunguscoin_interestnew(txheight,nValue,nLockTime,tiptime);
                    ...
                    else interest = _chunguscoin_interestnew(txheight,nValue,nLockTime,tiptime);
                }
                else if ( txheight < 1000000 )
                {
                    numerator = (nValue * CHUNGUSCOIN_INTEREST);
