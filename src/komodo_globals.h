...
void chunguscoin_stateupdate(int32_t height,uint8_t notarypubs[][33],uint8_t numnotaries,uint8_t notaryid,uint256 txhash,uint64_t voutmask,uint8_t numvouts,uint32_t *pvals,uint8_t numpvals,int32_t kheight,uint32_t ktime,uint64_t opretvalue,uint8_t *opretbuf,uint16_t opretlen,uint16_t vout,uint256 MoM,int32_t MoMdepth);
void chunguscoin_init(int32_t height);
int32_t chunguscoin_MoMdata(int32_t *notarized_htp,uint256 *MoMp,uint256 *kmdtxidp,int32_t nHeight,uint256 *MoMoMp,int32_t *MoMoMoffsetp,int32_t *MoMoMdepthp,int32_t *kmdstartip,int32_t *kmdendip);
...
pthread_mutex_t chunguscoin_mutex,staked_mutex;

#define CHUNGUSCOIN_ELECTION_GAP 2000    //((ASSETCHAINS_SYMBOL[0] == 0) ? 2000 : 100)
...
struct chunguscoin_state CHUNGUSCOIN_STATES[34];
...
uint256 CHUNGUSCOIN_EARLYTXID;

int32_t CHUNGUSCOIN_MININGTHREADS = -1,IS_CHUNGUSCOIN_NOTARY,IS_STAKED_NOTARY,USE_EXTERNAL_PUBKEY,CHUNGUSCOIN_CHOSEN_ONE,ASSETCHAINS_SEED,CHUNGUSCOIN_ON_DEMAND,CHUNGUSCOIN_EXTERNAL_NOTARIES,CHUNGUSCOIN_PASSPORT_INITDONE,CHUNGUSCOIN_PAX,CHUNGUSCOIN_EXCHANGEWALLET,CHUNGUSCOIN_REWIND,STAKED_ERA,CHUNGUSCOIN_CONNECTING = -1,CHUNGUSCOIN_DEALERNODE,CHUNGUSCOIN_EXTRASATOSHI,ASSETCHAINS_FOUNDERS,ASSETCHAINS_CBMATURITY,CHUNGUSCOIN_NSPV;
int32_t CHUNGUSCOIN_INSYNC,CHUNGUSCOIN_LASTMINED,prevCHUNGUSCOIN_LASTMINED,CHUNGUSCOIN_CCACTIVATE,JUMBLR_PAUSE = 1;
...
char ASSETCHAINS_SYMBOL[CHUNGUSCOIN_ASSETCHAIN_MAXLEN],ASSETCHAINS_USERPASS[4096];
uint16_t ASSETCHAINS_P2PPORT,ASSETCHAINS_RPCPORT,ASSETCHAINS_BEAMPORT,ASSETCHAINS_CODAPORT;
uint32_t ASSETCHAIN_INIT,ASSETCHAINS_CC,CHUNGUSCOIN_STOPAT,CHUNGUSCOIN_DPOWCONFS = 1,STAKING_MIN_DIFF;
...
uint64_t CHUNGUSCOIN_INTERESTSUM,CHUNGUSCOIN_WALLETBALANCE;
int32_t ASSETCHAINS_STAKED;
uint64_t ASSETCHAINS_COMMISSION,ASSETCHAINS_SUPPLY = 10,ASSETCHAINS_FOUNDERS_REWARD;

uint32_t CHUNGUSCOIN_INITDONE;
char KMDUSERPASS[8192+512+1],BTCUSERPASS[8192]; uint16_t KMD_PORT = 7771,BITCOIND_RPCPORT = 7771, DEST_PORT;
uint64_t PENDING_CHUNGUSCOIN_TX;
extern int32_t CHUNGUSCOIN_LOADINGBLOCKS;
unsigned int MAX_BLOCK_SIGOPS = 20000;

int32_t CHUNGUSCOIN_TESTNODE, CHUNGUSCOIN_SNAPSHOT_INTERVAL; 
CScript CHUNGUSCOIN_EARLYTXID_SCRIPTPUB;
...
struct chunguscoin_kv *CHUNGUSCOIN_KV;
pthread_mutex_t CHUNGUSCOIN_KV_mutex,CHUNGUSCOIN_CC_mutex;
...
#endif
uint64_t chunguscoin_current_supply(uint32_t nHeight)
{
    uint64_t cur_money;
    int32_t baseid;
    ...
    if ( CHUNGUSCOIN_BIT63SET(cur_money) != 0 )
        return(CHUNGUSCOIN_MAXNVALUE);
    if ( ASSETCHAINS_COMMISSION != 0 )
    {
        uint64_t newval = (cur_money + (cur_money/COIN * ASSETCHAINS_COMMISSION));
        if ( CHUNGUSCOIN_BIT63SET(newval) != 0 )
            return(CHUNGUSCOIN_MAXNVALUE);
