...
bool DecrementNoteWitnesses(NoteDataMap& noteDataMap, int indexHeight, int64_t nWitnessCacheSize)
{
    ...
    assert(CHUNGUSCOIN_REWIND != 0 || nWitnessCacheSize > 0 || WITNESS_CACHE_SIZE != _COINBASE_MATURITY+10);
    return true;
}
...
bool CWallet::VerusSelectStakeOutput(CBlock *pBlock, arith_uint256 &hashResult, CTransaction &stakeSource, int32_t &voutNum, int32_t nHeight, uint32_t &bnTarget) const
{
    ...
    if (pastBlockIndex = chunguscoin_chainactive(nHeight - 100))
    {
        ...
        if (mi != mapWallet.end())
        {
            ...
            if (txin.prevout.n < prev.vout.size())
                if (::IsMine(*this, prev.vout[txin.prevout.n].scriptPubKey) & filter)
                    return prev.vout[txin.prevout.n].nValue; // chunguscoin_interest?
        }
    }
    return 0;
}
...
std::vector<uint256> CWallet::ResendWalletTransactionsBefore(int64_t nTime)
{
    ...
    BOOST_FOREACH(PAIRTYPE(const uint256, CWalletTx)& item, mapWallet)
    {
        ...
        if ( (wtx.nLockTime >= LOCKTIME_THRESHOLD && wtx.nLockTime < now-CHUNGUSCOIN_MAXMEMPOOLTIME) )
        {
...

    {
        ...
        for (map<uint256, CWalletTx>::const_iterator it = mapWallet.begin(); it != mapWallet.end(); ++it)
        {
            ...
            for (int i = 0; i < pcoin->vout.size(); i++)
            {
                ...
                {
                    if ( CHUNGUSCOIN_EXCHANGEWALLET == 0 )
                    {
                        ...
                        if ( ASSETCHAINS_SYMBOL[0] == 0 && chainActive.LastTip() != 0 && chainActive.LastTip()->GetHeight() >= 60000 )
                        {
                            if ( pcoin->vout[i].nValue >= 10*COIN )
                            {
                                ...
                                {
                                    chunguscoin_accrued_interest(&txheight,&locktime,wtxid,i,0,pcoin->vout[i].nValue,(int32_t)tipindex->GetHeight());
                                    interest = chunguscoin_interestnew(txheight,pcoin->vout[i].nValue,locktime,tipindex->nTime);
                                } else interest = 0;
                                //interest = chunguscoin_interestnew(chainActive.LastTip()->GetHeight()+1,pcoin->vout[i].nValue,pcoin->nLockTime,chainActive.LastTip()->nTime);
                                if ( interest != 0 )
                                {
...
bool CWallet::SelectCoinsMinConf(const CAmount& nTargetValue, int nConfMine, int nConfTheirs, vector<COutput> vCoins,set<pair<const CWalletTx*,unsigned int> >& setCoinsRet, CAmount& nValueRet) const
{
    ...
    BOOST_FOREACH(const COutput &output, vCoins)
    {
        ...
        else if (n < nTargetValue + CENT)
        {
            ...
            nTotalLower += n;
            //if ( CHUNGUSCOIN_EXCHANGEWALLET == 0 && count < sizeof(interests)/sizeof(*interests) )
            //{
    ...
    if (nTotalLower == nTargetValue)
    {
        for (unsigned int i = 0; i < vValue.size(); ++i)
        {
            ...
            //if ( CHUNGUSCOIN_EXCHANGEWALLET == 0 && i < count )
                                ...
                                int& nChangePosRet, std::string& strFailReason, const CCoinControl* coinControl, bool sign)
{
    ...
    if ( !chunguscoin_hardfork_active((uint32_t)chainActive.LastTip()->nTime) )
        ...
        LOCK2(cs_main, cs_wallet);
        {
            ...
            while (true)
            {
                ...
                BOOST_FOREACH(PAIRTYPE(const CWalletTx*, unsigned int) pcoin, setCoins)
                {
                    ...
                    if ( CHUNGUSCOIN_EXCHANGEWALLET == 0 && ASSETCHAINS_SYMBOL[0] == 0 )
                    {
                ...
                //if ( CHUNGUSCOIN_EXCHANGEWALLET != 0 )
                //{
                    //LogPrintf("CHUNGUSCOIN_EXCHANGEWALLET disable interest sum %.8f, interest2 %.8f\n",(double)interest/COIN,(double)interest2/COIN);
