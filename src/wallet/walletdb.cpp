...
extern CBlockIndex *chunguscoin_blockindex(uint256 hash);
...
DBErrors CWalletDB::LoadWallet(CWallet* pwallet)
{
    ...
    if ( !deadTxns.empty() )
    {
        ...
        BOOST_FOREACH (uint256& hash, deadTxns) 
        {
            ...
            if ( GetTransaction(hash,tx,blockhash,false) && (pindex= chunguscoin_blockindex(blockhash)) != 0 && chainActive.Contains(pindex) )
            {
