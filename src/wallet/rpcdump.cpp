...
UniValue convertpassphrase(const UniValue& params, bool fHelp, const CPubKey& mypk)
{
    if (fHelp || params.size() < 1 || params.size() > 1)
        throw runtime_error(
            ...
            "\"address\": \"chunguscoinaddress\",             (string) Address corresponding to your passphrase\n"
...
UniValue importprivkey(const UniValue& params, bool fHelp, const CPubKey& mypk)
{
    ...
    if (fHelp || params.size() < 1 || params.size() > 5)
        throw runtime_error(
            "importprivkey \"chunguscoinprivkey\" ( \"label\" rescan height secret_key)\n"
            "\nAdds a private key (as returned by dumpprivkey) to your wallet.\n"
            "\nArguments:\n"
            "1. \"chunguscoinprivkey\"   (string, required) The private key (see dumpprivkey)\n"
...
UniValue z_exportwallet(const UniValue& params, bool fHelp, const CPubKey& mypk)
{
    ...
    if (fHelp || params.size() != 1)
        throw runtime_error(
            ...
            "\nArguments:\n"
            "1. \"filename\"    (string, required) The filename, saved in folder set by chunguscoind -exportdir option\n"
            "\nResult:\n"
...
UniValue dumpwallet_impl(const UniValue& params, bool fHelp, bool fDumpZKeys)
{
    ...
    if (exportdir.empty()) {
        throw JSONRPCError(RPC_WALLET_ERROR, "Cannot export wallet until the chunguscoind -exportdir option has been set");
    }
    ...
    // produce output
    file << strprintf("# Wallet dump created by Chunguscoin %s (%s)\n", CLIENT_BUILD, CLIENT_DATE);
...
extern int32_t CHUNGUSCOIN_NSPV;
