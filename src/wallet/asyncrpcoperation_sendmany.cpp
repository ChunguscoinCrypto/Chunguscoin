...
bool AsyncRPCOperation_sendmany::main_impl() {
    ...
    if (isfromtaddr_) {
        ...
        if (isUsingBuilder_) {
            ...
            // for Chunguscoin, set lock time to accure interest, for other chains, set
            ...
            if (ASSETCHAINS_SYMBOL[0] == 0)
            {
                ...
                if ( !chunguscoin_hardfork_active((uint32_t)chainActive.LastTip()->nTime) )
                    builder_.SetLockTime((uint32_t)time(NULL) - 60); // set lock time for Chunguscoin interest
                else
    ...
    while (!vpubNewProcessed) {
        ...
        while (numInputsNeeded++ < ZC_NUM_JS_INPUTS && zInputsDeque.size() > 0) {
            ...
            int wtxDepth = -1;
            {
                ...
                wtxHeight = chunguscoin_blockheight(wtx.hashBlock);
...
bool AsyncRPCOperation_sendmany::find_utxos(bool fAcceptCoinbase=false) {
    ...
    BOOST_FOREACH(const COutput& out, vecOutputs) {
        ...
        if( mindepth_ > 1 ) {
            ...
            int dpowconfs  = chunguscoin_dpowconfs(nHeight, out.nDepth);
