...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    else return GetSpecialFolderPath(CSIDL_APPDATA) / "Chunguscoin" / symbol;
#else
    fs::path pathRet;
    ...
    if ( symbol[0] == 0 )
        return pathRet / ".chunguscoin";
    else return pathRet / ".chunguscoin" / symbol;
#endif
#endif
}
...
boost::filesystem::path GetConfigFile()
{
    ...
    {
#ifdef __APPLE__
        strcpy(confname,"Chunguscoin.conf");
#else
        strcpy(confname,"chunguscoin.conf");
#endif
    }
                    ...
                    map<string, vector<string> >& mapMultiSettingsRet)
{
    ...
    {
        // Don't overwrite existing settings so command line settings override chunguscoin.conf
...
{
    boost::filesystem::path pathPidFile(GetArg("-pid", "chunguscoind.pid"));
...
std::string LicenseInfo()
{
    ...
    const std::string URL_SOURCE_CODE = "<https://github.com/ChunguscoinPlatform/Chunguscoin>";
...
std::string CopyrightHolders(const std::string& strPrefix)
{
    ...
    // Check for untranslated substitution to make sure Chunguscoin Core copyright is not removed by accident
    if (strprintf(COPYRIGHT_HOLDERS, COPYRIGHT_HOLDERS_SUBSTITUTION).find("Chunguscoin Core") == std::string::npos) {
        strCopyrightHolders += "\n" + strPrefix + "The Chunguscoin Core developers";
    }
