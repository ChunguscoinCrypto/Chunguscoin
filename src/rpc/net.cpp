...
int32_t chunguscoin_longestchain()
{
    ...
    if ( depth == 0 )
    {
        ...
        BOOST_FOREACH(const CNodeStats& stats, vstats)
        {
            //fprintf(stderr,"chunguscoin_longestchain iter.%d\n",n);
            CNodeStateStats statestats;
            bool fStateStats = GetNodeStateStats(stats.nodeid,statestats);
        ...
        if ( num > (n >> 1) )
        {
            if ( 0 && height != CHUNGUSCOIN_LONGESTCHAIN )
                fprintf(stderr,"set %s CHUNGUSCOIN_LONGESTCHAIN <- %d\n",ASSETCHAINS_SYMBOL,height);
            CHUNGUSCOIN_LONGESTCHAIN = height;
            return(height);
        }
        CHUNGUSCOIN_LONGESTCHAIN = 0;
    }
    return(CHUNGUSCOIN_LONGESTCHAIN);
}
...
UniValue getaddednodeinfo(const UniValue& params, bool fHelp, const CPubKey& mypk)
{
    if (fHelp || params.size() < 1 || params.size() > 2)
        throw runtime_error(
            ...
            "    \"addresses\" : [\n"
            "       {\n"
            "         \"address\" : \"192.168.0.201:8233\",  (string) The Chunguscoin server host and port\n"
