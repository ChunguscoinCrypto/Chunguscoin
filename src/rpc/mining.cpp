...
UniValue getgenerate(const UniValue& params, bool fHelp, const CPubKey& mypk)
{
    if (fHelp || params.size() != 0)
        throw runtime_error(
            ...
            "It is set with the command line argument -gen (or chunguscoin.conf setting gen) and -mint\n"
    ...
    obj.push_back(Pair("numthreads",       (int64_t)CHUNGUSCOIN_MININGTHREADS));
    return obj;
}
...
#else
        throw JSONRPCError(RPC_METHOD_NOT_FOUND, "chunguscoind compiled without wallet and -mineraddress not set");
#endif
    }
...
#ifdef ENABLE_WALLET
        std::unique_ptr<CBlockTemplate> pblocktemplate(CreateNewBlockWithKey(reservekey,nHeight,CHUNGUSCOIN_MAXGPUCOUNT));
#else
...
UniValue setgenerate(const UniValue& params, bool fHelp, const CPubKey& mypk)
{
    ...
    if ( ASSETCHAINS_LWMAPOS != 0 )
    {
        if (fGenerate && !nGenProcLimit)
        {
            ...
            CHUNGUSCOIN_MININGTHREADS = nGenProcLimit;
        }
        else if (!fGenerate)
        {
            VERUS_MINTBLOCKS = 0;
            CHUNGUSCOIN_MININGTHREADS = 0;
        }
        else CHUNGUSCOIN_MININGTHREADS = (int32_t)nGenProcLimit;
    }
    else
    {
        CHUNGUSCOIN_MININGTHREADS = (int32_t)nGenProcLimit;
    }
...
UniValue genminingCSV(const UniValue& params, bool fHelp, const CPubKey& mypk)
{
    ...
    if ( (fp= fopen(fname,"wb")) != 0 )
    {
        ...
        height = chunguscoin_nextheight();
        for (i=0; i<height; i++)
        {
            if ( (pindex= chunguscoin_chainactive(i)) != 0 )
            {
...
//            "  \"coinbasevalue\" : n,               (numeric) maximum allowable input to coinbase transaction, including the generation award and transaction fees (in Satoshis)\n"
        ...
        (mempool.GetTransactionsUpdated() != nTransactionsUpdatedLast && GetTime() - nStart > 5))
    {
        ...
        CReserveKey reservekey(pwalletMain);
        LEAVE_CRITICAL_SECTION(cs_main);
        pblocktemplate = CreateNewBlockWithKey(reservekey,pindexPrevNew->GetHeight()+1,CHUNGUSCOIN_MAXGPUCOUNT,false);
#else
    ...
    if ( ASSETCHAINS_STAKED != 0 )
    {
        ...
        POWtarget = chunguscoin_PoWtarget(&PoSperc,hashTarget,(int32_t)(pindexPrev->GetHeight()+1),ASSETCHAINS_STAKED,chunguscoin_newStakerActive(chainActive.Height()+1, pblock->nTime));
    ...
    /*else if ( ASSETCHAINS_ADAPTIVEPOW > 0 )
        result.push_back(Pair("target",chunguscoin_adaptivepow_target((int32_t)(pindexPrev->GetHeight()+1),hashTarget,pblock->nTime).GetHex()));*/
    else
    ...
    if ( strlen(ASSETCHAINS_OVERRIDE_PUBKEY.c_str()) == 66 || ASSETCHAINS_SCRIPTPUB.size() > 1 )
    {
        if ( ASSETCHAINS_FOUNDERS == 0 && ASSETCHAINS_COMMISSION != 0 )
        {
            ...
            if ( nHeight <= chainActive.Height() )
            {
                ...
                CBlock block;
                if ( chunguscoin_blockload(block, pblockIndex) == 0 )
                    nFoundersReward = chunguscoin_commission(&block, nHeight);
            }
        }
        else if ( ASSETCHAINS_FOUNDERS != 0 )
        {
            // Assetchains founders chains have a fixed reward so can be calculated at any given height.
            nFoundersReward = chunguscoin_commission(0, nHeight);
        }
