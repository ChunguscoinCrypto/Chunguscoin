...
int32_t chunguscoin_MoM(int32_t *notarized_htp,uint256 *MoMp,uint256 *kmdtxidp,int32_t nHeight,uint256 *MoMoMp,int32_t *MoMoMoffsetp,int32_t *MoMoMdepthp,int32_t *kmdstartip,int32_t *kmdendip);
...
UniValue height_MoM(const UniValue& params, bool fHelp, const CPubKey& mypk)
{
    ...
    depth = chunguscoin_MoM(&notarized_height,&MoM,&kmdtxid,height,&MoMoM,&MoMoMoffset,&MoMoMdepth,&kmdstarti,&kmdendi);
...
UniValue calc_MoM(const UniValue& params, bool fHelp, const CPubKey& mypk)
{
    ...
    //LogPrintf("height_MoM height.%d\n",height);
    MoM = chunguscoin_calcMoM(height,MoMdepth);
    ret.push_back(Pair("coin",(char *)(ASSETCHAINS_SYMBOL[0] == 0 ? "KMD" : ASSETCHAINS_SYMBOL)));
    ret.push_back(Pair("height",height));
...
UniValue migrate_converttoexport(const UniValue& params, bool fHelp, const CPubKey& mypk)
{
    ...
    if (ASSETCHAINS_CC < CHUNGUSCOIN_FIRSTFUNGIBLEID)
        throw runtime_error("-ac_cc < CHUNGUSCOIN_FIRSTFUNGIBLEID");
...
UniValue migrate_createnotaryapprovaltransaction(const UniValue& params, bool fHelp, const CPubKey& mypk)
{
    if (fHelp || params.size() != 2)
        throw runtime_error("migrate_createnotaryapprovaltransaction burntxid txoutproof\n\n"
            ...
            "txoutproof should be retrieved by chunguscoin-cli migrate_checkburntransactionsource call on the source chain\n" );
...
UniValue getNotarisationsForBlock(const UniValue& params, bool fHelp, const CPubKey& mypk)
{
    ...
    numNN = chunguscoin_notaries(notarypubkeys, height, chainActive[height]->nTime);
