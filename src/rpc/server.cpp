...
UniValue stop(const UniValue& params, bool fHelp, const CPubKey& mypk)
{
   ...
   // Accept the deprecated and ignored 'detach' boolean argument
    ...
    StartShutdown();
    sprintf(buf,"%s server stopping",ASSETCHAINS_SYMBOL[0] != 0 ? ASSETCHAINS_SYMBOL : "Chunguscoin");
    return buf;
}
...
std::string HelpExampleCli(const std::string& methodname, const std::string& args)
{
    if ( ASSETCHAINS_SYMBOL[0] == 0 ) {
        ...
        return "> chunguscoin-cli -ac_name=" + strprintf("%s", ASSETCHAINS_SYMBOL) + " " + methodname + " " + args + "\n";
    }
}
...
{
    string daemon = ASSETCHAINS_SYMBOL[0] == 0 ? "chunguscoind" : "hushd";
    string ticker = ASSETCHAINS_SYMBOL[0] == 0 ? "chunguscoin" : ASSETCHAINS_SYMBOL;
