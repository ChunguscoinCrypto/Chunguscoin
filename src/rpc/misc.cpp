...
extern int32_t CHUNGUSCOIN_LASTMINED,JUMBLR_PAUSE,CHUNGUSCOIN_LONGESTCHAIN,IS_STAKED_NOTARY,IS_CHUNGUSCOIN_NOTARY,STAKED_ERA,CHUNGUSCOIN_INSYNC;
...
#define CHUNGUSCOIN_VERSION "0.7.0"
...
UniValue getiguanajson(const UniValue& params, bool fHelp, const CPubKey& mypk)
{
    ...
    // get the min sigs .. this always rounds UP so min sigs in iguana is +1 min sigs in chunguscoind, due to some rounding error.
    int minsigs;
...
UniValue getinfo(const UniValue& params, bool fHelp, const CPubKey& mypk)
{
    ...
    if (fHelp || params.size() != 0)
        throw runtime_error(
            ...
            "  \"balance\": xxxxxxx,         (numeric) the total Chunguscoin balance of the wallet\n"
...
//#endif
    ...
    obj.push_back(Pair("KMDversion", CHUNGUSCOIN_VERSION));
    obj.push_back(Pair("synced", CHUNGUSCOIN_INSYNC!=0));
    ...
    if ( CHUNGUSCOIN_NSPV_FULLNODE )
    {
        ...
        if (pwalletMain) {
            ...
            if ( ASSETCHAINS_SYMBOL[0] == 0 )
            {
                obj.push_back(Pair("interest",       ValueFromAmount(CHUNGUSCOIN_INTERESTSUM)));
                obj.push_back(Pair("balance",       ValueFromAmount(CHUNGUSCOIN_WALLETBALANCE))); //pwalletMain->GetBalance()
            }
            else
            {
        ...
        if ( (longestchain= CHUNGUSCOIN_LONGESTCHAIN) != 0 && chainActive.Height() > longestchain )
    ...
    obj.push_back(Pair("errors",        GetWarnings("statusbar")));
     if ( NOTARY_PUBKEY33[0] != 0 ) {
        ...
        } else if( (notaryid= chunguscoin_whoami(pubkeystr,(int32_t)chainActive.LastTip()->GetHeight(),chunguscoin_chainactive_timestamp())) >= 0 )  {
            obj.push_back(Pair("notaryid",        notaryid));
            if ( CHUNGUSCOIN_LASTMINED != 0 )
                obj.push_back(Pair("lastmined", CHUNGUSCOIN_LASTMINED));
        }
...
UniValue coinsupply(const UniValue& params, bool fHelp, const CPubKey& mypk)
{
    ...
    if (height >= 0 && height <= currentHeight) {
        if ( (supply= chunguscoin_coinsupply(&zfunds,&sproutfunds,height)) > 0 )
        {
            ...
            if ( ASSETCHAINS_BLOCKTIME > 0 )
            {
                ...
                {
                    supply1 = chunguscoin_coinsupply(&zf1,&sf1,height - blocks_per_year/12);
                    supply3 = chunguscoin_coinsupply(&zf3,&sf3,height - blocks_per_year/4);
                    supply12 = chunguscoin_coinsupply(&zf12,&sf12,height - blocks_per_year);
...
UniValue validateaddress(const UniValue& params, bool fHelp, const CPubKey& mypk)
{
    if (fHelp || params.size() != 1)
        throw runtime_error(
            "validateaddress \"chunguscoinaddress\"\n"
            "\nReturn information about the given Chunguscoin address.\n"
            "\nArguments:\n"
            "1. \"chunguscoinaddress\"     (string, required) The Chunguscoin address to validate\n"
            "\nResult:\n"
            "{\n"
            "  \"isvalid\" : true|false,         (boolean) If the address is valid or not. If not, this is the only property returned.\n"
            "  \"address\" : \"chunguscoinaddress\",   (string) The Chunguscoin address validated\n"
    ...
    if (isValid)
    {
        ...
        ret.push_back(Pair("segid", (int32_t)chunguscoin_segid32((char *)params[0].get_str().c_str()) & 0x3f));
#ifdef ENABLE_WALLET
...
UniValue createmultisig(const UniValue& params, bool fHelp, const CPubKey& mypk)
{
    if (fHelp || params.size() < 2 || params.size() > 2)
    {
        string msg = "createmultisig nrequired [\"key\",...]\n"
            ...
            "2. \"keys\"       (string, required) A json array of keys which are Chunguscoin addresses or hex-encoded public keys\n"
            "     [\n"
            "       \"key\"    (string) Chunguscoin address or hex-encoded public key\n"
            "       ,...\n"
            "     ]\n"
...
UniValue verifymessage(const UniValue& params, bool fHelp, const CPubKey& mypk)
{
    if (fHelp || params.size() != 3)
        throw runtime_error(
            "verifymessage \"chunguscoinaddress\" \"signature\" \"message\"\n"
            "\nVerify a signed message\n"
            "\nArguments:\n"
            "1. \"chunguscoinaddress\"    (string, required) The Chunguscoin address to use for the signature.\n"
...
UniValue getnotarypayinfo(const UniValue& params, bool fHelp, const CPubKey& mypk)
{
    ...
    notarycount = chunguscoin_notaries(notarypubkeys, height, chainActive[height]->GetBlockTime());
    NotaryPay = chunguscoin_notarypayamount(height, notarycount)*notarycount;
...
UniValue getsnapshot(const UniValue& params, bool fHelp, const CPubKey& mypk)
{
    ...
    result = chunguscoin_snapshot(top);
    if ( result.size() > 0 ) {
...
UniValue txnotarizedconfirmed(const UniValue& params, bool fHelp, const CPubKey& mypk)
{
    ...
    notarizedconfirmed=chunguscoin_txnotarizedconfirmed(txid);
    UniValue result(UniValue::VOBJ);
