...
void TxToJSONExpanded(const CTransaction& tx, const uint256 hashBlock, UniValue& entry, int nHeight = 0, int nConfirmations = 0, int nBlockTime = 0)
{
    ...
    for (unsigned int i = 0; i < tx.vout.size(); i++)
    {
        ...
        if ( ASSETCHAINS_SYMBOL[0] == 0 && pindex != 0 && tx.nLockTime >= 500000000 && (tipindex= chainActive.LastTip()) != 0 )
        {
            ...
            interest = chunguscoin_accrued_interest(&txheight,&locktime,tx.GetHash(),i,0,txout.nValue,(int32_t)tipindex->GetHeight());
    ...
    if (!hashBlock.IsNull()) {
        ...
        if (nConfirmations > 0) {
            ...
            entry.push_back(Pair("confirmations", chunguscoin_dpowconfs(nHeight,nConfirmations)));
...
void TxToJSON(const CTransaction& tx, const uint256 hashBlock, UniValue& entry)
{
    ...
    for (unsigned int i = 0; i < tx.vout.size(); i++) {
        ...
        UniValue out(UniValue::VOBJ);
        out.push_back(Pair("value", ValueFromAmount(txout.nValue)));
        if ( CHUNGUSCOIN_NSPV_FULLNODE && ASSETCHAINS_SYMBOL[0] == 0 && tx.nLockTime >= 500000000 && (tipindex= chainActive.LastTip()) != 0 )
        {
            int64_t interest; int32_t txheight; uint32_t locktime;
            interest = chunguscoin_accrued_interest(&txheight,&locktime,tx.GetHash(),i,0,txout.nValue,(int32_t)tipindex->GetHeight());
            out.push_back(Pair("interest", ValueFromAmount(interest)));
    ...
    if (!hashBlock.IsNull()) {
        ...
        if (mi != mapBlockIndex.end() && (*mi).second) {
            ...
            if (chainActive.Contains(pindex)) {
                ...
                entry.push_back(Pair("confirmations", chunguscoin_dpowconfs(pindex->GetHeight(),1 + chainActive.Height() - pindex->GetHeight())));
...
UniValue createrawtransaction(const UniValue& params, bool fHelp, const CPubKey& mypk)
{
    ...
    if (fHelp || params.size() < 2 || params.size() > 4)
        throw runtime_error(
            ...
            "    {\n"
            "      \"address\": x.xxx,    (numeric or string, required) The key is the chunguscoin address or script (in hex), the numeric value (can be string) is the " + CURRENCY_UNIT + " amount\n"
    ...
    for (size_t idx = 0; idx < sendTo.size(); idx++) {
        ...
        if (name_ == "data") {
            ...
            if (IsValidDestination(destination)) {
                ...
                throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, std::string("Invalid Chunguscoin address or script: ") + name_);
            }

            if (!(fExperimentalMode && IS_CHUNGUSCOIN_NOTARY)) {
...
UniValue decoderawtransaction(const UniValue& params, bool fHelp, const CPubKey& mypk)
{
    if (fHelp || params.size() != 1)
        throw runtime_error(
            ...
            "         \"type\" : \"pubkeyhash\",  (string) The type, eg 'pubkeyhash'\n"
            "         \"addresses\" : [           (json array of string)\n"
            "           \"RTZMZHDFSTFQst8XmX2dR4DaH87cEUs3gC\"   (string) chunguscoin address\n"
            "           ,...\n"
            "         ]\n"
            "       }\n"
            "     }\n"
