...
UniValue blockheaderToJSON(const CBlockIndex* blockindex)
{
    ...
    result.push_back(Pair("confirmations", chunguscoin_dpowconfs(blockindex->GetHeight(),confirmations)));
    ...
    result.push_back(Pair("segid", (int)chunguscoin_segid(0,blockindex->GetHeight())));
...
UniValue getlastsegidstakes(const UniValue& params, bool fHelp, const CPubKey& mypk)
{
    ...
    {
        int8_t segid = chunguscoin_segid(0,i);
...
/*uint256 _chunguscoin_getblockhash(int32_t nHeight)
{
    uint256 hash;
    LOCK(cs_main);
    if ( nHeight >= 0 && nHeight <= chainActive.Height() )
...
UniValue kvsearch(const UniValue& params, bool fHelp, const CPubKey& mypk)
{
    ...
    if ( (keylen= (int32_t)strlen(params[0].get_str().c_str())) > 0 )
    {
        ...
        if ( keylen < sizeof(key) )
        {
            ...
            if ( (valuesize= chunguscoin_kvsearch(&refpubkey,chainActive.LastTip()->GetHeight(),&flags,&height,value,key,keylen)) >= 0 )
            {
                ...
                duration = ((flags >> 2) + 1) * CHUNGUSCOIN_KVDURATION;
...
UniValue minerids(const UniValue& params, bool fHelp, const CPubKey& mypk)
{
    ...
    if ( (n= chunguscoin_minerids(minerids,height,(int32_t)(sizeof(minerids)/sizeof(*minerids)))) > 0 )
    {
        memset(tally,0,sizeof(tally));
        numnotaries = chunguscoin_notaries(pubkeys,height,timestamp);
        if ( numnotaries > 0 )
        {
            for (i=0; i<n; i++)
            {
...
UniValue notaries(const UniValue& params, bool fHelp, const CPubKey& mypk)
{
    ...
    if ( (n= chunguscoin_notaries(pubkeys,height,timestamp)) > 0 )
    {
        for (i=0; i<n; i++)
        {
            UniValue item(UniValue::VOBJ);
...
int32_t chunguscoin_pending_withdraws(char *opretstr);
...
UniValue paxpending(const UniValue& params, bool fHelp, const CPubKey& mypk)
{
    ...
    LOCK(cs_main);
    if ( (opretlen= chunguscoin_pending_withdraws(opretbuf)) > 0 )
...
UniValue paxprice(const UniValue& params, bool fHelp, const CPubKey& mypk)
{
    ...
    //if ( params.size() == 3 || (basevolume= COIN * atof(params[3].get_str().c_str())) == 0 )
        basevolume = 100000;
    relvolume = chunguscoin_paxprice(&seed,height,(char *)base.c_str(),(char *)rel.c_str(),basevolume);
...
/*int32_t prices_extract(int64_t *pricedata,int32_t firstheight,int32_t numblocks,int32_t ind)
{
    ...
    chunguscoin_heightpricebits(&seed,rawprices,firstheight + numblocks - 1);
    if ( firstheight < width )
        return(-1);
    for (i=0; i<width; i++)
    {
        if ( (n= chunguscoin_heightpricebits(&ignore,rawprices,firstheight + numblocks - 1 - i)) < 0 )  // stores raw prices in backward order 
            return(-1);
        if ( numpricefeeds < 0 )
            numpricefeeds = n;
        if ( n != numpricefeeds )
    ...
    for (i=0; i<numblocks+PRICES_DAYWINDOW+PRICES_SMOOTHWIDTH; i++) // calculates +PRICES_DAYWINDOW more correlated values
    {
        ...
        if ( (pricedata[i*3+1]= chunguscoin_pricecorrelated(rngval,ind,(uint32_t *)&pricedata[i*3],6,0,PRICES_SMOOTHWIDTH)) < 0 ) // skip is 6 == sizeof(int64_t)/sizeof(int32_t)*3 
            return(-3);
    }
    ...
    for (i=0; i<numblocks; i++)
        ...
        pricedata[i*3+2] = chunguscoin_priceave(tmpbuf,&pricedata[i*3+1],3); 
    free(tmpbuf);
    return(0);
}*/
...
UniValue prices(const UniValue& params, bool fHelp, const CPubKey& mypk)
{
    ...
    if ( maxsamples < 1 )
        maxsamples = 1;
    nextheight = chunguscoin_nextheight();
    UniValue a(UniValue::VARR);
    ...
    numpricefeeds = chunguscoin_heightpricebits(&seed,rawprices,nextheight-1);
    if ( numpricefeeds <= 0 )
    ...
    for (ht=nextheight-1,i=0; i<width&&ht>2; i++,ht--)
    {
        ...
        {
            if ( (n= chunguscoin_heightpricebits(0,rawprices,ht)) > 0 )
            {
                if ( n != numpricefeeds )
            ...
            } else throw JSONRPCError(RPC_INVALID_PARAMETER, "no chunguscoin_rawprices found");
        }
    }
    numsamples = i;
    ...
    for (j=1; j<numpricefeeds; j++)
    {
        ...
        if ( (str= chunguscoin_pricename(name,j)) != 0 )
        {
            ...
            if ( numsamples >= width )
            {
                for (i=0; i<maxsamples+PRICES_DAYWINDOW+PRICES_SMOOTHWIDTH&&i<numsamples; i++)
                {
                    offset = j*width + i;
                    rngval = (rngval*11109 + 13849);
                    if ( (correlated[i]= chunguscoin_pricecorrelated(rngval,j,&prices[offset],1,0,PRICES_SMOOTHWIDTH)) < 0 )
                        throw JSONRPCError(RPC_INVALID_PARAMETER, "null correlated price");
                    {
                        if ( chunguscoin_priceget(checkprices,j,nextheight-1-i,1) >= 0 )
                        {
                ...
                {
                    offset = j*width + i;
                    smoothed = chunguscoin_priceave(tmpbuf,&correlated[i],1);
                    if ( chunguscoin_priceget(checkprices,j,nextheight-1-i,1) >= 0 )
                    {
                    ...
                    parr.push_back(ValueFromAmount((int64_t)prices[offset] * chunguscoin_pricemult(j)));
...
UniValue gettxout(const UniValue& params, bool fHelp, const CPubKey& mypk)
{
    if (fHelp || params.size() < 2 || params.size() > 3)
        throw runtime_error(
            ...
            "        \"chunguscoinaddress\"        (string) Chunguscoin address\n"
            "        ,...\n"
            "     ]\n"
            "  },\n"
    ...
    if ((unsigned int)coins.nHeight == MEMPOOL_HEIGHT) {
        ...
        ret.push_back(Pair("confirmations", chunguscoin_dpowconfs(coins.nHeight,pindex->GetHeight() - coins.nHeight + 1)));
    ...
    if ( (interest= chunguscoin_accrued_interest(&txheight,&locktime,hash,n,coins.nHeight,coins.vout[n].nValue,(int32_t)pindex->GetHeight())) != 0 )
...
UniValue getblockchaininfo(const UniValue& params, bool fHelp, const CPubKey& mypk)
{
    ...
    if ( ASSETCHAINS_SYMBOL[0] == 0 ) {
        ...
        int32_t longestchain = CHUNGUSCOIN_LONGESTCHAIN;//chunguscoin_longestchain();
