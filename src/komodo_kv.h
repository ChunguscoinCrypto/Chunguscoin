...
#ifndef H_CHUNGUSCOINKV_H
#define H_CHUNGUSCOINKV_H
...
int32_t chunguscoin_kvnumdays(uint32_t flags)
{
    int32_t numdays;
...
{
    return(chunguscoin_kvnumdays(flags) * CHUNGUSCOIN_KVDURATION);
}
...
uint64_t chunguscoin_kvfee(uint32_t flags,int32_t opretlen,int32_t keylen)
{
    ...
    if ( (k= keylen) > 32 )
        k = 32;
    numdays = chunguscoin_kvnumdays(flags);
    if ( (fee= (numdays*(opretlen * opretlen / k))) < 100000 )
        fee = 100000;
    return(fee);
}

int32_t chunguscoin_kvsearch(uint256 *pubkeyp,int32_t current_height,uint32_t *flagsp,int32_t *heightp,uint8_t value[IGUANA_MAXSCRIPTSIZE],uint8_t *key,int32_t keylen)
{
    struct chunguscoin_kv *ptr; int32_t duration,retval = -1;
    *heightp = -1;
    *flagsp = 0;
    memset(pubkeyp,0,sizeof(*pubkeyp));
    portable_mutex_lock(&CHUNGUSCOIN_KV_mutex);
    HASH_FIND(hh,CHUNGUSCOIN_KV,key,keylen,ptr);
    if ( ptr != 0 )
    {
        duration = chunguscoin_kvduration(ptr->flags);
        ...
        {
            HASH_DELETE(hh,CHUNGUSCOIN_KV,ptr);
            if ( ptr->value != 0 )
                free(ptr->value);
            if ( ptr->key != 0 )
                free(ptr->key);
    ...
    portable_mutex_unlock(&CHUNGUSCOIN_KV_mutex);
    if ( retval < 0 )
    {
        // search rawmempool
    }
...
void chunguscoin_kvupdate(uint8_t *opretbuf,int32_t opretlen,uint64_t value)
{
    static uint256 zeroes;
    uint32_t flags; uint256 pubkey,refpubkey,sig; int32_t i,refvaluesize,hassig,coresize,haspubkey,height,kvheight; uint16_t keylen,valuesize,newflag = 0; uint8_t *key,*valueptr,keyvalue[IGUANA_MAXSCRIPTSIZE*8]; struct chunguscoin_kv *ptr; char *transferpubstr,*tstr; uint64_t fee;
    ...
    if ( keylen+13 > opretlen )
    {
        ...
        if ( ++counter < 1 )
            LogPrintf("chunguscoin_kvupdate: keylen.%d + 13 > opretlen.%d, this can be ignored\n",keylen,opretlen);
        return;
    }
    valueptr = &key[keylen];
    ...
    if ( value >= fee )
    {
        ...
        if ( opretlen == coresize || opretlen == coresize+sizeof(uint256) || opretlen == coresize+2*sizeof(uint256) )
        {
            ...
            if ( (refvaluesize= chunguscoin_kvsearch((uint256 *)&refpubkey,height,&flags,&kvheight,&keyvalue[keylen],key,keylen)) >= 0 )
            {
                if ( memcmp(&zeroes,&refpubkey,sizeof(refpubkey)) != 0 )
                {
                    if ( chunguscoin_kvsigverify(keyvalue,keylen+refvaluesize,refpubkey,sig) < 0 )
                    {
                        //LogPrintf("chunguscoin_kvsigverify error [%d]\n",coresize-13);
                        return;
                    }
                }
            }
            ...
            if ( ptr != 0 )
            {
                ...
                //if ( (ptr->flags & CHUNGUSCOIN_KVPROTECTED) != 0 )
                {
                    tstr = (char *)"transfer:";
            ...
            {
                ptr = (struct chunguscoin_kv *)calloc(1,sizeof(*ptr));
                ptr->key = (uint8_t *)calloc(1,keylen);
                ptr->keylen = keylen;
                memcpy(ptr->key,key,keylen);
                newflag = 1;
                HASH_ADD_KEYPTR(hh,CHUNGUSCOIN_KV,ptr->key,ptr->keylen,ptr);
                //LogPrintf("KV add.(%s) (%s)\n",ptr->key,valueptr);
            }
            if ( newflag != 0 || (ptr->flags & CHUNGUSCOIN_KVPROTECTED) == 0 )
            {
                if ( ptr->value != 0 )
