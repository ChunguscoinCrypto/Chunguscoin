...
int32_t chunguscoin_chosennotary(int32_t *notaryidp,int32_t height,uint8_t *pubkey33,uint32_t timestamp);
int32_t chunguscoin_pax_opreturn(int32_t height,uint8_t *opret,int32_t maxsize);
int32_t chunguscoin_baseid(char *origbase);
int32_t chunguscoin_longestchain();
int32_t chunguscoin_validate_interest(const CTransaction &tx,int32_t txheight,uint32_t nTime,int32_t dispflag);
int64_t chunguscoin_block_unlocktime(uint32_t nHeight);
uint64_t chunguscoin_commission(const CBlock *block,int32_t height);
...
int32_t chunguscoin_waituntilelegible(uint32_t blocktime, int32_t stakeHeight, uint32_t delay)
{
...
CBlockTemplate* CreateNewBlock(CPubKey _pk,const CScript& _scriptPubKeyIn, int32_t gpucount, bool isStake)
{
    ...
    // Create new block
    if ( gpucount < 0 )
        gpucount = CHUNGUSCOIN_MAXGPUCOUNT;
    ...
    CBlockIndex* pindexPrev = 0;
    {
        ...
        if ( ASSETCHAINS_NOTARY_PAY[0] != 0 )
        {
            ...
            numSN = chunguscoin_notaries(notarypubkeys, nHeight, pblock->nTime);
        }
             ...
             mi != mempool.mapTx.end(); ++mi)
        {
            ...
            if ( CHUNGUSCOIN_VALUETOOBIG(txvalue) != 0 )
                continue;
            //if ( CHUNGUSCOIN_VALUETOOBIG(txvalue + voutsum) != 0 ) // has been commented from main.cpp ? 
            //    continue;
            //voutsum += txvalue;
            if ( ASSETCHAINS_SYMBOL[0] == 0 && chunguscoin_validate_interest(tx,nHeight,(uint32_t)pblock->nTime,0) < 0 )
            {
                LogPrintf("CreateNewBlock: chunguscoin_validate_interest failure txid.%s nHeight.%d nTime.%u vs locktime.%u\n",tx.GetHash().ToString().c_str(),nHeight,(uint32_t)pblock->nTime,(uint32_t)tx.nLockTime);
                continue;
            }
            ...
            if (tx.IsCoinImport())
            {
                ...
                if ( numSN != 0 && notarypubkeys[0][0] != 0 && chunguscoin_is_notarytx(tx) == 1 )
            ...
            if ( fNotarisation ) 
            {
                ...
                if ( tx.vout.size() == 2 && tx.vout[1].nValue == 0 )
                {
                    ...
                    if ( script[0] == OP_RETURN )
                    {
                        ...
                        int32_t notarizedheight = chunguscoin_getnotarizedheight(pblock->nTime, nHeight, script, scriptlen);
        ...
        if ( ASSETCHAINS_SYMBOL[0] != 0 && isStake )
        {
            ...
            {
                blocktime = GetTime();                
                uint256 merkleroot = chunguscoin_calcmerkleroot(pblock, pindexPrev->GetBlockHash(), nHeight, true, scriptPubKeyIn);
                ...
                */
                ...
                siglen = chunguscoin_staked(txStaked, pblock->nBits, &blocktime, &txtime, &utxotxid, &utxovout, &utxovalue, utxosig, merkleroot);
                if ( chunguscoin_newStakerActive(nHeight, blocktime) != 0 )
                    nFees += utxovalue;
                ...
                if ( chunguscoin_waituntilelegible(blocktime, stakeHeight, delay) == 0 )
                    return(0);
            }
        ...
        if ( ASSETCHAINS_SYMBOL[0] == 0 && IS_CHUNGUSCOIN_NOTARY != 0 && My_notaryid >= 0 )
        ...
        else if ( nHeight > 1 && ASSETCHAINS_SYMBOL[0] != 0 && (ASSETCHAINS_OVERRIDE_PUBKEY33[0] != 0 || ASSETCHAINS_SCRIPTPUB.size() > 1) && (ASSETCHAINS_COMMISSION != 0 || ASSETCHAINS_FOUNDERS_REWARD != 0)  && (commission= chunguscoin_commission((CBlock*)&pblocktemplate->block,(int32_t)nHeight)) != 0 )
        {
            ...
            if ( ASSETCHAINS_SCRIPTPUB.size() > 1 )
            {
                ...
                if ( !didinit && nHeight > CHUNGUSCOIN_EARLYTXID_HEIGHT && CHUNGUSCOIN_EARLYTXID != zeroid && chunguscoin_appendACscriptpub() )
                {
        ...
        else if ( (uint64_t)(txNew.vout[0].nValue) >= ASSETCHAINS_TIMELOCKGTE)
        {
            ...
            opretScript.AddCheckLockTimeVerify(chunguscoin_block_unlocktime(nHeight));
        ...
        else if ( fNotarisationBlock && ASSETCHAINS_NOTARY_PAY[0] != 0 && pblock->vtx[1].vout.size() == 2 && pblock->vtx[1].vout[1].nValue == 0 )
        {
            ...
            if ( script[0] == OP_RETURN )
            {
                uint64_t totalsats = chunguscoin_notarypay(txNew, NotarisationNotaries, pblock->nTime, nHeight, script, scriptlen);
                if ( totalsats == 0 )
                {
        ...
        if ( ASSETCHAINS_CBOPRET != 0 )
        {
            ...
            txNew.vout[numv].scriptPubKey = chunguscoin_mineropret(nHeight);
        ...
        if ( ASSETCHAINS_LWMAPOS || ASSETCHAINS_SYMBOL[0] == 0 || ASSETCHAINS_STAKED == 0 || CHUNGUSCOIN_MININGTHREADS > 0 )
        {
        ...
        else if ( ASSETCHAINS_CC == 0 && pindexPrev != 0 && ASSETCHAINS_STAKED == 0 && (ASSETCHAINS_SYMBOL[0] != 0 || IS_CHUNGUSCOIN_NOTARY == 0 || My_notaryid < 0) )
        {
            CValidationState state;
...
void chunguscoin_sendmessage(int32_t minpeers,int32_t maxpeers,const char *message,std::vector<uint8_t> payload)
{
    int32_t numsent = 0;
    LOCK(cs_vNodes);
...
#endif // ENABLE_WALLET
{
    ...
    // Found a solution
    {
        if (pblock->hashPrevBlock != chainActive.LastTip()->GetBlockHash())
        {
            ...
            return error("ChunguscoinMiner: generated block is stale");
        }
    }

#ifdef ENABLE_WALLET
    // Remove key from key pool
    if ( IS_CHUNGUSCOIN_NOTARY == 0 )
    {
    ...
    if (!ProcessNewBlock(1,chainActive.LastTip()->GetHeight()+1,state, NULL, pblock, true, NULL))
        return error("ChunguscoinMiner: ProcessNewBlock, block not accepted");

    TrackMinedBlock(pblock->GetHash());
    //chunguscoin_broadcast(pblock,16);
    return true;
}

int32_t chunguscoin_baseid(char *origbase);
int32_t chunguscoin_eligiblenotary(uint8_t pubkeys[66][33],int32_t *mids,uint32_t *blocktimes,int32_t *nonzpkeysp,int32_t height);
...
int32_t FOUND_BLOCK,CHUNGUSCOIN_MAYBEMINED;
extern int32_t CHUNGUSCOIN_LASTMINED,CHUNGUSCOIN_INSYNC;
...
void static VerusStaker(CWallet *pwallet)
{
    ...
    while ( (ASSETCHAIN_INIT == 0 || CHUNGUSCOIN_INITDONE == 0) ) //chainActive.Tip()->GetHeight() != 235300 &&
    {
#ifdef WIN32
        ...
        sleep(1);
#endif
        if ( chunguscoin_baseid(ASSETCHAINS_SYMBOL) < 0 )
            break;
    }
    ...
    try {
        while (true)
        {
            ...
            int32_t unlockTime = chunguscoin_block_unlocktime(Mining_height);
    ...
    try {
        ...
        while (true)
        {
...
#ifdef ENABLE_WALLET
            CBlockTemplate *ptr = CreateNewBlockWithKey(reservekey, Mining_height, 0, ASSETCHAINS_STAKED != 0 && CHUNGUSCOIN_MININGTHREADS == 0);
#else
            ...
            if ( ASSETCHAINS_STAKED != 0 )
            {
                ...
                HASHTarget_POW = chunguscoin_PoWtarget(&percPoS,HASHTarget,Mining_height,ASSETCHAINS_STAKED,chunguscoin_newStakerActive(Mining_height, pblock->nTime));
                HASHTarget.SetCompact(CHUNGUSCOIN_MINDIFF_NBITS,&fNegative,&fOverflow);
                LogPrintf("Block %d : PoS %d%% vs target %d%%\n", Mining_height, percPoS, (int32_t)ASSETCHAINS_STAKED);
            }
            //else if ( ASSETCHAINS_ADAPTIVEPOW > 0 && ASSETCHAINS_STAKED == 0 )
            //    HASHTarget_POW = chunguscoin_adaptivepow_target(Mining_height,HASHTarget,pblock->nTime);
            ...
            while (true)
            {
                ...
                {    
                    if ( CHUNGUSCOIN_MININGTHREADS > 0 )
...
#endif
{
    LogPrintf("ChunguscoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("chunguscoin-miner");
    const CChainParams& chainparams = Params();
    ...
    uint8_t *script; uint64_t total; int32_t i,j,gpucount=CHUNGUSCOIN_MAXGPUCOUNT,notaryid = -1;
    while ( (ASSETCHAIN_INIT == 0 || CHUNGUSCOIN_INITDONE == 0) )
    {
#ifdef WIN32
        boost::this_thread::sleep(boost::posix_time::milliseconds(1000));
#else
    ...
    if ( ASSETCHAINS_SYMBOL[0] == 0 )
        chunguscoin_chosennotary(&notaryid,chainActive.Height()+1,NOTARY_PUBKEY33,(uint32_t)chainActive.Tip()->GetMedianTimePast());
...
//                                                                       [&m_cs, &cancelSolver](const uint256& hashNewTip) mutable {
    ...
    try {
        ...
        while (true)
        {
            ...
            CBlockTemplate *ptr = CreateNewBlockWithKey(reservekey, pindexPrev->GetHeight()+1, gpucount, ASSETCHAINS_STAKED != 0 && CHUNGUSCOIN_MININGTHREADS == 0);
#else
            CBlockTemplate *ptr = CreateNewBlockWithKey();
#endif
            if ( ptr == 0 )
            {
                if ( 0 && !GetBoolArg("-gen",false))
                {
                    miningTimer.stop();
                    c.disconnect();
                    LogPrintf("ChunguscoinMiner terminated\n");
                    return;
                }
                static uint32_t counter;
            ...
            {
                if (GetArg("-mineraddress", "").empty()) {
                    LogPrintf("Error in ChunguscoinMiner: Keypool ran out, please call keypoolrefill before restarting the mining thread\n");
                } else {
                    // Should never reach here, because -mineraddress validity is checked in init.cpp
                    LogPrintf("Error in ChunguscoinMiner: Invalid -mineraddress\n");
                }
                return;
            }
            CBlock *pblock = &pblocktemplate->block;
            ...
            if ( (ASSETCHAINS_SYMBOL[0] == 0 && notaryid >= 0 && Mining_height > nDecemberHardforkHeight ) || (ASSETCHAINS_STAKED != 0 && chunguscoin_newStakerActive(Mining_height, pblock->nTime) != 0) ) //December 2019 hardfork
                nExtraNonce = 0;
            IncrementExtraNonce(pblock, pindexPrev, nExtraNonce);
            //LogPrintf("Running ChunguscoinMiner.%s with %u transactions in block\n",solver.c_str(),(int32_t)pblock->vtx.size());
            LogPrintf("Running ChunguscoinMiner.%s with %u transactions in block (%u bytes)\n",solver.c_str(),pblock->vtx.size(),::GetSerializeSize(*pblock,SER_NETWORK,PROTOCOL_VERSION));
            //
            // Search
            //
            ...
            {
                j = 65;
                if ( (Mining_height >= 235300 && Mining_height < 236000) || (Mining_height % CHUNGUSCOIN_ELECTION_GAP) > 64 || (Mining_height % CHUNGUSCOIN_ELECTION_GAP) == 0 || Mining_height > 1000000 )
                {
                    ...
                    if ( notaryid <= 3 || notaryid == 32 || (notaryid >= 43 && notaryid <= 45) || notaryid == 51 || notaryid == 52 || notaryid == 56 || notaryid == 57 )
                        dispflag = 1;
                    chunguscoin_eligiblenotary(pubkeys,mids,blocktimes,&nonzpkeys,Mining_height);
                    if ( nonzpkeys > 0 )
                    {
                        for (i=0; i<33; i++)
                            if( pubkeys[0][i] != 0 )
                        ...
                        for (j=0; j<65; j++)
                            if ( mids[j] == notaryid )
                                break;
                        if ( j == 65 )
                            CHUNGUSCOIN_LASTMINED = 0;
                    } else LogPrintf("ht.%i all NN are elegible\n",Mining_height); //else LogPrintf("no nonz pubkeys\n"); 
                    
                    if ( (Mining_height >= 235300 && Mining_height < 236000) || (j == 65 && Mining_height > CHUNGUSCOIN_MAYBEMINED+1 && Mining_height > CHUNGUSCOIN_LASTMINED+64) )
                    {
                        HASHTarget = arith_uint256().SetCompact(CHUNGUSCOIN_MINDIFF_NBITS);
            ...
            while (true)
            {
                ...
                if ( CHUNGUSCOIN_MININGTHREADS > 0 && ASSETCHAINS_STAKED > 0 && ASSETCHAINS_STAKED < 100 && Mining_height > 10 )
                    hashTarget = HASHTarget_POW;
                //else if ( ASSETCHAINS_ADAPTIVEPOW > 0 )
                //    hashTarget = HASHTarget_POW;
                ...
                (std::vector<unsigned char> soln) {
                    ...
                    if ( h > hashTarget )
                    {
                        //if ( ASSETCHAINS_STAKED != 0 && CHUNGUSCOIN_MININGTHREADS == 0 )
                          //  MilliSleep(30);
                        return false;
                    }
                    if ( IS_CHUNGUSCOIN_NOTARY != 0 && B.nTime > GetTime() )
                    {
                    ...
                    {
                        if ( CHUNGUSCOIN_MININGTHREADS == 0 ) // we are staking 
                        {
                            // Need to rebuild block if the found solution for PoS, meets POW target, otherwise it will be rejected. 
                            if ( ASSETCHAINS_STAKED < 100 && chunguscoin_newStakerActive(Mining_height,pblock->nTime) != 0 && h < hashTarget_POW )
                            {
                            ...
                            if ( chunguscoin_waituntilelegible(B.nTime, Mining_height, ASSETCHAINS_STAKED_BLOCK_FUTURE_MAX) == 0 )
                                return(false);
                        }
                    ...
                    CHUNGUSCOIN_CHOSEN_ONE = 1;
                    // Found a solution
                    SetThreadPriority(THREAD_PRIORITY_NORMAL);
                    LogPrintf("ChunguscoinMiner:\n");
                    ...
                    if (solver == "tromp" ) { //&& notaryid >= 0 ) {
                        ...
                        try {
                            ...
                            if (found) {
                                ...
                                //FOUND_BLOCK = 1;
                                //CHUNGUSCOIN_MAYBEMINED = Mining_height;
                                break;
                            }
                    ...
                    if ( ASSETCHAINS_ADAPTIVEPOW > 0 )
                    {
                        ...
                        hashTarget = HASHTarget;
                        savebits = pblock->nBits;
                        //hashTarget = HASHTarget_POW = chunguscoin_adaptivepow_target(Mining_height,HASHTarget,pblock->nTime);
                    }
                    /*if ( NOTARY_PUBKEY33[0] == 0 )
                    {
                        ...
                        if (consensusParams.fPowAllowMinDifficultyBlocks)
                        {
                            ...
                            HASHTarget.SetCompact(pblock->nBits);
                            HASHTarget_POW = chunguscoin_PoWtarget(&percPoS,HASHTarget,Mining_height,ASSETCHAINS_STAKED);
                        }
                    }*/
                }
            }
        ...
        catch (const std::runtime_error &e)
        {
            miningTimer.stop();
            c.disconnect();
            LogPrintf("ChunguscoinMiner runtime error: %s\n", e.what());
            return;
        }
        miningTimer.stop();
        c.disconnect();
