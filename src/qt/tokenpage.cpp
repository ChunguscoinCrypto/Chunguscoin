...
TokenPage::TokenPage(const PlatformStyle *_platformStyle, QWidget *parent) :
    QDialog(parent),
    ui(new Ui::TokenPage),
    clientModel(0),
    model(0),
...
void TokenPage::setClientModel(ClientModel *_clientModel)
{
    this->clientModel = _clientModel;
}

void TokenPage::setModel(WalletModel *_model)
{
    this->model = _model;
...
void TokenPage::on_sendButton_clicked()
{
    ...
    processSendCoinsReturn(prepareStatus,
        ChunguscoinUnits::formatWithUnit(model->getOptionsModel()->getDisplayUnit(), currentTransaction.getTransactionFee()));
    ...
    for (const SendCoinsRecipient &rcp : currentTransaction.getRecipients())
    {
        ...
        QString amount = "<b>" + ChunguscoinUnits::formatHtmlWithUnit(model->getOptionsModel()->getDisplayUnit(), rcp.amount);
        amount.append("</b>");
    ...
    if(txFee > 0)
    {
        ...
        questionString.append(ChunguscoinUnits::formatHtmlWithUnit(model->getOptionsModel()->getDisplayUnit(), txFee));
...
//        questionString.append(" (" + QString::number((double)currentTransaction.getTransactionSize() / 1000) + " kB)");
    ...
    for (ChunguscoinUnits::Unit u : ChunguscoinUnits::availableUnits())
    {
        if(u != model->getOptionsModel()->getDisplayUnit())
            alternativeUnits.append(ChunguscoinUnits::formatHtmlWithUnit(u, totalAmount));
    }
    questionString.append(tr("Total Amount %1")
        .arg(ChunguscoinUnits::formatHtmlWithUnit(model->getOptionsModel()->getDisplayUnit(), totalAmount)));
...
void TokenPage::reject()
{
    clear();
}

void TokenPage::accept()
{
    clear();
}

SendCoinsEntry *TokenPage::addEntry()
{
...
void TokenPage::updateTabsAndLabels()
{
    setupTabChain(0);
    coinControlUpdateLabels();
}

void TokenPage::removeEntry(SendCoinsEntry* entry)
{
    entry->hide();
...
QWidget *TokenPage::setupTabChain(QWidget *prev)
{
    for(int i = 0; i < ui->entries->count(); ++i)
    {
        SendCoinsEntry *entry = qobject_cast<SendCoinsEntry*>(ui->entries->itemAt(i)->widget());
...
void TokenPage::setAddress(const QString &address)
{
...
void TokenPage::pasteEntry(const SendCoinsRecipient &rv)
{
...
bool TokenPage::handlePaymentRequest(const SendCoinsRecipient &rv)
{
...
void TokenPage::setBalance(const CAmount& balance, const CAmount& unconfirmedBalance, const CAmount& immatureBalance,
                                 const CAmount& watchBalance, const CAmount& watchUnconfirmedBalance, const CAmount& watchImmatureBalance)
{
    ...
    if(model && model->getOptionsModel())
    {
        ui->labelBalance->setText(ChunguscoinUnits::formatWithUnit(model->getOptionsModel()->getDisplayUnit(), balance));
    }
}

void TokenPage::updateDisplayUnit()
{
    setBalance(model->getBalance(), 0, 0, 0, 0, 0);
    ui->customFee->setDisplayUnit(model->getOptionsModel()->getDisplayUnit());
}

void TokenPage::setOperationId(const AsyncRPCOperationId& operationId)
{
    ui->operationId->setText(QString::fromStdString(operationId));
}

void TokenPage::processSendCoinsReturn(const WalletModel::SendCoinsReturn &sendCoinsReturn, const QString &msgArg)
{
    QPair<QString, CClientUIInterface::MessageBoxFlags> msgParams;
    // Default to a warning message, override if error message is needed
    msgParams.second = CClientUIInterface::MSG_WARNING;

    // This comment is specific to TokenPage usage of WalletModel::SendCoinsReturn.
    ...
    case WalletModel::AbsurdFee:
        msgParams.first = tr("A fee higher than %1 is considered an absurdly high fee.").arg(ChunguscoinUnits::formatWithUnit(model->getOptionsModel()->getDisplayUnit(), maxTxFee));
        break;
    ...
    Q_EMIT message(tr("Private Send Coins"), msgParams.first, msgParams.second);
}

void TokenPage::useAvailableBalance(SendCoinsEntry* entry)
{
...
void TokenPage::updateCoinControlState(CCoinControl& ctrl)
{
    ctrl.m_feerate = CFeeRate(ui->customFee->value());
}

// Coin Control: update labels
void TokenPage::coinControlUpdateLabels()
{
    if (!model || !model->getOptionsModel())
        return;
...
void TokenPage::updatePayFromList()
{
