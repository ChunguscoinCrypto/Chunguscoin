...
extern pthread_mutex_t CHUNGUSCOIN_CC_mutex;

bool RunCCEval(const CC *cond, const CTransaction &tx, unsigned int nIn)
{
    EvalRef eval;
    pthread_mutex_lock(&CHUNGUSCOIN_CC_mutex);
    bool out = eval->Dispatch(cond, tx, nIn);
    pthread_mutex_unlock(&CHUNGUSCOIN_CC_mutex);
...
extern int32_t chunguscoin_notaries(uint8_t pubkeys[64][33],int32_t height,uint32_t timestamp);


int32_t Eval::GetNotaries(uint8_t pubkeys[64][33], int32_t height, uint32_t timestamp) const
{
    return chunguscoin_notaries(pubkeys, height, timestamp);
}
