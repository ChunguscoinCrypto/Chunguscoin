...
///
/// One other technical note is that chunguscoind has the insight-explorer extensions built in 
...
/// https://developers.chunguscoinplatform.com/basic-docs/start-here/about-chunguscoin-platform/product-introductions.html#smart-chains-antara
...
#define CC_REQUIREMENTS_MSG (CHUNGUSCOIN_NSPV_SUPERLITE?"to use CC contracts you need to nspv_login first\n":"to use CC contracts, you need to launch daemon with valid -pubkey= for an address in your wallet\n")
...
//CBlockIndex *chunguscoin_getblockindex(uint256 hash);  //moved to chunguscoin_def.h
//int32_t chunguscoin_nextheight();  //moved to chunguscoin_def.h
...
//CBlockIndex *chunguscoin_blockindex(uint256 hash); //moved to chunguscoin_def.h
//CBlockIndex *chunguscoin_chainactive(int32_t height); //moved to chunguscoin_def.h
//int32_t chunguscoin_blockheight(uint256 hash); //moved to chunguscoin_def.h
//void StartShutdown();
...
//void chunguscoin_sendmessage(int32_t minpeers,int32_t maxpeers,const char *message,std::vector<uint8_t> payload); // moved to komodo_defs.h

/// @private
int32_t payments_parsehexdata(std::vector<uint8_t> &hexdata,cJSON *item,int32_t len);
// int32_t chunguscoin_blockload(CBlock& block,CBlockIndex *pindex); // this def in komodo_defs.h
...
/// Returns my pubkey, that is set by -pubkey chunguscoind parameter
...
// bool chunguscoin_txnotarizedconfirmed(uint256 txid); //moved to komodo_defs.h
...
/// By using -addressindex=1 of chunguscoind daemon, it allows tracking of all the CC addresses.
///
...
/// 'My pubkey' is the -pubkey parameter of chunguscoind.
/// @param mtx mutable transaction object
/// @param total amount of inputs to add. If total equals to 0 the function does not add inputs but returns amount of all available normal inputs in the wallet
/// @param maxinputs maximum number of inputs to add
...
/// Logging category is set by -debug=category chunguscoind param.
