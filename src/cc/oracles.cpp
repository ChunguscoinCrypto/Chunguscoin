...
*/
extern int32_t chunguscoin_get_current_height();
...
{
    if ( CHUNGUSCOIN_NSPV_SUPERLITE ) return (NSPV_blocktime(height));
...
{
    CMutableTransaction mtx = CreateNewContextualCMutableTransaction(Params().GetConsensus(), chunguscoin_nextheight()); CTransaction tx;
    CPubKey mypk,oraclespk; struct CCcontract_info *cp,C; std::string name,desc,format; int32_t numvouts; uint256 hashBlock;

    if (GetLatestTimestamp(chunguscoin_currentheight())<PUBKEY_SPOOFING_FIX_ACTIVATION)
...
UniValue OracleRegister(const CPubKey& pk, int64_t txfee,uint256 oracletxid,int64_t datafee)
{
    ...
    {
        if (GetLatestTimestamp(chunguscoin_currentheight())>PUBKEY_SPOOFING_FIX_ACTIVATION && AddMyOraclesFunds(cp,mtx,mypk,oracletxid)!=CC_MARKER_VALUE)
        ...
        if (GetLatestTimestamp(chunguscoin_get_current_height())>PUBKEY_SPOOFING_FIX_ACTIVATION) mtx.vout.push_back(CTxOut(txfee,CScript() << ParseHex(HexStr(mypk)) << OP_CHECKSIG));
