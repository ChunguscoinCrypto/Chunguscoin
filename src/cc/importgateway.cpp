...
{
    CMutableTransaction mtx = CreateNewContextualCMutableTransaction(Params().GetConsensus(), chunguscoin_nextheight()), burntx = CreateNewContextualCMutableTransaction(Params().GetConsensus(), chunguscoin_nextheight());
    ...
    if (CHUNGUSCOIN_EARLYTXID!=zeroid && bindtxid!=CHUNGUSCOIN_EARLYTXID)
    {
        CCerror = strprintf("invalid import gateway. On this chain only valid import gateway is %s",CHUNGUSCOIN_EARLYTXID.GetHex());
        LOGSTREAM("importgateway",CCLOG_INFO, stream << CCerror << std::endl);
        return("");
    }
    cp = CCinit(&C,EVAL_IMPORTGATEWAY);
...
std::string ImportGatewayPartialSign(uint64_t txfee,uint256 lasttxid,std::string refcoin, std::string hex)
{
    ...
    if (funcid=='W')
    {
        ...
        else if (CHUNGUSCOIN_EARLYTXID!=zeroid && bindtxid!=CHUNGUSCOIN_EARLYTXID)
        {
            CCerror = strprintf("invalid import gateway. On this chain only valid import gateway is %s",CHUNGUSCOIN_EARLYTXID.GetHex());
            LOGSTREAM("importgateway",CCLOG_INFO, stream << CCerror << std::endl);
            return("");
