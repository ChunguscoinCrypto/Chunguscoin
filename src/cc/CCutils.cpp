...
extern uint32_t CHUNGUSCOIN_DPOWCONFS;
...
{
    if ( CHUNGUSCOIN_NSPV_SUPERLITE ) return ((uint32_t)NSPV_blocktime(height));
    return(chunguscoin_heightstamp(height));
} // :P
...
int64_t CCduration(int32_t &numblocks,uint256 txid)
{
    ...
    else if ( (pindex= chunguscoin_getblockindex(hashBlock)) == 0 || (txtime= pindex->nTime) == 0 || (txheight= pindex->GetHeight()) <= 0 )
    {
...
int32_t chunguscoin_get_current_height()
{
    if ( CHUNGUSCOIN_NSPV_SUPERLITE )
    {
...
bool chunguscoin_txnotarizedconfirmed(uint256 txid)
{
    ...
    if ( CHUNGUSCOIN_NSPV_SUPERLITE )
    {
        ...
        {
            LogPrintf("chunguscoin_txnotarizedconfirmed cant find txid %s\n",txid.ToString().c_str());
            return(0);
        }
        else if (txheight<=0)
        {
            LogPrintf("chunguscoin_txnotarizedconfirmed no txheight.%d for txid %s\n",txheight,txid.ToString().c_str());
            return(0);
        }
        else if (txheight>currentheight)
        {
            LogPrintf("chunguscoin_txnotarizedconfirmed backwards heights for txid %s hts.(%d %d)\n",txid.ToString().c_str(),txheight,currentheight);
            return(0);
        }
    ...
    else
    {
        ...
        {
            LogPrintf("chunguscoin_txnotarizedconfirmed no hashBlock for txid %s\n",txid.ToString().c_str());
            return(0);
        }
        else if ( (pindex= chunguscoin_blockindex(hashBlock)) == 0 || (txheight= pindex->GetHeight()) <= 0 )
        {
            LogPrintf("chunguscoin_txnotarizedconfirmed no txheight.%d %p for txid %s\n",txheight,pindex,txid.ToString().c_str());
            return(0);
        }
    ...
    if ((sp= chunguscoin_stateptr(symbol,dest)) != 0 && (notarized=sp->NOTARIZED_HEIGHT) > 0 && txheight > sp->NOTARIZED_HEIGHT)  notarized=0;            
#ifdef TESTMODE           
    notarized=0;
#endif //TESTMODE
...
bool ProcessCC(struct CCcontract_info *cp,Eval* eval, std::vector<uint8_t> paramsNull,const CTransaction &ctx, unsigned int nIn)
{
    ...
    height = CHUNGUSCOIN_CONNECTING;
    if ( CHUNGUSCOIN_CONNECTING < 0 ) // always comes back with > 0 for final confirmation
        return(true);
    if ( ASSETCHAINS_CC == 0 || (height & ~(1<<30)) < CHUNGUSCOIN_CCACTIVATE )
        return eval->Invalid("CC are disabled or not active yet");
    if ( (CHUNGUSCOIN_CONNECTING & (1<<30)) != 0 )
    {
        from_mempool = 1;
        height &= ((1<<30) - 1);
    }
    ...
    //LogPrintf("CHUNGUSCOIN_CONNECTING.%d mempool.%d vs CCactive.%d\n",height,from_mempool,CHUNGUSCOIN_CCACTIVATE);
