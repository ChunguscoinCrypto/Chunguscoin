...
./chunguscoind -ac_name=TESTPRC -ac_supply=100000000 -ac_reward=1000000000 -ac_nk=96,5 -ac_blocktime=20 -ac_cc=2 -ac_snapshot=50 -ac_sapling=1 -ac_earlytxidcontract=237 -testnode=1 -gen -genproclimit=1

Then in very early block < 10 or so, do paymentsairdrop eg. 
    `./chunguscoin-cli -ac_name=TESTPRC paymentsairdrop '[10,10,0,3999,0,0]'
...
Get the full `hex` of the vout[0] that pays to CryptoCondition. then place it on chain with the following command: with the hex you got in place of the hex below.
    './chunguscoin-cli -ac_name=TESTPRC opreturn_burn 1 2ea22c8020292ba5c8fd9cc89b12b35bf8f5d00196990ecbb06102b84d9748d11d883ef01e81031210008203000401cc'
...
./chunguscoind -ac_name=TESTPRC -ac_supply=100000000 -ac_reward=1000000000 -ac_nk=96,5 -ac_blocktime=20 -ac_cc=2 -ac_snapshot=50 -ac_sapling=1 -ac_earlytxidcontract=237 -earlytxid=cf89d17fb11037f65c160d0749dddd74dc44d9893b0bb67fe1f96c1f59786496 -testnode=1 -gen -genproclimit=1
...
if ( ASSETCHAINS_EARLYTXIDCONTRACT == EVAL_PRICES && CHUNGUSCOIN_EARLYTXID_SCRIPTPUB.size() == 0 )
    GetChunguscoinEarlytxidScriptPub();
...
GetChunguscoinEarlytxidScriptPub is on line #2080 of komodo_bitcoind.h 
 */
...
static bool ValidateBetTx(struct CCcontract_info *cp, Eval *eval, const CTransaction & bettx)
{
    ...
    if ( bettx.vout[4].scriptPubKey != CHUNGUSCOIN_EARLYTXID_SCRIPTPUB )
...
static bool ValidateAddFundingTx(struct CCcontract_info *cp, Eval *eval, const CTransaction & addfundingtx, const CTransaction & vintx)
{
    ...
    // This should be all you need to verify it, maybe also check amount? 
    if (addfundingtx.vout[2].scriptPubKey != CHUNGUSCOIN_EARLYTXID_SCRIPTPUB)
        return eval->Invalid("the fee was paid to wrong address.");
...
static std::string prices_getsourceexpression(const std::vector<uint16_t> &vec) {
    ...
    for (int32_t i = 0; i < vec.size(); i++) 
    {
        ...
        int32_t value = (opcode & (CHUNGUSCOIN_MAXPRICES - 1));   // index or weight 

        switch (opcode & CHUNGUSCOIN_PRICEMASK)
        {
        case 0: // indices 
            chunguscoin_pricename(name, value);
...
int32_t prices_syntheticvec(std::vector<uint16_t> &vec, std::vector<std::string> synthetic)
{
    ...
    for (i = 0; i < synthetic.size(); i++)
    {
        ...
        else if (!is_weight_str(opstr) && (ind = chunguscoin_priceind(opstr.c_str())) >= 0)
            opcode = ind, need = 0;
        else if ((weight = atoi(opstr.c_str())) > 0 && weight < CHUNGUSCOIN_MAXPRICES)
        {
        ...
        if ((opcode & CHUNGUSCOIN_PRICEMASK) != PRICES_WEIGHT) { // skip weight
...
int64_t prices_syntheticprice(std::vector<uint16_t> vec, int32_t height, int32_t minmax, int16_t leverage)
{
    ...
    {
        opcode = vec[i];
        value = (opcode & (CHUNGUSCOIN_MAXPRICES - 1));   // index or weight 

        mpz_set_ui(mpzResult, 0);  // clear result to test overflow (see below)

        //std::cerr << "prices_syntheticprice" << " i=" << i << " mpzTotalPrice=" << mpz_get_si(mpzTotalPrice) << " value=" << value << " depth=" << depth <<  " opcode&CHUNGUSCOIN_PRICEMASK=" << (opcode & CHUNGUSCOIN_PRICEMASK) <<std::endl;
        switch (opcode & CHUNGUSCOIN_PRICEMASK)
        {
        case 0: // indices 
            pricestack[depth] = 0;
            if (chunguscoin_priceget(pricedata, value, height, 1) >= 0)
            {
...
{
    int32_t nextheight = chunguscoin_nextheight();
    ...
    if (AddNormalinputs(mtx, mypk, amount + 4 * txfee, 64) >= amount + 4 * txfee)
    {
        ...
        mtx.vout.push_back(CTxOut(amount-betamount, CHUNGUSCOIN_EARLYTXID_SCRIPTPUB)); 
...
int32_t prices_getbetinfo(uint256 bettxid, BetInfo &betinfo)
{
    ...
    if (myGetTransaction(bettxid, bettx, hashBlock) && bettx.vout.size() > 3)
    {
        ...
        // TODO: forget old tx
        //CBlockIndex *bi = chunguscoin_getblockindex(hashBlock);
...
static bool prices_ispositionup(const std::vector<uint16_t> &vecparsed, int16_t leverage) {
    if (vecparsed.size() > 1 && vecparsed.size() <= 3) {
        ...
        int32_t value = (opcode & (CHUNGUSCOIN_MAXPRICES - 1));   // filter index or weight = opcode & (2048-1)

        if ((opcode & CHUNGUSCOIN_PRICEMASK) == 0) {
            char name[65];
            if (chunguscoin_pricename(name, value)) {
                std::string upperquote, bottomquote;
                prices_splitpair(std::string(name), upperquote, bottomquote);
                
                uint16_t opcode1 = vecparsed[1];
                bool isInverted = ((opcode1 & CHUNGUSCOIN_PRICEMASK) == PRICES_INV);

                //std::cerr << "prices_ispositionup upperquote=" << upperquote << " bottomquote=" << bottomquote << " opcode1=" << opcode1 << " (opcode1 & CHUNGUSCOIN_PRICEMASK)=" << (opcode1 & CHUNGUSCOIN_PRICEMASK) << std::endl;
...
static bool prices_isopposite(BetInfo p1, BetInfo p2) {
    if (p1.vecparsed.size() <= 3 && p2.vecparsed.size() <= 3) {   // simple synthetic exp
        ...
        int32_t value1 = (opcode1 & (CHUNGUSCOIN_MAXPRICES - 1));   // index or weight 
        int32_t value2 = (opcode2 & (CHUNGUSCOIN_MAXPRICES - 1));   // index or weight 

        if ( (opcode1 & CHUNGUSCOIN_PRICEMASK) == 0 && (opcode2 & CHUNGUSCOIN_PRICEMASK) == 0 ) {
            char name1[65];
            char name2[65];
            if (chunguscoin_pricename(name1, value1) && chunguscoin_pricename(name2, value2)) {
                ...
                bool isInverted1 = ((opcode1 & CHUNGUSCOIN_PRICEMASK) != PRICES_INV);
                bool isInverted2 = ((opcode2 & CHUNGUSCOIN_PRICEMASK) != PRICES_INV);

                if (/*upperquote1 == bottomquote2 && bottomquote1 == upperquote2 && (p1.leverage > 0 == p2.leverage > 0 || (opcode1 & CHUNGUSCOIN_PRICEMASK) == PRICES_INV == (opcode2 & CHUNGUSCOIN_PRICEMASK) == PRICES_INV) ||*/
    ...
    while (book.size() > 0) {
        ...
        if (book[0].isOpen) {
            ...
            if (book[0].vecparsed.size() <= 3) {   // only short expr check for match: "BTC_USD,1" or "BTC_USD,!,1"
                char name[65];
                chunguscoin_pricename(name, (book[0].vecparsed[0] & (CHUNGUSCOIN_MAXPRICES - 1)));
