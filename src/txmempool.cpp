        ...
        if (!CheckFinalTx(tx, flags)) {
            ...
            BOOST_FOREACH(const CTxIn& txin, tx.vin) {
                ...
                if (!coins || (coins->IsCoinBase() && (((signed long)nMemPoolHeight) - coins->nHeight < COINBASE_MATURITY) && 
                                                       ((signed long)nMemPoolHeight < chunguscoin_block_unlocktime(coins->nHeight) && 
...
void CTxMemPool::removeExpired(unsigned int nBlockHeight)
{
    ...
    for (indexed_transaction_set::const_iterator it = mapTx.begin(); it != mapTx.end(); it++)
    {
        ...
        In ConnectBlock we have a condition for each tx like chunguscoin_validate_interest(..., block.nTime), so
        ...
        */
        ...
        // if (IsExpiredTx(tx, nBlockHeight) || (ASSETCHAINS_SYMBOL[0] == 0 && tipindex != 0 && chunguscoin_validate_interest(tx,tipindex->GetHeight()+1,tipindex->GetMedianTimePast() + 777,0)) < 0)
        bool fInterestNotValidated = ASSETCHAINS_SYMBOL[0] == 0 && tipindex != 0 && chunguscoin_validate_interest(tx,tipindex->GetHeight()+1,tipindex->GetMedianTimePast() + 777,0) < 0;
