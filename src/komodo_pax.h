...
uint64_t chunguscoin_maxallowed(int32_t baseid)
{
...
uint64_t chunguscoin_paxvol(uint64_t volume,uint64_t price)
{
...
void pax_rank(uint64_t *ranked,uint32_t *pvals)
{
    ...
    for (i=0; i<32; i++)
    {
        vals[i] = chunguscoin_paxvol(M1SUPPLY[i] / MINDENOMS[i],pvals[i]);
        sum += vals[i];
    }
    for (i=0; i<32; i++)
    {
...
#ifdef _WIN32
    sprintf(fname,"%s\\%s",GetDataDir(false).string().c_str(),(char *)"chunguscoinfeed");
#else
    sprintf(fname,"%s/%s",GetDataDir(false).string().c_str(),(char *)"chunguscoinfeed");
#endif
            ...
            if ( (retval= (int32_t)fread(data,1,fsize,fp)) == fsize )
            {
                ...
                if ( check == crc32 )
                {
                    ...
                    if ( dpow_readprices(height,&data[len],&timestamp,&KMDBTC,&BTCUSD,&CNYUSD,pvals) > 0 )
                    {
                        ...
                        {
                            n = chunguscoin_opreturnscript(opret,'P',data+sizeof(crc32),(int32_t)(fsize-sizeof(crc32)));
                            if ( 0 && lastcrc != crc32 )
                            {
                                for (i=0; i<n; i++)
...
void chunguscoin_pvals(int32_t height,uint32_t *pvals,uint8_t numpvals)
{
...
uint64_t chunguscoin_paxcorrelation(uint64_t *votes,int32_t numvotes,uint64_t seed)
{
...
uint64_t chunguscoin_paxcalc(int32_t height,uint32_t *pvals,int32_t baseid,int32_t relid,uint64_t basevolume,uint64_t refkmdbtc,uint64_t refbtcusd)
{
    uint32_t pvalb,pvalr; double BTCUSD; uint64_t price,kmdbtc,btcusd,usdvol,baseusd,usdkmd,baserel,ranked[32];
    if ( basevolume > CHUNGUSCOIN_PAXMAX )
    {
    ...
    if ( (pvalb= pvals[baseid]) != 0 )
    {
        if ( relid == MAX_CURRENCIES )
        {
            ...
            if ( pvals[USD] != 0 && kmdbtc != 0 && btcusd != 0 )
            {
                ...
                usdvol = chunguscoin_paxvol(basevolume,baseusd);
                ...
                if ( height >= 236000-10 )
                {
                    ...
                    price = ((uint64_t)10000000000 * MINDENOMS[USD] / MINDENOMS[baseid]) / chunguscoin_paxvol(usdvol,usdkmd);
                    //LogPrintf("ht.%d %.3f kmdbtc.%llu btcusd.%llu base -> USD %llu, usdkmd %llu usdvol %llu -> %llu\n",height,BTCUSD,(long long)kmdbtc,(long long)btcusd,(long long)baseusd,(long long)usdkmd,(long long)usdvol,(long long)(MINDENOMS[USD] * chunguscoin_paxvol(usdvol,usdkmd) / (MINDENOMS[baseid]/100)));
                    //LogPrintf("usdkmd.%llu basevolume.%llu baseusd.%llu paxvol.%llu usdvol.%llu -> %llu %llu\n",(long long)usdkmd,(long long)basevolume,(long long)baseusd,(long long)chunguscoin_paxvol(basevolume,baseusd),(long long)usdvol,(long long)(MINDENOMS[USD] * chunguscoin_paxvol(usdvol,usdkmd) / (MINDENOMS[baseid]/100)),(long long)price);
                    //LogPrintf("usdkmd.%llu basevolume.%llu baseusd.%llu paxvol.%llu usdvol.%llu -> %llu\n",(long long)usdkmd,(long long)basevolume,(long long)baseusd,(long long)chunguscoin_paxvol(basevolume,baseusd),(long long)usdvol,(long long)(MINDENOMS[USD] * chunguscoin_paxvol(usdvol,usdkmd) / (MINDENOMS[baseid]/100)));
                } else price = (MINDENOMS[USD] * chunguscoin_paxvol(usdvol,usdkmd) / (MINDENOMS[baseid]/100));
                return(price);
        ...
        else if ( (pvalr= pvals[relid]) != 0 )
        {
            ...
            return(chunguscoin_paxvol(basevolume,baserel));
        }
...
uint64_t _chunguscoin_paxprice(uint64_t *kmdbtcp,uint64_t *btcusdp,int32_t height,char *base,char *rel,uint64_t basevolume,uint64_t kmdbtc,uint64_t btcusd)
{
    int32_t baseid=-1,relid=-1,i; uint32_t *ptr,*pvals;
    if ( height > 10 )
        height -= 10;
    if ( (baseid= chunguscoin_baseid(base)) >= 0 && (relid= chunguscoin_baseid(rel)) >= 0 )
    {
        //portable_mutex_lock(&chunguscoin_mutex);
        for (i=NUM_PRICES-1; i>=0; i--)
        {
            ...
            if ( *ptr < height )
            {
                ...
                if ( kmdbtc != 0 && btcusd != 0 )
                    return(chunguscoin_paxcalc(height,pvals,baseid,relid,basevolume,kmdbtc,btcusd));
                else return(0);
            }
        }
        //portable_mutex_unlock(&chunguscoin_mutex);
...
int32_t chunguscoin_kmdbtcusd(int32_t rwflag,uint64_t *kmdbtcp,uint64_t *btcusdp,int32_t height)
{
...
uint64_t _chunguscoin_paxpriceB(uint64_t seed,int32_t height,char *base,char *rel,uint64_t basevolume)
{
    ...
    if ( basevolume > CHUNGUSCOIN_PAXMAX )
    {
        LogPrintf("chunguscoin_paxprice overflow %.8f\n",dstr(basevolume));
        return(0);
    }
    ...
    //if ( chunguscoin_kmdbtcusd(0,&kmdbtc,&btcusd,height) < 0 ) crashes when via passthru GUI use
    {
        memset(btcusds,0,sizeof(btcusds));
        memset(kmdbtcs,0,sizeof(kmdbtcs));
        for (i=0; i<numvotes; i++)
        {
            _chunguscoin_paxprice(&kmdbtcs[numvotes-1-i],&btcusds[numvotes-1-i],height-i,base,rel,100000,0,0);
            //LogPrintf("(%llu %llu) ",(long long)kmdbtcs[numvotes-1-i],(long long)btcusds[numvotes-1-i]);
        }
        kmdbtc = chunguscoin_paxcorrelation(kmdbtcs,numvotes,seed) * 539;
        btcusd = chunguscoin_paxcorrelation(btcusds,numvotes,seed) * 539;
        //chunguscoin_kmdbtcusd(1,&kmdbtc,&btcusd,height);
    }
    for (i=nonz=0; i<numvotes; i++)
    {
        if ( (votes[numvotes-1-i]= _chunguscoin_paxprice(0,0,height-i,base,rel,100000,kmdbtc,btcusd)) == 0 )
            zeroes++;
        else
        {
            nonz++;
    ...
    //LogPrintf("chunguscoin_paxprice nonz.%d of numvotes.%d seed.%llu %.8f\n",nonz,numvotes,(long long)seed,nonz!=0?dstr(1000. * (double)sum/nonz):0);
    if ( nonz <= (numvotes >> 1) )
    {
        return(0);
    }
    return(chunguscoin_paxcorrelation(votes,numvotes,seed) * basevolume / 100000);
}

uint64_t chunguscoin_paxpriceB(uint64_t seed,int32_t height,char *base,char *rel,uint64_t basevolume)
{
    uint64_t baseusd,basekmd,usdkmd; int32_t baseid = chunguscoin_baseid(base);
    if ( height >= 236000 && strcmp(rel,"kmd") == 0 )
    {
        usdkmd = _chunguscoin_paxpriceB(seed,height,(char *)"USD",(char *)"KMD",SATOSHIDEN);
        if ( strcmp("usd",base) == 0 )
            return(chunguscoin_paxvol(basevolume,usdkmd) * 10);
        baseusd = _chunguscoin_paxpriceB(seed,height,base,(char *)"USD",SATOSHIDEN);
        basekmd = (chunguscoin_paxvol(basevolume,baseusd) * usdkmd) / 10000000;
        //if ( strcmp("KMD",base) == 0 )
        //    LogPrintf("baseusd.%llu usdkmd.%llu %llu\n",(long long)baseusd,(long long)usdkmd,(long long)basekmd);
        return(basekmd);
    } else return(_chunguscoin_paxpriceB(seed,height,base,rel,basevolume));
}
...
/*uint64_t chunguscoin_paxpriceB(uint64_t seed,int32_t height,char *base,char *rel,uint64_t basevolume)
{
    ...
    //    return(_chunguscoin_paxpriceB(seed,height,base,rel,basevolume));
    //else
    {
        baseusd = _chunguscoin_paxpriceB(seed,height,base,(char *)"USD",SATOSHIDEN);
        usdkmd = _chunguscoin_paxpriceB(seed,height,(char *)"USD",(char *)"KMD",SATOSHIDEN);
...
uint64_t chunguscoin_paxprice(uint64_t *seedp,int32_t height,char *base,char *rel,uint64_t basevolume)
{
    ...
    if ( ASSETCHAINS_SYMBOL[0] == 0 && chainActive.LastTip() != 0 && height > chainActive.LastTip()->GetHeight() )
    {
        if ( height < 100000000 )
        {
            ...
            if ( counter++ < 3 )
                LogPrintf("chunguscoin_paxprice height.%d vs tip.%d\n",height,chainActive.LastTip()->GetHeight());
        }
        return(0);
    }
    *seedp = chunguscoin_seed(height);
    portable_mutex_lock(&chunguscoin_mutex);
    for (i=0; i<17; i++)
    {
        if ( (price= chunguscoin_paxpriceB(*seedp,height-i,base,rel,basevolume)) != 0 )
        {
            sum += price;
            nonz++;
            if ( 0 && i == 1 && nonz == 2 )
...
int32_t chunguscoin_paxprices(int32_t *heights,uint64_t *prices,int32_t max,char *base,char *rel)
{
    ...
    if ( (baseid= chunguscoin_baseid(base)) >= 0 && (relid= chunguscoin_baseid(rel)) >= 0 )
    {
        for (i=NUM_PRICES-1; i>=0; i--)
        {
            ptr = &PVALS[36 * i];
            heights[num] = *ptr;
            prices[num] = chunguscoin_paxcalc(*ptr,&ptr[1],baseid,relid,COIN,0,0);
            num++;
            if ( num >= max )
                return(num);
        }
...
void chunguscoin_paxpricefeed(int32_t height,uint8_t *pricefeed,int32_t opretlen)
{
    ...
    chunguscoin_stateupdate(height,0,0,0,zero,0,0,pvals,numpvals,0,0,0,0,0,0,zero,0);
    if ( 0 )
    {
        int32_t i;
        for (i=0; i<numpvals; i++)
            LogPrintf("%u ",pvals[i]);
        LogPrintf("chunguscoin_paxpricefeed vout OP_RETURN.%d prices numpvals.%d opretlen.%d kmdbtc %.8f BTCUSD %.8f CNYUSD %.8f\n",height,numpvals,opretlen,KMDBTC,BTCUSD,CNYUSD);
    }
}

uint64_t PAX_fiatdest(uint64_t *seedp,int32_t tochunguscoin,char *destaddr,uint8_t pubkey33[33],char *coinaddr,int32_t height,char *origbase,int64_t fiatoshis)
{
    uint8_t shortflag = 0; char base[4]; int32_t i,baseid; uint8_t addrtype,rmd160[20]; int64_t chunguscoinshis = 0;
    *seedp = chunguscoin_seed(height);
    if ( (baseid= chunguscoin_baseid(origbase)) < 0 || baseid == MAX_CURRENCIES )
    {
        if ( 0 && origbase[0] != 0 )
    ...
    chunguscoinshis = chunguscoin_paxprice(seedp,height,base,(char *)"KMD",(uint64_t)fiatoshis);
    //LogPrintf("PAX_fiatdest ht.%d price %s %.8f -> KMD %.8f seed.%llx\n",height,base,(double)fiatoshis/COIN,(double)chunguscoinshis/COIN,(long long)*seedp);
    if ( bitcoin_addr2rmd160(&addrtype,rmd160,coinaddr) == 20 )
    {
        PAX_pubkey(1,pubkey33,&addrtype,rmd160,base,&shortflag,tochunguscoin != 0 ? &chunguscoinshis : &fiatoshis);
        bitcoin_address(destaddr,CHUNGUSCOIN_PUBTYPE,pubkey33,33);
    }
    return(chunguscoinshis);
}
